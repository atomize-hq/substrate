...
Packet `4` closed the Slice `55` validation wall without reopening runtime scope:

1. the final spec/plan/tasks stack now records Slice `55` as a bounded contract-freeze/productization slice,
2. the gap matrix and remaining-scope note no longer treat caller-surface wording convergence as the next open seam,
3. deferred follow-ons remain explicit: default-agent UX, broader non-REPL targeting, public member-root lifecycle, and richer write-side selector ergonomics.

## Current tree note

1. Slice `54` landed selected-host runtime-family host-tool parity and is no longer the active next seam.
2. Slice `55` closes the caller-surface contract freeze above the already-landed prompt-taking runtime foundations.
3. The repo now freezes default backend versus default-agent routing, shell-wrap-only boundaries, exact public follow-up selectors, and host-rooted world-backed `start` wording without widening runtime behavior.
4. Later work remains deferred rather than implicit: default-agent UX, broader non-REPL targeting, public member-root lifecycle, richer write-side selector ergonomics, and separate read-side/control hardening.

## Task List

- [x] Task 55.1: Freeze the caller-surface decision ledger in repo-facing docs
  - Acceptance:
...
- [x] Task 55.2: Reconcile operator-facing wording for prompt-taking vs shell-wrap surfaces
  - Acceptance:
...
- [x] Task 55.3: Reconcile host-rooted world-backed start wording against deferred member-root lifecycle
  - Acceptance:
...
- [x] Task 55.4: Preserve the exact public follow-up selector contract and shell-wrap regression floor
  - Acceptance:
...
- [x] Task 55.5: Close the Slice 55 validation wall and document what remains deferred
  - Acceptance:

...
4. `crates/shell/tests/**` and inline `#[cfg(test)]` coverage may retain legacy names only for explicit bridge-removal coverage, persisted-state continuity coverage, or fail-closed retirement assertions. Packet `4` must shrink those remaining hits to explicit fail-closed guidance, explicit negative tests, or historical coverage only.
...
- **Manual diff review**: ensure remaining split-entry legacy-name hits and remaining unqualified pre-placement exact-selector hits are explicit fail-closed guidance, explicit negative tests, or historical coverage only, not forward product truth.

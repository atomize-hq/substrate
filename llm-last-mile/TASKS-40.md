# TASKS-40: Internal Retained Host Approval Response Bootstrap
...
Source plan: [PLAN-40.md](./PLAN-40.md)  
...
- Packet 1 freezes the typed approval-response contract and minimal policy surface.
- Packet 2 lands approval-obligation consumer semantics.
...
1. accept typed `approval_response` host payloads on `continue_world_worker`,
...
1. the typed `approval_response` host payload is frozen,
...
1. bind typed approval responses to exact unresolved `ApprovalRequired` obligations,
...
1. every typed approval response has one authoritative approval-obligation consumer path,
2. resolve and dismiss semantics stay explicit and durable,
...
1. deliver typed approval responses through the live `continue_world_worker` path,
...
1. live continue handling can deliver the typed approval-response slice,
2. delivery failures leave obligations unresolved,
3. successful delivery performs deterministic one-time approval-obligation closeout,
...
- [x] Task 4.1: Align planning/config truth without widening the slice
  ...
  - Expected files touched:
    ...
    - [`llm-last-mile/SPEC-40-internal-retained-host-approval-response-bootstrap.md`](./SPEC-40-internal-retained-host-approval-response-bootstrap.md)
...
- [x] Task 4.2: Run the final validation wall
  ...
  - Expected files touched:
    - final validation may require bounded follow-up fixes inside already-touched Slice `40` runtime surfaces, but must not reopen broader host-response/control widening, active-ephemeral identity, or Family-2 router execution

### Packet 4 Checkpoint

Packet 4 is complete only when:

1. the Slice `40` surface is safely bounded,
...
1. honest typed approval-response delivery requires transport-api widening first,
2. approval-obligation closeout cannot stay exact and fail-closed on the current local ledger model,
3. deterministic prompt rendering proves unable to carry the typed approval-response contract honestly,

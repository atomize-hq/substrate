...
| **RG-EVENT-01** | Runtime event/frame identity, ordering, and exact terminal event | **Unresolved** | `AgentEvent` has attribution but no canonical event ID/sequence. `ExecuteStreamFrame` has no stream/frame sequence, and `Exit` has span ID but no exact terminal event identity/order. Producers use process-local channels; host arrival order or stream exhaustion is not replay-safe truth. | B0 makes the runtime producer assign one stable stream ID, strictly monotonic frame/event sequences, stable semantic event IDs, and an exact terminal event ID/sequence before emission; it never originates gaps, reorder, conflicting reuse, or post-terminal frames. Exact byte-identical replay preserves identity and B2.1 consumes it as a no-op; B2.1 rejects conflicting reuse, gaps, reorder, frames after terminal, and EOF-as-terminal. | B0, B2.1 |
...
dependency-ready. B0 is the exact next implementation packet; A1 as a whole remains incomplete and
non-landable.

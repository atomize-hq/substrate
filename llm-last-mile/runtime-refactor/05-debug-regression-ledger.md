...
| **RG-AUTH-03** | Revision-bound host transition intent proof | **Unresolved** | `HiddenOwnerHelperLaunchPlan` currently carries authoritative `Start`/`Attach`/`ResumeOneTurn` mode, identity, lineage, workspace/world binding, descriptor, attach contract, resume handle, and input through arbitrary JSON. Plan load/removal and helper execution do not join a durable authority revision or expected absence to a unique replay-safe intent, and no `Issued`/`Claimed`/`Applied` reconciliation, revisioned input handoff, or retained reprojection payload prevents load-then-remove/helper failure, stale replay, substitution, or duplicate application/input from stranding or rewriting authority. | On real public CLI and REPL paths, prove `Start` against durable expected absence and `Attach`/`ResumeOneTurn` against the exact authority revision/hash and enumerated source posture, with exact caller/source/target lineage, workspace/world binding, descriptor/inline-attach/resume/input commitments, unique intent/request identity, fixed expiry, canonical intent/payload hashes, and retained transport reprojection through terminal handoff. Exercise `Issued`, `Claimed`, `Applied`, terminal `Rejected`, `Expired`, attempt rejection without valid-intent mutation, and `NotApplicable`/`Pending`/`Accepted`/`TerminalWithoutAcceptance` input handoff; crash after issue, after plan load/removal, after claim, after authority commit, and after input acceptance before result observation; prove restart reprojection/reconciliation, terminal payload cleanup, and exact applied/input retry join from an older read revision without a second transition or delivery. Stale revisions used for mutation, substituted plans, mismatched hashes/identity/binding/descriptor, expired intent, superseded claim, and conflicting replay fail closed. The bounded auto-attach producer uses the same contract, and `RG-BASE-01` remains green. A1 closes only its scoped clauses of `RG-AUTH-01`/`RG-AUTH-02`; those ledger-wide gates remain unresolved for A2/A3. | A1 |
...
1. Issue a public `Start` intent against expected absence, then terminate delivery once while `Issued`, once after plan load/removal but before claim, once while `Claimed`, once after the authority commit, and once after input acceptance but before success is observed.
2. Restart and retry the exact intent; prove missing plan transport is reproduced from the retained payload, pre-application cases safely resume/reconcile, and committed/accepted cases join one immutable result without another authority revision, participant allocation, or input delivery. Prove payload cleanup occurs only after exact terminal handoff.
3. Repeat public CLI and real REPL `Attach` and `ResumeOneTurn` against exact revisions, including bounded auto-attach plan production, and prove the same identity/binding/descriptor commitments reach application.
4. Reject stale revision, substituted plan, payload/ref hash mismatch, wrong caller/source/target/lineage, wrong workspace/world generation, expired intent, superseded claim, and conflicting replay before authority mutation.
5. Run public world-scoped start → reattach → stop and prove exact session/world binding plus durable terminal truth are unchanged.

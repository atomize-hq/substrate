...
```text
A0 -> A1.1e -> B0 -> B1 -> B2.1 -> B3.1 -> C1 -> A1.2 -> A1.3 -> A1.4 -> A1
...
authority foundation for the bounded B0/B1/B2.1/B3.1/C1 prerequisite corridor even while A1.1d
...
2. B1 precedes B2.1 because a durable observer claim and journal require one accepted task or
   active-run identity. B1 also preallocates the proposed acceptance ID and typed request context
   that world-service retains before B3.1 can emit an exact accepted-run envelope; the proposal is
   not accepted truth until the B0 acknowledgement is durably recorded.
...
The A1.1d cross-packet integration hold permits the bounded B0/B1/B2.1/B3.1/C1 prerequisite
corridor despite open integrated Linux and native-macOS closeout. The corridor moves only the
...
normative current rule is stricter: do not restore, modify, or resume A1.2 until B0, B1, B2.1,
B3.1, and C1 have landed.
...
- The newly authorized implementation order is B0, B1, B2.1, B3.1, C1, then A1.2. B0 is landed and
  B1 is the exact next packet. B1 is required before B2.1; B3.1 is required before C1; no A2/A3
  prerequisite moves into the corridor.
...
### B1 exact carrier and activated-store allowlist
...
3. **B1-4 — production proof and closeout:** prove the real acknowledgement paths, differential
   baseline, supported doctor/smoke, and review-clean closeout without beginning B2.1.
...
| **B1 — Receipt registry and acceptance records** | Persist `WorldWorkAcceptanceRecordV1` for both ephemeral tasks and retained turns from an exact B0 pre-terminal runtime acknowledgement; keep accepted identity inspectable independently of foreground scope without claiming the final model-facing receipt. | WorldWorkReceiptRegistry. | `02` ReceiptRegistry row; `04` acceptance context/record + active receipts + receipt acceptance; `05` receipt rows; tool-invocation async model. | HostSessionAuthority; RuntimeEventTransport; EffectivePolicyResolver; B2.1 supervisor handoff; RetainedWorkerRuntime; E2 policy commitments. | Exactly the canonical ownership, activated-store substrate, and carrier-constructor/test files enumerated in **B1 exact carrier and activated-store allowlist** above; no implicit wildcard or additional integration site is authorized. | No host transition issuance, authentication, or interpretation, no HostSessionAuthority schema/hash/verifier edit, no generic activated-store writer, no weakening of legacy-writer exclusion, no `ExecuteStreamFrame` change, no accepted record before runtime acknowledgement, no final retained receipt/policy-cap contract, no stream ownership, no foreground early return, no retained lifecycle/park, no obligation materialization, no terminal-only identity relabeled as acceptance, and no UAA mediation. | ReceiptRegistry preallocates a proposed acceptance-record ID and sends it with exact request/message/caller-backend identity plus optional shared correlation on the existing typed task/turn request; the proposal alone is not accepted truth. World-service retains the exact context. A valid B0 acknowledgement for that request durably creates exactly one record with the same ID and exact store/session/caller participant/backend/target/world/current-policy/stream/sequence identity. The optional correlation remains absent on production paths until the host-transition owner later supplies it; B1 retains it unchanged and validates only structural and record-scope equality. Exact retry reuses and joins the proposal; conflict fails closed; caller drop/restart retains and exact inspect finds only acknowledged work. The current blocking UX remains. | Acceptance clauses of `RG-RECEIPT-03` and `RG-BASE-02`; B1 acceptance clause of `RG-OBS-01`. | B0 and landed A1.1e. B1 is required before B2.1; its optional carrier does not require A1.2 to land and cannot authenticate, mint, verify, or interpret a correlation. | E2 immutable cap/final receipt commitments, A1.2 production correlation supply, supervisor claim/closeout/reconciliation, foreground early return, `RG-RECEIPT-04`, full `RG-RECEIPT-01`/`RG-RECEIPT-02`, retained lifecycle, obligations, and seam promotion. |
| **B2.1 — Supervisor handoff and durable observation journal** | Transfer sole post-acceptance stream ownership into a receipt-scoped durable journal with restart-safe observation, dedupe, ordering, and terminal reconciliation while preserving blocking foreground behavior. | WorldWorkExecutionSupervisor. | `01` invariants 4–6; `02` supervisor row; `04` runtime carrier, receipt acceptance, and supervisor rules; `05` `RG-SUP-01`/`RG-SUP-02`/`RG-OBS-01`. | RuntimeEventTransport; ReceiptRegistry; MessagingProtocol; ObligationLedger; RetainedWorkerRuntime. | Bounded supervisor/journal module under `crates/shell/src/execution/`; `execution/orchestrator_world_dispatch.rs`; bounded receipt/journal persistence in `agent_runtime/state_store.rs`; bounded transport API/client and `crates/world-service/src/{service,member_runtime}.rs` journal/replay/reconciliation plumbing owned by the supervisor; focused claim/no-gap/dedupe/restart/reconciliation tests. | No foreground early return, no runtime event identity generation, no event taxonomy or retained-message semantics, no obligation classification/materialization, no inbox/router behavior, no provider policy broker, and no PID/helper/socket/EOF/timeout inference as terminal proof. | At B1 acceptance, the supervisor claim and durable journal become the only observation path with no unjournaled post-acceptance gap. It records exact B0 frames/events and the immutable B1 fields before waiter delivery, publishes a stable generic journal-event ref committing the exact canonical runtime event bytes without interpreting B3.1 semantics, makes byte-identical replay a no-op, rejects gaps/reorder/conflicting reuse/stale observers, and restarts from its durable cursor plus exact producer replay/reconciliation. Missing exact terminal truth records interruption/protocol failure but cannot close the run or C1 cut. Only the exact B0 terminal event closes observation. A compatibility waiter may still block on the receipt. | Consumer clauses of `RG-EVENT-01`; `RG-SUP-01` observation/receipt clauses; `RG-SUP-02`; `RG-RECEIPT-03`; `RG-RECEIPT-04`; B2.1 clauses of `RG-OBS-01`. | B0, B1, and landed A1.1e. | Model-facing early return (`B2.2`), retained semantic envelope and its validation (`B3.1`), obligation clauses of `RG-SUP-01`, C1 cut, cancel, and seam promotion. |
...
| **B4 — Receipt-targeted cancel/inspect/stop** | Resolve control against active receipts and worker manifests with distinct durable outcomes. | WorldDispatchControl resolves the verb; ReceiptRegistry owns active-work identity; RetainedWorkerRuntime owns worker stop lifecycle. | `04` cancel categories + supervisor cancellation rules; `02` WorldDispatchControl/ReceiptRegistry/RetainedRuntime rows; `05` cancel/closeout rows. | HostSessionAuthority; Supervisor; private transport episode status. | `orchestrator_world_dispatch.rs`; `agent_runtime/{state_store,dispatch_contract,control}.rs`; world-service cancel seam; control tests. | No weakening exact identity, no treating no active run as stale linkage, and no stop/cancel conflation. | Same-turn continue→cancel targets active run; no-active, terminal, unreachable, invalid, mismatch, ambiguity, and policy denial are distinct; closeout is idempotent. | `RG-CANCEL-01`, `RG-CANCEL-02`, `RG-CLOSE-01`, `RG-BASE-04`; B4 cancel clause of `RG-OBS-01`. | B2.2 and B3.2. | C2/C3 and unrelated lifecycle/policy work. |

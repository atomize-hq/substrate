...
| **A1.3-P0 — authority-bound transport split preparatory packet** | Isolate the public Start/turn/reattach hidden-owner transport from the legacy launcher corridor before A1.3 resumes. The later code task may create a new transport-only launcher or a mechanically isolated authority-managed branch for those public callers only. `HostSessionAuthority` remains the sole durable owner; plans remain immutable transport projections; readiness/process/prompt/timeout/EOF/local-adapter state remains observation only. | Only the exact public-launch transport branch in `agent_runtime/control.rs`; only the named public `run_start`/`run_turn`/`run_reattach`/`run_owner_helper` regions in `execution/agents_cmd.rs`; only owner-helper validation/application plus the exact public startup/post-turn adapter regions in `repl/async_repl.rs`; only explicit-bootstrap-home entry points needed by those callers; focused public control-surface/helper integration tests. | Immutable plan projection; exact public caller/source/target lineage; launch-only transport handoff; exact startup/post-turn evidence transport into HostSessionAuthority; frozen legacy caller split between public callers and compatibility/auto-attach callers. | No HostSessionAuthority or StateStore schema/persistence-semantics change; no joined readiness/continuity receipt reconstruction; no legacy `persist_orchestration_session` or `persist_participant` authority writes on the isolated public path; no readiness/process-derived startup or terminal inference; no `auto_attach.rs` edit, producer change, settlement change, or behavior change; no A2 demotion, A3 persistence split, endpoint redesign, receipt/supervisor/member-runtime work, macOS, Windows, or R3. | Bind the frozen WIP evidence, the public-path trace into the legacy launcher, the exact forbidden write/inference boundaries, the current plan/process lifecycle, and the unchanged auto-attach baseline before code begins. Then prove public Start/turn/reattach use only the new isolated transport path, legacy callers remain unchanged, no legacy authority writes or readiness/process inferences survive on the isolated path, and independent review is clean. Exiting A1.3-P0 only permits the later A1.3 packet to resume. | A1.3 public/successor adoption, `RG-BASE-01`, A1.4 auto-attach adoption, and A1 completion remain unclaimable. |
| **A1.3 — public/successor adoption and startup-result completion** | Resume only after A1.3-P0 lands cleanly. Consume the separately isolated public transport split, finish public Start adoption around the bounded A1.2a-S internal bootstrap without changing its applied identity, resolve the still-Pending A1.2a-S startup result, perform real-path parked-successor adoption, and close `RG-BASE-01`. | The landed A1.2 and A1.3-P0 public transport integration; only the named intent issuance/consumption regions in `execution/agents_cmd.rs`; only owner-helper validation/application, exact startup/post-turn event submission, remaining public/real REPL adoption, and explicit bootstrap-home threading in `repl/async_repl.rs`; only the A1.1 explicit-home resolver entry points needed by those named CLI/REPL paths; focused CLI, helper, and REPL integration tests. | Mode-specific preconditions; exact caller/source/target lineage; bootstrap-home/workspace/world binding; typed descriptor/attach/resume/policy/input refs; claim ownership; authority/intent revisions; applied result; exact actor-bound ownership-acknowledgement/pre-ownership rejection/failure event identity and result; exact actor-bound post-turn event ID/sequence/outcome/reason and one-turn disposition. | No rewrite of A1.2a-S initial identity or bound capability, resolver precedence, policy/config/inventory interpretation, or broad REPL lifecycle cleanup; no legacy-launcher/control-path redesign already owned by A1.3-P0; no general episode model rewrite or relabeling of readiness/PID/helper/socket/timeout/EOF/local errors as startup or post-turn protocol evidence; no private endpoint/path redesign; no receipt, supervisor, or member-runtime work. | Prove the bounded internal Start's Pending ownership resolves only from exact actor evidence; a public turn from parked truth issues/applies `ResumeOneTurn`; explicit reattach issues/applies `Attach`; and the landed A1.3-P0 public transport split launches the helper/REPL only after durable application. Prove exact `OwnershipAccepted` accepts ownership, exact `RuntimeCreationRejected`/`StartupFailedBeforeOwnership` terminally reconcile with matching actor/evidence/result identity, and timeout/drop/EOF/readiness/process posture or local adapter error remains Pending. Every applied Resume terminal outcome has an immutable exact-scope protocol event, matching completion plus post-turn application, and closed input terminalization; exact retry joins; the exact public start → turn/reattach → stop path becomes green. Retain existing negative tests for helper loss, stale revision, ambient-home/CWD reread, substitution, role mismatch, duplicate application, conflicting startup evidence, and substituted post-turn events. | Bounded auto-attach producer adoption; complete A1 regression closure; A1 completion. |
...
historically selected [A1.3](linux-first-runtime-resumption/A1.3-LINUX-FIRST-PACKET.md). The
controlling amendment now makes
[A1.3-P0](linux-first-runtime-resumption/A1.3-P0-LINUX-FIRST-PREPARATORY-PACKET.md) the active
bounded Linux-first implementation/proof fence and holds A1.3 open behind it.
...
Neither lane dispatches implementation or the other's successor. A1.3 remains held open behind
A1.3-P0; A1.4, Windows, E03, and all other product work remain undispatched until their own fresh
exact authority packets activate them. See

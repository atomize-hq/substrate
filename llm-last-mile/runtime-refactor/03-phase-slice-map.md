...
| Exact production sections/symbols | Shared `InstallBootstrapContextV1`/carrier and fixed line framing in transport types; OS principal/path construction and validation in the one shell module; exact Unix release-link/dev-A-bin-symlink installed-product invocation-witness construction; current Unix principal binding for internal carriers; module declaration only in execution `mod.rs`; `Cli.install_prefix`, hidden `Cli.install_bootstrap_context_v1`, and the `auto_sync.rs` literal-field compatibility update; `run_shell`/`run_shell_with_cli` construction/child-discrimination/principal-binding order, non-mutating parse/help/version exits, mandatory checked environment projection, typed leaf dispatch, explicit-context call to a new `home_bootstrap` intake that preserves every R1 physical/security semantic, and one pre-manager/policy trace binding to `A/trace.jsonl` plus policy Git directory A; dev install/uninstall prefix/context/bootstrap, shim invocation, projection writers, and the exact PI-003 copy from the already-selected version manifest to generated base `A/manager_hooks.yaml` only after IH exists; dev-shim parse/context, existing shim-call carrier, and `write_env_file`; `render_env_sh`, `write_env_sh_at`, `MANAGER_ENV_SCRIPT`, `write_manager_env_script_at`; invocation-plan deploy/remove/status carrier plumbing, A-derived preexec path, and trace reuse only; `BASH_PREEXEC_SCRIPT` plus `write_bash_preexec_script` explicit A/self-binding; `ShimDeployer` context/path constructor and automatic deploy call only; shim doctor/repair dispatch signatures, report/path derivation, intended-principal account-home repair target, text/JSON projection, and bound-trace reuse; manager and routing-telemetry trace calls may only reuse the entry-bound projection. In `trace/context.rs`, the existing global `TraceContext` gains an immutable explicit policy Git directory, `init_trace(Some(A/trace.jsonl))` is the only normal-product first initialization, and `init_trace(None)` may only reuse an initialized output or fail; in `trace/util.rs`, `get_policy_git_hash` accepts the explicit directory and never calls `dirs::home_dir()`. An explicitly supplied diagnostic/test trace path is non-product and has no implicit policy directory. Trace writer, rotation, retention, serialization, span, replay, policy, and environment-hash semantics are byte-frozen. `ShimDeployer::{ensure_deployed,deploy_shims,migrate_old_shims}` replacement/migration predicates/bodies, invocation-plan recursive shim-remove action, old ambient-home `.substrate_preexec` removal, repair write/backup semantics, and all installer deletion/removal/recursive-fallback sections are byte-frozen except carrier/target arguments at existing calls. The A1 `CanonicalJsonV1` implementation and every HostSessionAuthority semantic caller are frozen. |
...
| GitNexus posture | LOW direct impact is expected for routing/home-bootstrap and MEDIUM/HIGH at shared script/invocation/diagnostic leaves; the planning check found MEDIUM test-only fan-out for `TraceContext::init_trace` and LOW for `get_policy_git_hash`. `substrate_home` is already CRITICAL and remains frozen. Any edit to `crates/common/src/paths.rs::substrate_home`, trace serialization/rotation semantics, A1 canonical/authority code, a new CRITICAL result, or an unexpected world/runtime process is a stop-and-review condition, not implicit expansion. |

...
B1/B2.1-R0 → B3.2a builds the authority/retained branch. Those branches first join at
...
The B1/B2.1 production-ingress ownership audit selects **Case B**. A1.2a first provides the
current-authority protocol and exact read; A1.2a-WB must correct its Host/world-binding validation;
A1.2a-S must then adopt its greenfield Start on the
ordinary internal host bootstrap and carry the bound capability into the live toolbox. B1/B2.1-R0 must then provide the minimal
...
B3.2a must supply the real Spawn integration plus canonical admission/routability truth that R0
deliberately lacks; B1/B2.1-0 then partitions
...
Only the Host/world-binding Start validation condition and its directly colocated matrix, zero-
mutation, retry/conflict, authority-preservation, placement-separation, and unchanged-byte tests may
change. Any required file or symbol outside this list is `ImpactDecisionRequired`.
...
participant ID across processes. Before transport, the bounded `SpawnWorldWorker` integration
...
park/cancel/stop/fork, or live-admission semantics. B3.2a alone connects that protocol to the real
Spawn ingress and owns the minimum durable admission/routability record. B1/B2.1-0 consumes the
A1.2a, R0, and B3.2a read results, authorizes at most one
...
retained truth, or broaden the two-file runtime boundary. A1.2a is already landed. This correction
authorizes only A1.2a-WB after its docs review; A1.2a-S, B1/B2.1-R0, B3.2a, and B1/B2.1-0 remain
gated by their applicable independent clean review and exact allowlist.

The A1.1d cross-packet integration hold keeps the preserved B0 → B1 receipt core → B2.1 branch
independent while A1.1e → A1.2a → A1.2a-WB → A1.2a-S → B1/B2.1-R0 → B3.2a builds the
...
- The preserved B0 → B1-3a/B1-3b receipt core → B2.1-1/2/3 branch remains independent while
  A1.1e → A1.2a → A1.2a-WB → A1.2a-S → B1/B2.1-R0 → B3.2a builds the authority/retained branch. They join
  only at B1/B2.1-0, then proceed through the joint B1/B2.1
  production closeout, B3.1, C1, and A1.2b. B0 and the review-clean B1/B2.1 cores are already
  preserved; A1.2a is independently review-clean and the exact next implementation packet, after
  this docs-only correction is independently review-clean, is A1.2a-WB. B3.1 still requires the joint closeout; no
  A2/A3 prerequisite moves into the corridor.
...
After all three B2.1 subpackets plus review-clean A1.2a, A1.2a-WB, A1.2a-S, B1/B2.1-R0, B3.2a, and B1/B2.1-0,
...
This docs-only correction does not yet authorize those edits. Once the corrected sequence is
independently review-clean, the two-file boundary is not a wildcard and does not authorize semantic
...
| **B3.2 — Remaining retained receipt, messaging, and lifecycle completion** | Complete model-facing retained receipts, host-to-worker envelopes, delivery semantics, accepted-turn lifecycle/park behavior, and full creation-observer restart reconciliation without changing B3.1 event truth or B3.2a's already-landed registration/admission identities. | WorldWorkerMessagingProtocol owns message semantics; RetainedWorkerRuntime owns worker lifecycle. | `02` MessagingProtocol + RetainedWorkerRuntime rows; `04` retained receipt/manifest and B3.2a admission state; full messaging and lifecycle designs. | Supervisor; SteeringPolicyEngine; ObligationLedger; B4 cancel. | `orchestrator_world_dispatch.rs`; `agent_runtime/{dispatch_contract,session,state_store}.rs`; `world-service/member_runtime.rs`; focused retained lifecycle tests. | No auto-attach, no rewrite of B3.2a registration identity or exact terminal truth, no obligation ownership transfer, and no dispatch narrowing beyond carrying E2 snapshot/cap refs. | Continue returns the B1 accepted active-run receipt completed by E2 through B2.2; host-to-worker messages preserve exact identity/causation; clean turn parks without losing worker continuity; creation-observer interruption/restart reconciles without liveness inference; failure is durable and exact. | Remaining `RG-RECEIPT-02`, `RG-MSG-01`, `RG-BASE-04`, `RG-WORKER-EXIT-01`. | B3.2a, A1, A2/A3, E2, B2.2, B3.1, and C1. | B4 cancel/inspect/stop, C2/C3, and auto-attach. |

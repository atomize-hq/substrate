...
| Shim-doctor module export restriction | Unix only: `crates/shell/src/builtins/shim_doctor/mod.rs` may expose the already-existing `report::collect_report_for_context` with the narrowest crate-private visibility sufficient for sibling `health.rs`, equivalent to `#[cfg(unix)] pub(crate) use report::collect_report_for_context;`. It may not make `report` public, add a wrapper/resolver, alter any collector or output, change non-Unix behavior, or promote environment-compatible `collect_report` into the typed-IH route. This authorization is name visibility only. |
| Bounded Route A CRITICAL aggregate | For the preserved nine-file Route A WIP plus the single mechanical `shim_doctor/mod.rs` export, initial unstaged detection reported CRITICAL, 33 changed symbols, 18 affected existing processes, and nine files; reverse committed comparison reported CRITICAL, 11 changed symbols, the same 18 processes, and the same four existing modules (Builtin, Execution, Invocation, Platform). The count difference is comparison-direction/hunk-attribution variance and grants no semantic breadth. The exact processes are `Run → Lookup_unix_account_by_uid`, `Run → Validate`, `Run → Encode`, `Run → As_path`, `Run → WorldDisableAttribution`, `Run → WorldDisableSource`, `Run → As_str`, `Run → Current_dir`, `Run → CliConfigOverrides`, `Handle_host_command → As_str`, `Handle_host_command → Workspace_legacy_settings_path`, `Handle_host_command → For_path`, `Handle_host_command → Resolve_replace`, `Handle_host_command → ConfigExplainKey`, `Handle_host_command → I64ClampInfo`, `Handle_host_command → ConfigExplainSource`, `Handle_host_command → WorldDisableAttribution`, and `Handle_host_command → WorldDisableSource`. This aggregate is approved only for `ShellConfig::from_cli` explicit typed IH through Host/Health/Config/Policy, their exact named handlers/consumers, and the one Unix crate-private re-export. All processes remain existing Route A families; non-Route-A line-shift/test-colocation detections remain textually and semantically unchanged. Any new HIGH/CRITICAL symbol, process family, authority source, environment fallback, global state, physical-shim migration, capability, or lifecycle change stops for a new decision. |
...
| GitNexus posture | HIGH is expected around runtime/world-enable and service orchestration; impact must be reported before edits. The bounded world-enable CRITICAL aggregate, manager-environment HIGH, and child-boundary HIGH apply only to the preserved PI-027/PI-071 closure. The separate bounded Route A CRITICAL aggregate applies only to its preserved nine-file WIP plus the mechanical Unix crate-private `shim_doctor/mod.rs` export, with the comparison-direction counts and exact existing processes recorded above. The Host/Health/Config/Policy `ShellConfig::from_cli` HIGH, separately authorized hidden owner-helper use of that same HIGH graph result, additive `WorldDoctorReportV1` HIGH, and `resolve_current_inventory_view` HIGH apply only to the exact branch/field/argument transports and counts recorded above. Large-file PI-105 symbols remain graph-under-resolved and require manual closure. Any broader CRITICAL/HIGH result, new module, new process family, or change outside the named route stops for explicit review. |

...
integration closeout is now recorded below; B3.1 is dependency-ready, and no seam is promoted.
...
previous authority wall `AUTHORITY_REQUIRED:B1_B2_1_JOINT_CLOSEOUT` is now closed. The next
candidate implementation line is `B3.1 -> C1 -> A1.2b`. Only after that corridor lands may a
fresh user/meta authority gate revalidate live repository truth and reopen the preserved future
edge `AUTHORITY_REQUIRED:R3_IMPLEMENTATION -> A1.1d-5R3-HOME`; R3 must then complete before A1.3,
A1.4, or A1 closeout. Every later edge remains an explicit authority gate.

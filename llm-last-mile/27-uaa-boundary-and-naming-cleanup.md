<!-- /autoplan restore point: /Users/spensermcconnell/.gstack/projects/atomize-hq-substrate/testing-autoplan-restore-20260521-102622.md -->
...
- or renaming true upstream `agent_api` identifiers.
...
- and docs stop using `world-agent` / `agent-api-*` as the conceptual names for the local transport boundary, except where historical references are explicitly being renamed to `world-service` and `transport-api-*`.
...
- the local in-world daemon is renamed to the `world-service` family,
...
- The pure-agent protocol label is centralized enough to migrate safely if a dual-read/write plan is added.
...
The review found follow-up work, but the main SOW direction itself is under challenge, so deferring implementation TODOs before the naming contract is resolved would create noise.
...
| CEO plan             | skipped                                     |
| Outside voice        | skipped (Claude auth unavailable)           |
...
- Consequence: the implementation must hard-gate the cutover with exhaustive repo-wide search, installer/bundle/CI validation, and fail-closed checks instead of relying on compatibility aliases.
...
| 5 | CEO | Challenge the greenfield premise | User Challenge | Choose completeness | Live install, CI, and runtime artifacts make migration/support real | Accepting "no migration concern" |

...
- [30-public-world-scoped-agent-start-and-capability-flags.md](./30-public-world-scoped-agent-start-and-capability-flags.md)
  - Landed Linux-first public world-start slice: `substrate agent start --scope world` now keeps durable authority host-rooted, supports public capability narrowing through `--disable-capability` / `--disable-cap`, persists authoritative host-attach truth plus world binding before `start` returns, and still does not expose standalone member-root public lifecycle.

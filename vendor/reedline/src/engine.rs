...
use {
    crate::external_printer::{ExternalBytePrinter, ExternalPrinter},
...
pub struct Reedline {
    ...
    #[cfg(feature = "external_printer")]
    external_byte_printer: Option<ExternalBytePrinter>,
}
...
impl Reedline {
    ...
    fn read_line_helper(&mut self, prompt: &dyn Prompt) -> Result<Signal> {
        ...
        loop {
            #[cfg(feature = "external_printer")]
            if let Some(ref external_byte_printer) = self.external_byte_printer {
                let bytes = Self::external_byte_messages(external_byte_printer)?;
                if !bytes.is_empty() {
                    self.painter
                        .print_external_bytes(&bytes, self.editor.line_buffer(), prompt)?;
                    self.repaint(prompt)?;
                }
            }
    ...
    /// Adds an external byte printer.
    ///
    /// This is intended for rendering raw terminal/PTY bytes while editing a line.
    ///
    /// ## Required feature:
    /// `external_printer`
    #[cfg(feature = "external_printer")]
    pub fn with_external_byte_printer(mut self, printer: ExternalBytePrinter) -> Self {
        self.external_byte_printer = Some(printer);
        self
    }
    ...
    #[cfg(feature = "external_printer")]
    fn external_byte_messages(external_byte_printer: &ExternalBytePrinter) -> Result<Vec<u8>> {
        let mut bytes = Vec::new();
        loop {
            let result = external_byte_printer.receiver().try_recv();
            match result {

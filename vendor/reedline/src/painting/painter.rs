...
impl Painter {
    ...
    /// Prints external raw bytes (e.g. PTY stdout) while editing a line.
    ///
    /// This function is byte-safe and intended for output that may be non-UTF8.
    ///
    /// This function doesn't flush the painter buffer beyond what is required to
    /// position the cursor before writing bytes. A subsequent repaint should be
    /// used to restore the prompt/buffer.
    #[cfg(feature = "external_printer")]
    pub(crate) fn print_external_bytes(
        &mut self,
        bytes: &[u8],
        line_buffer: &LineBuffer,
        prompt: &dyn Prompt,
        ...
        // Clear the current terminal line before emitting external bytes so we
        // don't leave prompt fragments interleaved with output.
        let erase_line = format!("\r{}\r", " ".repeat(self.screen_width().into()));
        self.stdout.queue(Print(&erase_line))?;
        self.stdout.flush()?;

        // Render bytes to stdout (not the painter stream) to preserve existing
        // stdout/stderr expectations in interactive shells.
        let mut out = std::io::stdout();
        out.write_all(bytes)?;
        out.flush()?;

        // Re-calibrate the prompt origin based on the terminal cursor position
        // after printing bytes so the subsequent repaint can restore the input buffer.
        self.initialize_prompt_position(None)?;

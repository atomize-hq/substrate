...
| Broader caller-surface contract | `Packet 1 ledger frozen; follow-through still open` | The repo now has two narrow, explicit caller surfaces: REPL `::<backend_id> <prompt>` and public `substrate agent start|turn|reattach|fork|stop` with exact selectors and host-only root `start`, and the Packet `1` ledger above now freezes default backend vs default-agent routing, shell-wrap-only boundaries, exact selectors, and host-rooted world-backed start wording | Remaining Slice `55` work is follow-through rather than contract invention: align operator-facing wording, keep the regression floor honest, and continue deferring default-agent UX, broader non-REPL targeting, public member-level selectors, and standalone member-root public lifecycle |

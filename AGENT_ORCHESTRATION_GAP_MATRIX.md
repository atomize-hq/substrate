...
| User-turn submission into active UAA sessions | `Landed, Linux-first` | The REPL can now submit arbitrary targeted follow-up turns into retained active sessions: host follow-up turns resume only the active orchestrator backend, and Linux world follow-up turns go through the typed `/v1/member_turn/stream` route using persisted `internal.uaa_session_id` rather than replaying bootstrap prompts | Remaining work is broader operator-surface breadth, a non-REPL caller surface, and platform parity. On macOS specifically, world follow-up turns still fail closed because the non-Linux shell path does not yet ready or resume a world member runtime |
...
| Explicit backend/member targeting | `Partially landed, REPL-first` | Effective inventory entries derive canonical `backend_id` values, targeted REPL follow-up turns now route by exact `backend_id`, host follow-up targeting fails closed unless it names the active orchestrator backend, and Linux world follow-up targeting no longer relies on ambiguous "pick the eligible member" behavior | Remaining work is broader productization: targeted invocation is still REPL-only, and policy/default-selection decisions outside this exact caller path are still open |
...
- Productizing targeted invocation beyond the current REPL path still means deciding whether named `backend_id` selection becomes mandatory whenever multiple eligible members exist outside this exact explicit caller path, or whether a default-selection layer is introduced elsewhere.

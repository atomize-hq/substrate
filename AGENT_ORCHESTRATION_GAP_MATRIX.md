...
| Session handles | `Partial` | The public/operator-facing contract is frozen to exact `orchestration_session_id`; the runtime still extracts and persists surfaced UAA session ids (`internal.uaa_session_id`) and parent sessions still track `active_session_handle_id` as retained internal/compatibility state | The remaining governance work is to retire or rename the retained legacy/internal handle vocabulary without widening the public selector contract beyond exact `orchestration_session_id` |
...
| Read-side and strict control-surface hardening | `Partial` | `substrate agent status` now merges live runtime state with trace fallback, preserves durable `parked_resumable` / `awaiting_attention` host sessions, and warns rather than dropping several torn-state cases; strict surfaces such as `toolbox status` / `toolbox env` remain intentionally fail-closed. Forward design should keep those read surfaces aligned with obligation-derived attention truth rather than treating the inbox projection itself as canonical. | The remaining hardening work is to keep the operator-facing contract explicit: participant-less trace fallback stays warning-bearing and coarse, readable degraded `agent status` stays separate from strict fail-closed control/doctor surfaces, and `orchestration_session_id` remains the only forward public session handle while `session_handle_id` / `active_session_handle_id` stay compatibility/storage-only if retained |
...
- Keep `orchestration_session_id` as the only forward public handle and treat `session_handle_id` / `active_session_handle_id` as compatibility/storage-only if they remain.
- Do not widen this packet into `--current`, fuzzy lookup, repair/reap/invalidate UX, or other broader selector ergonomics.

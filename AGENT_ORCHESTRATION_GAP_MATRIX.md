...
| Toolbox surface | `Partial` | Config, `toolbox status`, `toolbox env`, live-session endpoint derivation, and world-binding projection are present | The internal MCP server plus its orchestrator-only auth/session/trace contract remain unimplemented. Mutating toolbox tools are out of scope for v1 and require a separate ADR rather than silently widening this surface |
| Toolbox role in orchestration | `Constrained by design` | ADR-0045 keeps toolbox as a read-only internal control-plane surface in [ADR-0045](/Users/spensermcconnell/__Active_Code/atomize-hq/substrate/docs/adr/draft/ADR-0045-orchestration-toolbox-internal-mcp-identity-trace-contract.md:19) | Nothing here should become a second execution plane or a member-launch path |
| Toolbox near-term implementation scope | `Still open` | The visible toolbox surface is intentionally read-side only today (`status|env` plus derived projection), and current architecture keeps toolbox out of the execution/control plane | The repo still needs an explicit near-term decision on whether to invest in the internal MCP server, orchestrator-only access rules, fail-closed auth/session gating, transport contract, and trace-joinable toolbox calls soon, or to keep toolbox intentionally introspection-only while dispatch/router/caller work lands first |
...
- The near-term decision is whether the internal MCP server plus its orchestrator-only auth/session/trace contract moves onto the v1 path soon, or remains deferred so it does not become a second execution plane or distract from orchestration/router work.
- Mutating toolbox tools are not part of the v1 near-term question; they stay out of scope until a separate ADR and rollout plan explicitly widens the surface.

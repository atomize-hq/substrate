...
| Explicit backend/member targeting | `Partially landed, REPL-first` | Effective inventory entries derive canonical `backend_id` values, targeted REPL follow-up turns now route by exact `backend_id`, host follow-up targeting fails closed unless it names the active orchestrator backend, Linux shell reuse is keyed by exact `backend_id` rather than `agent_id`, distinct retained members such as `cli:codex` and `cli:claude_code` can coexist in one orchestration session / world generation, and duplicate retained members for the same `backend_id` still fail closed | Remaining work is broader productization: targeted invocation is still REPL-only, and policy/default-selection decisions outside this exact caller path are still open |

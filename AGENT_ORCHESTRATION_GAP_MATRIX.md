...
- Substrate-local host/world transport:
  ...
  - `crates/transport-api-core`
  - `crates/transport-api-client`
  - Purpose: Substrate host components talking to `world-service`
...
- Refer to `crates/transport-api-types`, `crates/transport-api-core`, and `crates/transport-api-client` as the Substrate-local host/world transport layer or world-service transport layer.
...
- Design-alignment note:
  ...
  - but the forward design stack has now pivoted to [DESIGN-durable-orchestration-obligation-ledger.md](./llm-last-mile/DESIGN-durable-orchestration-obligation-ledger.md), where the canonical durable artifact is one session-local obligation ledger and inbox/review plus auto-attach are projections over that same record,
  - so this matrix should distinguish between current landed runtime shape and preferred forward durable-object direction when both matter.

- This is no longer modeling-only.
  - `crates/shell` now depends on `unified-agent-api` (`agent_api`) and the REPL boots a shell-owned orchestrator runtime in `crates/shell/src/repl/async_repl.rs`.
  ...
  - Host orchestration sessions now persist explicit durable posture and attachment truth. In the currently landed runtime that still surfaces as `posture`, `attached_participant_id`, `pending_inbox_count`, and per-session durable `inbox/` artifacts under `sessions/<orchestration_session_id>/inbox/<item_id>.json`.
  - That currently landed inbox state is intentionally narrow: persistence, posture normalization into `awaiting_attention`, internal ack/dismiss support, and dev-support/test ingress exist, but no public inbox command surface or automatic resume workflow is shipped.
  - The preferred forward design direction is now one canonical session-local obligation ledger, with review/inbox and attach-processing handled as projections rather than sibling durable ledgers.
...
- The main remaining gaps are:
  ...
  - the shipped public control plane is intentionally narrow: root `start` stays host-rooted even when `--scope world` selects the Linux-first world-backed path, existing-session prompt-taking accepts only exact `orchestration_session_id` plus exact `backend_id`, there is still no default-agent routing, and there is still no standalone member-root public world-root start,
...
## Caller-Surface Contract Freeze Ledger

Slice `55` is now landed and validated, so the v1 caller-surface decision ledger below is the frozen repo-truth source for later docs and regression-floor work.

| Contract area | Allowed in v1 | Not allowed in v1 | Deferred follow-on |
|---|---|---|---|
| Default backend vs default-agent routing | config/runtime default backend resolution remains real for runtime, policy, inventory, and adapter realization | implicit default-agent routing from plain REPL input, `substrate -c`, `--command`, piped stdin, missing `--backend`, or recency/latest heuristics | any future default-agent UX |
| Prompt-taking surfaces | exact REPL `::<backend_id> <prompt>` plus explicit `substrate agent start|turn`; `reattach` stays lifecycle recovery, not prompt-taking | ambient shell aliases or fuzzy “target anything from anywhere” semantics | broader non-REPL targeting beyond the current namespace |
| Shell-wrap surfaces | `substrate -c`, `--command`, and piped stdin remain shell-wrap-only | reinterpreting shell execution surfaces as agent-prompt entrypoints | any future one-shot explicit agent caller introduced on purpose |
| Public follow-up selectors | exact `--session <orchestration_session_id> --backend <backend_id>` | `participant_id`, `session_handle_id`, `active_session_handle_id`, `internal.uaa_session_id`, fuzzy recency selectors, or hidden backend inference | richer write-side discovery such as bounded `--current`-style affordances |
| World-start wording | host-rooted world-backed `start` exists under a host orchestration session | implying standalone member-root public start/continuity already ships; treating `public world-root start` as current truth | direct member-root public lifecycle |
| Broader ergonomics | keep the narrow explicit caller surfaces fail-closed | hidden routing convenience or broader member-selector semantics | later caller-surface ergonomics/productization work |
...
| Non-interactive agent invocation surface | `Landed narrowly` | `substrate agent start --backend <backend_id> (--prompt <text> | --prompt-file <path> | --prompt-file -) [--json]` and `substrate agent turn --session <orchestration_session_id> --backend <backend_id> ...` now provide a real public prompt-taking caller surface with helper-owned streaming, completion-time `session_posture`, explicit fail-closed exact selector coverage, Linux world-member follow-up proof through typed `/v1/member_turn/stream`, an explicit non-regression guard that keeps `substrate -c` / `--command` in shell-wrap mode, and host-rooted world-backed `start --scope world` rather than standalone member-root lifecycle | Remaining gaps are broader caller-surface breadth and parity only: there is still no default-agent routing, no standalone member-root public world-root start, and Windows/WSL world-sensitive follow-up still fails closed |
...
| Live agent session registry | `Landed` | `AgentRuntimeStateStore` persists authoritative parent sessions and participant records under `~/.substrate/run/agent-hub/sessions/<orchestration_session_id>/session.json` plus per-session `participants/`, `leases/`, and the currently landed `inbox/` artifacts; host sessions now persist explicit posture/attachment truth instead of inferring validity from attached-control diagnostics alone. Forward design direction: move deferred-work authority to the session-local obligation ledger and treat inbox/read state as a projection. | Only a future product-surface rename to something like `agent-sessions` remains undecided; there is no current runtime gap here |
| Agent session control plane | `Landed for narrow public control, Linux-source with supported macOS/Lima world-sensitive parity` | Internal runtime paths now support attached-control `start`, public `reattach` / `fork`, prompt-taking `start` / `turn`, authoritative persistence, clean host detach parking into `parked_resumable`, attention-needed escalation into `awaiting_attention`, exact-session `reattach` recovery back to durable attached truth, `stop`/cancel as the canonical closeout path for attached and parked durable sessions, replacement-member creation across world-generation rollover, the hidden owner-helper launch seam, the same private stop-owner plane for helper-owned and REPL-owned sessions, explicit post-`Accepted` terminal envelopes, and host-rooted world-backed `start --scope world` without promoting world members to public root-session status. In the currently landed runtime, that attention-needed path is still surfaced through pending-inbox state; forward design should derive it from unresolved obligations. | Remaining gaps are broader surface breadth and parity: no standalone member-root public world-root start, no member-level public selectors beyond exact session/backend targeting, and Windows/WSL world-sensitive control posture still fails closed |
| User-turn submission into active UAA sessions | `Landed for REPL and narrow public caller surfaces on Linux plus supported macOS/Lima` | The REPL can now submit arbitrary targeted follow-up turns into retained active sessions, and the public `substrate agent start|turn` surface can now submit exact prompts with helper-owned streaming: parked host follow-up turns resume the exact authoritative orchestrator session, supported world follow-up turns go through the typed `/v1/member_turn/stream` route using the persisted retained-member tuple (`participant_id`, `orchestrator_participant_id`, `backend_id`, `world_id`, `world_generation`), reusing the exact retained backend slot when current and relaunching that exact slot when stale, and detached-world follow-up remains fail-closed with `reattach` guidance | Remaining work is broader operator-surface breadth and platform parity. Public root `start` remains host-rooted even when world-backed, default-agent routing still does not exist, and Windows/WSL world follow-up still fails closed |
| Session selector and remediation ergonomics | `Partial` | The public selector is already frozen to exact `orchestration_session_id`, noncanonical aliases such as `active_session_handle_id`, `participant_id` / `session_handle_id`, and `internal.uaa_session_id` are rejected, `toolbox status` can surface `active_orchestration_session_id` when exactly one live host-scoped orchestrator session resolves, and bounded internal timeout-reconciliation / detached-stop / stale-world-invalidation flows already exist | The broader operator UX is still missing: there is no public `--current` selector, no public repair/reap/invalidate surface, current-session discovery remains read-side-only and fail-closed on ambiguity or torn roots, and the `owner_unreachable` family still fronts several no-owner/no-continuity/transport-timeout cases rather than richer operator-facing actions |
| Session handles | `Landed with bounded compatibility posture` | The public/operator-facing contract is frozen to exact `orchestration_session_id`; the runtime still extracts and persists surfaced UAA session ids (`internal.uaa_session_id`) and parent sessions still track `active_session_handle_id` as retained internal/compatibility state, but those legacy/internal names are explicitly noncanonical compatibility/storage-only artifacts rather than public selectors | The remaining work is governance cleanup only: retire or rename the retained legacy/internal handle vocabulary without widening the public selector contract beyond exact `orchestration_session_id` |
...
| Explicit shared-world ownership | `Landed on Linux and supported macOS/Lima; Linux remains source-of-truth` | Persistent world startup now sends `SharedWorldOwnerSpec`, world-service echoes `SharedWorldBindingSnapshot`, session metadata persists `orchestration_session_id` / `world_generation`, shared-world replacement rolls back correctly on failure, and the shell invalidates stale world members after rollover on Linux plus the supported macOS/Lima forwarded path | Remaining work is broader platform scope and operator maturity, not missing shared-owner proof on the supported macOS/Lima path. Windows/WSL still does not implement this contract |
...
| Broader caller-surface contract | `Landed as a bounded contract freeze` | The repo now has two narrow, explicit caller surfaces: REPL `::<backend_id> <prompt>` and public `substrate agent start|turn|reattach|fork|stop` with exact selectors and host-rooted root `start`, and the Slice `55` ledger above now freezes default backend vs default-agent routing, shell-wrap-only boundaries, exact selectors, and host-rooted world-backed start wording | Remaining work is no longer caller-surface contract invention. Deferred follow-ons remain default-agent UX, broader non-REPL targeting, public member-level selectors, standalone member-root public lifecycle, richer write-side discovery ergonomics, and the separate read-side/control-hardening and platform-parity tracks |
| Read-side and strict control-surface hardening | `Landed as a bounded hardening slice` | `substrate agent status` now merges live runtime state with trace fallback, preserves durable `parked_resumable` / `awaiting_attention` host sessions, warns rather than dropping torn-state cases, keeps participant-aware fallback sibling-distinct when evidence exists, and leaves strict surfaces such as `toolbox status` / `toolbox env` intentionally fail-closed. Forward design should keep those read surfaces aligned with obligation-derived attention truth rather than treating the inbox projection itself as canonical. | No immediate runtime gap remains inside this slice. Later follow-on is broader selector ergonomics, producer-side trace evidence widening, or governance cleanup only if the product wants more than the current bounded contract |
...
| Status ambiguity handling | `Landed for the current operator contract` | `substrate agent status` keeps valid rows visible while surfacing degraded warnings for torn parent/session state such as missing `active_session_handle_id`, missing parent-session metadata, incomplete selected participants, and coarse trace-only fallback; durable parked and attention-needed host sessions remain visible on the read surface instead of collapsing into terminal absence | Strict selectors still fail closed by design on the control-plane surfaces (`toolbox status`, `toolbox env`, doctor/member selection). Any later work here is broader operator UX, not a missing correctness slice |
| Trace-only participant-aware fallback | `Landed with explicit coarse-fallback boundary` | Trace rows that carry `participant_id` stay sibling-distinct, sibling-specific suppression is keyed by `participant_id`, nested correlation honors `parent_participant_id` when same-agent siblings coexist in one `orchestration_session_id`, and participant-less rows emit explicit warnings while falling back only to coarse matching | The remaining follow-on is optional producer-side evidence widening if the product later wants fewer coarse trace-only rows; the current operator contract is now explicit and verified |
...
| Toolbox surface | `Landed for the Codex-backed floor plus selected-host claude_code parity` | Config, `toolbox status`, `toolbox env`, live-session endpoint derivation, world-binding projection, the session-scoped internal toolbox transport, the Slice `52` frozen seven-tool adapter contract, the Slice `53` first live Codex-backed host-tool landing, and the Slice `54` selected-host `claude_code` parity path are now present | The remaining gap is no longer runtime-family parity itself. The open follow-on is broader smoke/docs/operator-truth work only when later live-surface widening changes repo truth again. Public toolbox verbs, mutating human CLI use, and internal MCP landing remain deferred |
...
| Toolbox near-term implementation scope | `Specified, multi-family selected-host floor landed` | The visible toolbox surface remains intentionally read-side for operators (`status|env` plus derived projection), and the runtime path is now explicit and landed through the selected-host multi-family floor: Slice `52` froze the shared seven-tool contract, Slice `53` validated Codex-backed disclosure/env injection, and Slice `54` validated selected-host `claude_code` parity on that same seam | The remaining open scope is now bounded follow-through rather than parity itself: broader runtime-family smoke coverage, later docs/operator-truth alignment when some future live-surface widening changes repo truth, and the still-deferred internal MCP/auth/audit and public toolbox-verb work |
...
1. Keep the landed read-side and strict control-surface contract explicit.
- Slice `56` is now checkpoint-green: readable degraded `agent status` output remains separate from fail-closed `toolbox` / doctor / control-plane selectors, participant-less trace fallback stays warning-bearing and coarse, and `orchestration_session_id` remains the only forward public handle while `session_handle_id` / `active_session_handle_id` remain compatibility/storage-only if retained.
- Do not widen this into `--current`, fuzzy lookup, repair/reap/invalidate UX, or other broader selector ergonomics unless a later product slice chooses to.
...
3. Whether `substrate.agent.session` should remain the Substrate-local normalized protocol-family label, or be renamed now that real UAA runtime integration is live.
...
- This is closely related to the `substrate.agent.session` decision above, but it is a crate/module/package naming question rather than a protocol/identity-label question.
...
5. Whether to stop at the newly landed hidden shell↔world-service helper over the existing execute/stream transport, or continue on to a broader reusable world-owned UAA service surface.
...
9. How far to push later host-tool follow-through now that second runtime-family parity is landed.
- The current architecture is no longer deciding whether selected-host multi-family parity should exist at all; the Slice `52`/`53`/`54` path already landed the shared contract, the first Codex-backed validated floor, and selected-host `claude_code` parity above the existing transport.
- The real remaining decisions are follow-through breadth and later widening: how much of the already-landed adapter stays runtime-generic in practice, when broader runtime validation/docs/reporting should expand again, and whether any later live-surface widening justifies new host-tool work beyond the now-landed parity baseline.
...
7. Keep `substrate agent status` on the current split contract: surface durable `parked_resumable` and `awaiting_attention` sessions on the read surface, degrade torn parent/session linkage into warnings, keep control-plane selectors fail-closed, and keep participant-less trace fallback explicitly warning-bearing and coarse by design.
...
At this point, the missing work is no longer basic UAA adoption, REPL targeted-turn grammar, REPL user-turn dispatch, session persistence, event modeling, gateway secret-carrier honesty, Linux member-runtime placement, or Slice `56` status/control hardening. The live remaining work is broader caller-surface productization, later runtime-family smoke plus reporting/docs follow-through, explicit platform-parity decisions, Family-2 host-global ingress/federation follow-ons, and governance cleanup.

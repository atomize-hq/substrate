...
fn validate_exact_launchctl_not_found_tuple_v2(
    label: &str,
    exit_status: i32,
    stdout: &[u8],
    stderr: &[u8],
    ...
    {
        bail!("cleanup receipt launchctl print tuple is not the exact service-not-found result")
    }
    Ok(())
}
...
impl NativeEvidenceCleanupReceiptV2 {
    pub fn validate(
        ...
        let latch_paths = scopes
            ...
            .collect::<Vec<_>>();
        let launchctl_bootout_stdout = self.launchctl_bootout_stdout.validate()?;
        let launchctl_bootout_stderr = self.launchctl_bootout_stderr.validate()?;
        let launchctl_print_stdout = self.launchctl_print_stdout.validate()?;
        let launchctl_print_stderr = self.launchctl_print_stderr.validate()?;
        validate_exact_launchctl_not_found_tuple_v2(
            &self.launchd_label,
            self.launchctl_print_not_found_exit_status,
        ...
        if self.schema_owner != NATIVE_EVIDENCE_EXPORT_OWNER_V2
            ...
            || !launchctl_bootout_stdout.is_empty()
            || !launchctl_bootout_stderr.is_empty()
...
mod tests {
    ...
    #[test]
    fn cleanup_launchd_tuple_rejects_raw_prefix_and_suffix() {
        let label = MAC_R3_FINALIZER_LAUNCHD_LABEL_V2;
        let exact =
            format!("Bad request.\nCould not find service \"{label}\" in domain for system\n");
        assert!(
            validate_exact_launchctl_not_found_tuple_v2(label, 113, b"", exact.as_bytes()).is_ok()
        );
        assert!(validate_exact_launchctl_not_found_tuple_v2(
            label,
            113,
            b"",
            format!("prefix{exact}").as_bytes(),
        )
        .is_err());
        assert!(validate_exact_launchctl_not_found_tuple_v2(
            label,

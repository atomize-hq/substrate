...
impl SupplementaryGroupAttestationV2 {
    ...
    pub fn inherited_from_unprivileged_parent() -> Self {
        Self {
            groups: Vec::new(),
            evidence: SupplementaryGroupEvidenceV2::UnprivilegedParentInheritance,
        }
    }
...
pub fn attest_fixed_peer_process_v2(
    ...
    let supplementary_groups = if pid == self_pid {
        SupplementaryGroupAttestationV2::current_process()?
    } else {
        let parent_groups = SupplementaryGroupAttestationV2::current_process()?;
        if expected_uid == 0 || unsafe { libc::geteuid() } != expected_uid {
            bail!("non-self peer attestation lacks its unprivileged empty-group parent join")
        }
        parent_groups.validate_empty()?;

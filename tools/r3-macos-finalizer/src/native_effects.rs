...
const LAUNCHCTL_RAW_STREAM_MAX_BYTES: usize = 256 * 1024;
...
pub fn classify_launchctl_print(
    ...
    if code == Some(0) {
        ...
        && stdout.is_empty()
        && stderr == exact_launchctl_service_not_found_stderr(label).as_bytes()
    {
...
fn exact_launchctl_service_not_found_stderr(label: &str) -> String {
    let service = label.strip_prefix("system/").unwrap_or(label);
    format!("Bad request.\nCould not find service \"{service}\" in domain for system\n")
}

fn classify_launchd_absence_recovery(
    phase: EffectObservationPhase,
    cached_exact_bootout_witness: bool,
) -> ObservationClass {
    if phase == EffectObservationPhase::Prepared || cached_exact_bootout_witness {
        ObservationClass::ExactFinal
    } else {
        // Absence after an interrupted bootout cannot reconstruct the exact native return or its
        // raw streams. Preserve rather than silently upgrading exit-status-only absence to proof.
...
#[derive(Debug, Serialize)]
#[serde(deny_unknown_fields)]
struct BoundedLaunchctlRawStreamV2 {
...
#[derive(Debug, Serialize)]
#[serde(deny_unknown_fields)]
struct ExactLaunchdBootoutEvidenceV2 {
    schema_owner: String,
    schema_version: u32,
    scope_id: String,
    target_set_kind: TargetSetKindV2,
    ...
    label: String,
    expected_identity_sha256: String,
    bootout_exit_status: i32,
    bootout_stdout: BoundedLaunchctlRawStreamV2,
    bootout_stderr: BoundedLaunchctlRawStreamV2,
    print_exit_status: i32,
    print_stdout: BoundedLaunchctlRawStreamV2,
    print_stderr: BoundedLaunchctlRawStreamV2,
    classification: String,
    exact_absence_observed: bool,
}

#[derive(Debug)]
struct LaunchctlPrintObservation {
    class: LaunchctlPrintClass,
    ...
    fn observe(
        ...
        match &target.target {
            ...
            FixedTarget::LaunchdService { label } => {
                ...
                let (class, observation_sha256) = match state.class {
                    LaunchctlPrintClass::NotFound => (
                        classify_launchd_absence_recovery(
                            phase,
                            self.invocation_evidence.contains_key(&target.ordinal),
                        ),
                    ...
                    LaunchctlPrintClass::Ambiguous(code) => (
                        ObservationClass::Ambiguous,
                        service_ambiguous_observation(label, code, &state.stdout, &state.stderr),
                    ),
                };
                Ok(EffectObservation {
                    class,
    ...
    fn invoke(&mut self, target: &DerivedTarget, expected_before_sha256: &str) -> Result<()> {
        match &target.target {
            ...
            FixedTarget::LaunchdService { label } => {
                ...
                let bootout_exit_status = output
                    .status
                    .code()
                    .context("fixed publisher launchd bootout was signaled")?;
                let bootout_stdout = bounded_launchctl_raw_stream(&output.stdout)?;
                let bootout_stderr = bounded_launchctl_raw_stream(&output.stderr)?;
                let state = launchd_service_state(label)?;
                match state.class {
                    LaunchctlPrintClass::NotFound
                        if bootout_exit_status == 0
                            && output.stdout.is_empty()
                            && output.stderr.is_empty() =>
                    {
                        let print_exit_status = state.exit_status.context(
                            "fixed publisher launchd absence observation was signaled",
                        )?;
                        let evidence = canonical_bytes_v2(&ExactLaunchdBootoutEvidenceV2 {
                            schema_owner: NATIVE_EFFECT_EVIDENCE_OWNER.to_owned(),
                            schema_version: NATIVE_EFFECT_EVIDENCE_VERSION,
                            scope_id: self.receipt.scope_id.clone(),
                            target_set_kind: self.receipt.target_set_kind,
                            ...
                            bootout_stderr,
                            print_exit_status,
                            print_stdout: bounded_launchctl_raw_stream(&state.stdout)?,
                            print_stderr: bounded_launchctl_raw_stream(&state.stderr)?,
                            classification:
                                "bootout_success_and_exact_service_not_found_observed".to_owned(),
                            exact_absence_observed: true,
                        })?;
                        self.record_invocation_evidence(target.ordinal, evidence)
                    }
                    LaunchctlPrintClass::NotFound => bail!(
                        "fixed publisher launchd bootout returned a noncanonical success tuple: status={bootout_exit_status} stdout_bytes={} stderr_bytes={}",
                        output.stdout.len(),
                        output.stderr.len()
                    ),
                    LaunchctlPrintClass::Present => {
                    ...
                    LaunchctlPrintClass::Ambiguous(code) => bail!(
                        "fixed publisher launchd post-bootout state is ambiguous ({code:?}); bootout_status={bootout_exit_status}"
                    ),
                }
            }
...
fn launchd_service_state(label: &str) -> Result<LaunchctlPrintObservation> {
    ...
    bounded_launchctl_raw_stream(&output.stdout)?;
    bounded_launchctl_raw_stream(&output.stderr)?;
    let exit_status = output.status.code();
    Ok(LaunchctlPrintObservation {
        class: classify_launchctl_print(label, exit_status, &output.stdout, &output.stderr),
        exit_status,
...
fn bounded_launchctl_raw_stream(bytes: &[u8]) -> Result<BoundedLaunchctlRawStreamV2> {
    if bytes.len() > LAUNCHCTL_RAW_STREAM_MAX_BYTES {
        bail!("launchctl raw stream exceeds the closed evidence bound")
    }
    Ok(BoundedLaunchctlRawStreamV2 {
        raw_base64url: URL_SAFE_NO_PAD.encode(bytes),
        raw_sha256: sha256_hex_v2(bytes),
        raw_byte_length: u64::try_from(bytes.len()).context("launchctl raw stream exceeds u64")?,
    })
}
...
fn service_ambiguous_observation(
    label: &str,
    code: Option<i32>,
    stdout: &[u8],
    stderr: &[u8],
    ...
    joined_observation(
        ...
        format!(
            "{label}\0{code:?}\0{}\0{}",
            sha256_hex_v2(stdout),
            sha256_hex_v2(stderr)
        )
        .as_bytes(),
    )
}
...
mod tests {
    ...
    fn launchctl_only_exact_not_found_is_absence() {
        let label = PRODUCT_LAUNCHD_SERVICE;
        let exact_stderr = exact_launchctl_service_not_found_stderr(label);
        assert_eq!(
            classify_launchctl_print(label, Some(0), b"arbitrary present output", b""),
            LaunchctlPrintClass::Present
        );
        assert_eq!(
            classify_launchctl_print(label, Some(113), b"", exact_stderr.as_bytes()),
            LaunchctlPrintClass::NotFound
        );
        for code in [None, Some(1), Some(3), Some(5), Some(125)] {
            assert_eq!(
                classify_launchctl_print(label, code, b"", exact_stderr.as_bytes()),
                LaunchctlPrintClass::Ambiguous(code)
            );
        }
        for (stdout, stderr) in [
            (b"unexpected stdout".as_slice(), exact_stderr.as_bytes()),
            (b"".as_slice(), b"arbitrary status-113 output".as_slice()),
            (
                b"".as_slice(),
                b"Bad request.\nCould not find service \"another\" in domain for system\n"
                    .as_slice(),
            ),
        ] {
            assert_eq!(
                classify_launchctl_print(label, Some(113), stdout, stderr),
                LaunchctlPrintClass::Ambiguous(Some(113))
            );
        }
    }

    #[test]
    fn invoked_launchd_absence_requires_same_process_exact_bootout_witness() {
        assert_eq!(
            classify_launchd_absence_recovery(EffectObservationPhase::Prepared, false),
            ObservationClass::ExactFinal
        );
        assert_eq!(
            classify_launchd_absence_recovery(EffectObservationPhase::Invoked, false),
            ObservationClass::Ambiguous
        );
        assert_eq!(
            classify_launchd_absence_recovery(EffectObservationPhase::Invoked, true),
            ObservationClass::ExactFinal
        );
    }

    #[test]
    fn launchctl_raw_evidence_is_exact_and_bounded() {
        let bytes = b"Could not find service";
        let evidence = bounded_launchctl_raw_stream(bytes).unwrap();
        assert_eq!(evidence.raw_byte_length, bytes.len() as u64);
        assert_eq!(evidence.raw_sha256, sha256_hex_v2(bytes));
        assert_eq!(
            URL_SAFE_NO_PAD.decode(evidence.raw_base64url).unwrap(),
            bytes
        );
        assert!(
            bounded_launchctl_raw_stream(&vec![0; LAUNCHCTL_RAW_STREAM_MAX_BYTES + 1]).is_err()
        );
    }

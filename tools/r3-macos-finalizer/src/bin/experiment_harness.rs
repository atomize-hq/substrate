...
mod macos {
    ...
    use substrate_r3_macos_finalizer::experiment::process::{
        attest_fixed_peer_process_v2, CoordinatorProcessAttestationV2,
    };
    ...
    fn spawn_and_attest_coordinator(
        ...
        let attestation =
            match read_coordinator_startup_attestation(&mut stdout).and_then(|attestation| {
                attestation.validate(identity)?;
                if attestation.pid != pid {
                    bail!("coordinator self-attestation PID differs from the spawned child")
                }
                Ok(attestation)
            }) {
                ...
                Err(error) => {
                    let _ = child.kill();
                    let _ = child.wait();
                    return Err(error).context("read exact coordinator self-attestation");
                }
            };
    ...
    fn read_coordinator_startup_attestation(
        stdout: &mut ChildStdout,
    ) -> Result<CoordinatorProcessAttestationV2> {
        let mut prefix = [0_u8; 8];
        stdout
            .read_exact(&mut prefix)
            .context("read coordinator self-attestation frame prefix")?;
        let length = usize::try_from(u64::from_be_bytes(prefix))
            .context("coordinator self-attestation length exceeds usize")?;
        if length == 0 || length > MAC_R3_FINALIZER_MAX_FRAME_BYTES_V2 {
            bail!("coordinator self-attestation frame is outside the fixed bound")
        }
        let mut bytes = vec![0_u8; length];
        stdout
            .read_exact(&mut bytes)
            .context("read coordinator self-attestation frame body")?;

...
use crate::securityagent::{
    ...
    observe_root_operation_with_raw, observe_sealed_child, observe_sealed_child_with,
    observe_stopped_child_startup, ObservedChildV2,
};
use crate::{
...
pub fn run_fixed_experiment() -> Result<()> {
    ...
    let (security, report_sha256, report_bytes) = observe_root_operation_with_raw(
        NonInteractiveSecurity::establish_first,
        |alert| {
...
fn attest_stopped_creator_process(
    ...
    let attestation = CreatorProcessAttestationV2 {
        ...
        supplementary_groups: SupplementaryGroupAttestationV2::sealed_pre_exec(),
...
fn persist_next_root_operation_securityagent_report(
    ...
    for ordinal in 1_u8..=32 {
        ...
        if read_runner_private_bytes_optional(&name)?.is_none() {
            return persist_root_operation_securityagent_report(
                &name,
                expected_sha256,
                bytes,
            );
...
fn spawn_observed_publisher(inputs: &FrozenRunnerInputs, purpose: &'static str) -> Result<Child> {
    if !matches!(purpose, "normal" | "publisher_ui_rollback" | "general_failure_rollback") {
    ...
    write_runner_receipt(
        &format!(
            "publisher-startup-{purpose}-{startup_ordinal:02}.attestation.v2.json"
        ),
        &canonical_bytes_v2(&attestation)?,
    )?;
...
fn reattest_exact_process_member(
    ...
    let observed = GeneralFailureGroupMemberV2 {
        pid: expected.pid,
        effective_uid: process.uid,
        supplementary_groups: expected.supplementary_groups.clone(),
        process_start_identity_sha256: document_sha256_v2(&ProcessStartJoinV2 {
            pid: expected.pid,
            seconds: process.start_seconds,
            microseconds: process.start_microseconds,
...
fn observe_process_group_members(
    ...
    for pid in process_group_members(process_group_id)? {
        ...
        observations.push(GeneralFailureGroupMemberV2 {
            pid,
            effective_uid: process.uid,
            supplementary_groups: if pid == process_group_id {
                SupplementaryGroupAttestationV2::sealed_pre_exec()
            } else {
                SupplementaryGroupAttestationV2::inherited_from_unprivileged_parent()
            },
            process_start_identity_sha256: document_sha256_v2(&ProcessStartJoinV2 {
                pid,
                seconds: process.start_seconds,
...
fn spawn_publisher_rollback_only(
    ...
    if !matches!(purpose, "publisher_ui_rollback" | "general_failure_rollback") {
...
fn sealed_dynamic_library_command() -> Result<(Command, UnixStream)> {
...
fn sealed_nobody_owner_command(command: &NobodyOwnerProbeCommandV2) -> Result<Command> {
    ...
    let (terminal_cleanup, report_sha256, report_bytes) = observe_root_operation_with_raw(
        || {
            ...
            let loaded = if initial_status == 0 {
                ...
                let stderr = std::str::from_utf8(&initial.stderr)?;
                if !initial.stdout.is_empty()
                    || !stderr.contains("Could not find service")
                    || !stderr.contains(MAC_R3_FINALIZER_LAUNCHD_LABEL_V2)

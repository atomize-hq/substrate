...
pub(crate) const SEALED_GROUP_MEASUREMENT_MAGIC_V2: [u8; 8] = *b"R3GRP2\0\0";
pub(crate) const SEALED_GROUP_MEASUREMENT_FRAME_BYTES_V2: usize = 20;

static SEALED_CHILD_GROUP_MEASUREMENTS_V2: OnceLock<
    Mutex<BTreeMap<u32, SupplementaryGroupAttestationV2>>,
> = OnceLock::new();

pub(crate) struct MeasuredCommandV2 {
    command: Command,
    group_measurement: Option<UnixStream>,
}

impl MeasuredCommandV2 {
    pub(crate) fn new(command: Command, group_measurement: UnixStream) -> Self {
        Self {
            command,
            group_measurement: Some(group_measurement),
        }
    }
    ...
    pub(crate) fn spawn(&mut self) -> Result<Child> {
        let mut child = self
            .command
            .spawn()
            .context("spawn getgroups-measured child")?;
        let measurement = (|| -> Result<()> {
            let mut frame = [0_u8; SEALED_GROUP_MEASUREMENT_FRAME_BYTES_V2];
            self.group_measurement
                .take()
                .context("sealed command group-measurement receiver was already consumed")?
                .read_exact(&mut frame)
                .context("read sealed child post-drop getgroups measurement")?;
            if frame[..8] != SEALED_GROUP_MEASUREMENT_MAGIC_V2 {
                bail!("sealed child supplementary-group measurement magic changed")
            }
            let effective_uid = u32::from_ne_bytes(frame[8..12].try_into().unwrap());
            let effective_gid = u32::from_ne_bytes(frame[12..16].try_into().unwrap());
            let group_count = u32::from_ne_bytes(frame[16..20].try_into().unwrap());
            if effective_uid == u32::MAX {
                bail!("sealed child effective UID measurement is invalid")
            }
            if effective_gid == u32::MAX || group_count != 0 {
                bail!("sealed child retained supplementary groups after credential drop")
            }
            let attestation = SupplementaryGroupAttestationV2::sealed_pre_exec(Vec::new())?;
            SEALED_CHILD_GROUP_MEASUREMENTS_V2
                .get_or_init(|| Mutex::new(BTreeMap::new()))
                .lock()
                .map_err(|_| {
                    anyhow::anyhow!("sealed child group-measurement registry was poisoned")
                })?
                .insert(child.id(), attestation);
            Ok(())
        })();
        if let Err(error) = measurement {
            return Err(terminate_and_reap_measured_child(&mut child, error));
        }
        Ok(child)
    }
}

fn terminate_and_reap_measured_child(child: &mut Child, error: anyhow::Error) -> anyhow::Error {
    let pid = child.id();
    let kill_result = child.kill();
    let wait_result = child.wait();
    anyhow::anyhow!(
        "sealed child {pid} failed post-spawn group attestation and was contained; cause={error:#}; kill={kill_result:?}; wait={wait_result:?}"
    )
}

impl Deref for MeasuredCommandV2 {
    type Target = Command;
...
impl DerefMut for MeasuredCommandV2 {
    ...
    F: FnOnce(u32) -> Result<T> + Send,
{
    ...
    let mut child = command.spawn().context("spawn sealed observed child")?;
...
#[cfg(test)]
mod tests {
    use super::terminate_and_reap_measured_child;
    ...
    #[test]
    fn post_spawn_measurement_failure_terminates_and_reaps_the_exact_child() {
        let mut child = Command::new("/bin/sleep").arg("30").spawn().unwrap();
        let pid = child.id();
        let error = terminate_and_reap_measured_child(
            &mut child,
            anyhow::anyhow!("synthetic sealed-frame failure"),
        );
        assert!(error.to_string().contains(&format!("sealed child {pid}")));

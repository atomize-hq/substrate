//! PTY WebSocket handler for world-service implementing JSON frame protocol
...
use transport_api_types::{PolicySnapshotV3, SharedWorldOwnerSpec, WorldNetworkRoutingV1};
#[cfg(target_os = "linux")]
...
pub async fn handle_ws_pty(
    #[cfg_attr(not(target_os = "linux"), allow(unused_variables))] service: WorldService,
    ws: WebSocket,
) {
...
async fn handle_persistent_session(
    ...
    let _ = send_persistent_ws_message(
        ...
        &PersistentServerMessage::Error {
            code: "internal_error".to_string(),
            message: "Persistent sessions are only supported on Linux world-service".to_string(),
            fatal: true,
            seq: None,
        },
    )
...
async fn handle_legacy_start(
    ...
    let _ = send_persistent_ws_message(
        ...
        &PersistentServerMessage::Error {
            code: "internal_error".to_string(),
            message: "Legacy PTY streaming is only supported on Linux world-service".to_string(),
            fatal: true,
            seq: None,
        },
    )

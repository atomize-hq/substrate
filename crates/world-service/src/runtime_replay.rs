...
impl RuntimeReplayRegistry {
    ...
    ) -> Result<UnboundedReceiver<ExecuteStreamFrame>> {
...
mod tests {
    ...
    #[test]
    fn replay_subscriber_receives_exact_backlog_and_live_terminal() {
    ...
    #[test]
    fn replay_rejects_absent_mismatched_ahead_and_post_close_state() {
        let registry = RuntimeReplayRegistry::default();
    ...
    #[test]
    fn closed_streams_are_evicted_before_registry_capacity_rejects_new_work() {
        let registry = RuntimeReplayRegistry::default();
        for index in 0..MAX_REPLAY_STREAMS {
            let acceptance_record_id = format!("wwa_closed_{index:04}");
            let stream_id = format!("rts_closed_{index:04}");
            let start = ExecuteStreamFrame::Start {
                ...
                span_id: format!("spn_closed_{index:04}"),
            };
            let publisher = registry
                .begin(&acceptance_record_id, &start)
                .expect("fill bounded registry with closed streams");
            publisher
                .publish(&ExecuteStreamFrame::Error {
                    frame_identity: RuntimeFrameIdentityV1 {
                        schema_version: RUNTIME_FRAME_IDENTITY_SCHEMA_VERSION_V1,
                        stream_id,
                        frame_sequence: 2,
                ...
                .expect("close bounded replay stream");
        }
        assert!(registry
            .begin(
                "wwa_after_eviction",
                &ExecuteStreamFrame::Start {
                    frame_identity: RuntimeFrameIdentityV1 {
                        schema_version: RUNTIME_FRAME_IDENTITY_SCHEMA_VERSION_V1,
                        stream_id: "rts_after_eviction".to_string(),
                        frame_sequence: 1,
                    },
                    span_id: "spn_after_eviction".to_string(),
                },
            )
            .is_ok());
    }

...
async fn post_execute_json(service: WorldService, payload: Value) -> (StatusCode, Value) {
    let app = Router::new()
        .route("/v1/execute", post(world_service::handlers::execute))
        .with_state(service);
...
async fn ws_start_session_rejects_policy_snapshot_schema_version_1_and_2_and_closes() {
    ...
    let (addr, server) = spawn_world_service_ws(service).await;

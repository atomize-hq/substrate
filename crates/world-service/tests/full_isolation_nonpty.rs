...
fn landlock_exec_subprocess_entry() {
    ...
    match world_service::internal_exec::run_landlock_exec() {
        Ok(()) => {
...
fn make_service() -> Option<WorldService> {
    let _ = set_global_broker(BrokerHandle::new());
    match WorldService::new() {
        Ok(svc) => Some(svc),
        Err(err) => {
...
) -> Option<transport_api_types::ExecuteResponse> {
...
fn landlock_exec_fails_closed_with_actionable_error_when_overlay_backing_dirs_missing() {
    ...
    let output = Command::new(exe)
        ...
        .output()
        .expect("run world-service landlock exec wrapper");

...
async fn spawn_world_service_ws(
    ...
    let router = Router::new()
        .route("/v1/stream", get(world_service::handlers::stream))
        .with_state(service);
...
async fn spawn_world_service_ws_with_fionread_blocked() -> (
    SocketAddr,
    tokio::sync::oneshot::Sender<()>,
    ...
    let handle = std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            let service = WorldService::new().expect("WorldService::new");
            let router = Router::new()
                .route("/v1/stream", get(world_service::handlers::stream))
                .with_state(service);
...
async fn legacy_one_shot_start_remains_accepted() {
    ...
    let (addr, shutdown, server) = spawn_world_service_ws(service).await;
...
async fn session_nonce_is_unique_per_session_restart() {
    ...
    let (addr1, shutdown1, server1) = spawn_world_service_ws(service.clone()).await;
    ...
    let (addr2, shutdown2, server2) = spawn_world_service_ws(service).await;
...
async fn start_session_fails_closed_when_fionread_watermark_is_unavailable() {
    ...
    // protocol v1 (Linux ioctl(FIONREAD)), world-service MUST NOT emit `ready`.
    let (addr, shutdown, server_thread) = spawn_world_service_ws_with_fionread_blocked().await;
...
async fn start_session_is_robust_to_inheritable_non_stdio_fds() {
    ...
    // This test opens a non-CLOEXEC fd in the world-service process and requires that the

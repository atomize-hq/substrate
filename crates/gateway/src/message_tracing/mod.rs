...
use crate::structured_events::NormalizedEvent;
...
pub struct MessageTracer {
    ...
    seq_by_trace: DashMap<String, u64>,
}
...
/// A trace entry for a structured event
#[derive(Serialize)]
struct EventTrace {
    ts: DateTime<Utc>,
    dir: &'static str,
...
impl MessageTracer {
    ...
    /// Trace a response
    pub fn trace_response(&self, id: &str, response: &GatewayResponse, latency_ms: u64) {
        let Some(ref file_mutex) = self.file else {
            return;
        };
    ...
    /// Trace a normalized structured event (C-06)
    pub fn trace_event(&self, id: &str, event: &NormalizedEvent) {
        let Some(ref file_mutex) = self.file else {
            return;
        };
        if id.is_empty() {
        ...
        let mut entry = self.seq_by_trace.entry(id.to_string()).or_insert(0);
        let seq = *entry;
        *entry += 1;
        drop(entry);

...
use crate::models::{MessageContent, RouteDecision, RouteType, SystemPrompt};
use anyhow::Result;
...
impl Router {
    ...
    /// 5. Tool-result continuation handoff - execution/default when a plan-mode turn resumes after tools
    /// 6. Think - Plan Mode / reasoning enabled
    /// 7. Default - auto-mapped or original model name
    pub fn route(&self, request: &mut GatewayRequest) -> Result<RouteDecision> {
        ...
        // 5. Tool-result continuation handoff for plan-mode turns
        if self.is_plan_mode(request) && self.is_tool_result_continuation(request) {
            debug!("↪️ Routing to execution/default after plan-mode tool-result continuation");
            return Ok(RouteDecision {
    ...
    fn has_web_search_tool(&self, request: &GatewayRequest) -> bool {
    ...
    fn is_plan_mode(&self, request: &GatewayRequest) -> bool {
        request
            .reasoning
            .as_ref()
    ...
    /// Check whether the latest user turn is a tool-result-only continuation.
    /// This mirrors the server's continuation injection rule so plan-mode turns
    /// can hand off to execution without depending on provider parsing details.
    fn is_tool_result_continuation(&self, request: &GatewayRequest) -> bool {
        use crate::models::{ContentBlock, MessageContent};

        let last_user = match request.messages.iter().rev().find(|msg| msg.role == "user") {
            Some(msg) => msg,
            None => return false,
        };
        ...
        match &last_user.content {
            ...
            MessageContent::Blocks(blocks) => {
                let has_tool_results = blocks.iter().any(|block| block.is_tool_result());
                let has_text = blocks.iter().any(|block| match block {
                    ContentBlock::Known(crate::models::KnownContentBlock::Text {
                        text, ..
                    }) => !text.trim().is_empty(),
                    _ => false,
                });
    ...
    fn match_prompt_rule(&self, request: &mut GatewayRequest) -> Option<(String, String)> {
    ...
    fn extract_last_user_message(&self, request: &GatewayRequest) -> Option<String> {
    ...
    fn find_turn_start_index(&self, request: &GatewayRequest) -> usize {
        use crate::models::ContentBlock;
    ...
    fn extract_turn_starting_user_message(&self, request: &GatewayRequest) -> Option<String> {
    ...
    fn strip_match_from_turn_starting_message(&self, request: &mut GatewayRequest, regex: &Regex) {
        let turn_start_idx = self.find_turn_start_index(request);
    ...
    fn strip_match_from_last_user_message(&self, request: &mut GatewayRequest, regex: &Regex) {
    ...
    /// Checks for <CCM-SUBAGENT-MODEL>model-name</CCM-SUBAGENT-MODEL> in the second
    /// system prompt text block when system content is block-based.
    ...
    fn extract_subagent_model(&self, request: &mut GatewayRequest) -> Option<String> {
        ...
        if let SystemPrompt::Blocks(blocks) = system {
            ...
            let text = second_block.as_text_mut()?;
...
mod tests {
    ...
    use crate::models::{
        ContentBlock, KnownContentBlock, Message, MessageContent, ToolResultContent,
    };
    ...
    fn create_simple_request(text: &str) -> GatewayRequest {
        GatewayRequest {
    ...
    fn test_plan_mode_detection() {
        ...
        request.reasoning = Some(crate::core::ReasoningConfig {
    ...
    #[test]
    fn test_tool_result_continuation_hands_off_from_think_mode() {
        let config = create_test_config();
        let router = Router::new(config);
        ...
        let mut request = GatewayRequest {
            ...
            messages: vec![
                Message {
                    role: "user".to_string(),
                    content: MessageContent::Text("Plan a filesystem cleanup".to_string()),
                },
                Message {
                    ...
                    content: MessageContent::Blocks(vec![ContentBlock::Known(
                        KnownContentBlock::ToolUse {
                            id: "tool_1".to_string(),
                            name: "Read".to_string(),
                            input: serde_json::json!({"file_path": "/tmp/demo"}),
                        },
                    )]),
                },
                Message {
    ...
    #[test]
    fn test_tool_result_continuation_without_think_mode_stays_default() {
        let config = create_test_config();
        let router = Router::new(config);

...
impl GeminiProvider {
    pub fn new(
        ...
        let base_url = base_url.unwrap_or_else(|| {
            if auth.oauth_provider_id.is_some() {
                // Code Assist API (OAuth)
                "https://cloudcode-pa.googleapis.com/v1internal".to_string()
            } else if auth.project_id.is_some() && auth.location.is_some() {
                // Vertex AI
                format!(
                    "https://{}-aiplatform.googleapis.com/v1",
                    auth.location.as_ref().expect("location checked above")
                )
            } else {
                // Google AI (API Key)
        ...
        Self {
            ...
            oauth_provider_id: auth.oauth_provider_id,
    ...
    fn transform_request(&self, request: &GatewayRequest) -> Result<GeminiRequest, ProviderError> {
...
#[async_trait]
impl super::GatewayProvider for GeminiProvider {

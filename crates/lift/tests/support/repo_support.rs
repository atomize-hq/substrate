...
impl TempDir {
    pub(crate) fn new(prefix: &str) -> Self {
        ...
        .join(format!(
            "substrate-lift-{prefix}-{}-{suffix}-{counter}",
            std::process::id(),
        ));
        // macOS can produce identical wall-clock timestamps across parallel test threads,
        // so include a per-process counter and require the final directory to be new.
        fs::create_dir(&path).expect("temp dir should be creatable");
        Self { path }
    }

...
fn bench_credential_patterns(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        vec!["curl", "-H", "Authorization: Bearer secret123", "-H", "X-API-Key: key456", "https://api.example.com"],
        // Mixed sensitive and normal args
        vec!["myapp", "--config", "/path/to/config", "--token", "secret", "--verbose", "true"],
    ];
    ...
    for (i, case) in test_cases.iter().enumerate() {
        ...
        group.bench_with_input(
            BenchmarkId::new("pattern_matching", i),
            &args,
            |b, args| {
                b.iter(|| {
...
fn bench_memory_patterns(c: &mut Criterion) {
    ...
    group.bench_function("argv_vector_creation", |b| {
        ...
        b.iter(|| {
            let os_strings: Vec<OsString> = args
                .iter()

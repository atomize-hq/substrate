...
#[cfg(target_os = "linux")]
struct PacketThreeObligationCase {
    event_class: &'static str,
    obligation_kind: &'static str,
    attach_state: &'static str,
...
#[cfg(target_os = "linux")]
fn enable_packet_three_worker_event_policy(home_substrate: &Path) {
    let policy_path = home_substrate.join("policy.yaml");
    let mut policy = fs::read_to_string(&policy_path).expect("read packet three policy");
    policy.push_str(
        "    fork:\n      requests_allowed: true\n      recommendations_allowed: true\n    obligations:\n      approval_allowed: true\n",
    );
    fs::write(&policy_path, policy).expect("write packet three policy");
}
...
fn wait_for_obligation_record(
    ...
    let deadline = Instant::now() + timeout;
    let path = canonical_obligation_path(substrate_home, orchestration_session_id, obligation_id);
    while Instant::now() < deadline {
        if path.is_file() {
    ...
    panic!(
        "timed out waiting for obligation {} in session {} at {}",
        obligation_id,
...
#[test]
#[serial]
fn c3_targeted_world_turn_persists_packet_three_worker_event_obligations() {
    let cases = [
        PacketThreeObligationCase {
            event_class: "approval_request",
            obligation_kind: "approval_required",
            attach_state: "eligible",
            attention_required: true,
        },
        PacketThreeObligationCase {
            event_class: "fork_request",
            obligation_kind: "fork_request",
            attach_state: "eligible",
            attention_required: true,
        },
        PacketThreeObligationCase {
            event_class: "fork_recommendation",
            obligation_kind: "fork_recommendation",
            attach_state: "not_eligible",
            attention_required: false,
        },
    ];

    for case in cases {
        let temp = temp_dir("substrate-c3-targeted-world-packet3-");
        let home = temp.path().join("home");
        let project = temp.path().join("project");
        let substrate_home = home.join(".substrate");
        fs::create_dir_all(&home).expect("create home");
        ...
        enable_packet_three_worker_event_policy(&substrate_home);

        let sock_temp = short_socket_dir("sub-c3ws-targeted-world-packet3-");
        let sock = sock_temp.path().join("world.sock");
        let server = ReplWorldAgentStub::start_with_member_dispatch_scripts(
            &sock,
            StreamBehavior::Normal,
            vec![MemberDispatchStreamScript::ReadyAndHoldUntilCancel {
                session_handle_id: format!("session-targeted-packet3-{}", case.event_class),
                exit_code_on_cancel: 130,
            }],
        );
        let records = server.records();
        ...
        let prompt = format!(
            "::cli:codex {PACKET_THREE_MEMBER_TURN_PROMPT_PREFIX}{}",
            case.event_class
        );
        repl.send_line(&prompt);
        wait_for_min_member_turn_submit_requests(&records, 1, Duration::from_secs(3));
        repl.wait_for_output(
            &format!(
                "__MEMBER_TURN_SUBMIT_STUB__ {PACKET_THREE_MEMBER_TURN_PROMPT_PREFIX}{}",
                case.event_class
            ),
            Duration::from_secs(3),
        )
        ...
        let submit = {
            ...
            assert_eq!(
                guard.member_dispatch_requests.len(),
                1,
                "packet three targeted follow-up must reuse the retained member instead of relaunching it: {guard:#?}"
            );
            assert_eq!(
                guard.member_turn_submit_requests.len(),
                1,
                "packet three targeted follow-up must submit exactly one typed member turn request: {guard:#?}"
            );
            guard.member_turn_submit_requests[0].clone()
        };
        assert_eq!(submit.orchestration_session_id, orchestration_session_id);
        assert_eq!(submit.participant_id, member_participant_id);
        assert_eq!(
        ...
        assert_eq!(
            submit.prompt,
            format!(
                "{PACKET_THREE_MEMBER_TURN_PROMPT_PREFIX}{}",
                case.event_class
            )
        );

        let obligation_id = format!("obl_continue_{}_{}", submit.run_id, case.obligation_kind);
        let obligation = wait_for_obligation_record(
            &substrate_home,
            &orchestration_session_id,
            &obligation_id,
            Duration::from_secs(5),
        ...
        assert_eq!(
            ...
            Some(case.obligation_kind)
        );
        assert_eq!(
            obligation.get("attach_state").and_then(Value::as_str),
            Some(case.attach_state)
        );
        assert_eq!(
            obligation
                .get("attention_required")
                .and_then(Value::as_bool),
            Some(case.attention_required)
        );
        assert_eq!(
            obligation
                .get("source_participant_id")
                .and_then(Value::as_str),
            Some(member_participant_id.as_str())
        );
        assert_eq!(
            obligation.get("target_backend_id").and_then(Value::as_str),
            Some("cli:codex")
        );
        assert_eq!(
            obligation.get("world_id").and_then(Value::as_str),
            Some(world_id.as_str())
        );
        assert_eq!(
            obligation.get("world_generation").and_then(Value::as_u64),
            Some(world_generation)
        );
        assert_eq!(
            obligation
                .pointer("/payload/event_class")
                .and_then(Value::as_str),
        ...
        assert_eq!(
            obligation
                ...
                .and_then(Value::as_str),
            Some(submit.run_id.as_str())
        );
        assert_eq!(
            obligation
                .pointer("/payload/target_participant_id")
                .and_then(Value::as_str),
            Some(orchestrator_participant_id.as_str())
        );
        assert_eq!(
            obligation
                .pointer("/payload/source_backend_id")
                .and_then(Value::as_str),
            Some("cli:codex")
        );
        assert_eq!(
            obligation
                .pointer("/payload/payload/message")
                .and_then(Value::as_str),
            Some(format!("stub packet three payload for {}", case.event_class).as_str())
        );
        ...
        assert_eq!(
            live_members_after[0]
                ...
                .and_then(Value::as_str),
            Some(member_participant_id.as_str()),
            "packet three targeted follow-up {} must not allocate a child worker",
            case.event_class
        );
...
fn c3_targeted_world_turn_cancels_denied_packet_three_worker_events() {
    ...
    let submit_run_id = {
        ...
        assert!(
            guard.execute_cancel_requests.len() >= 1,
            "truncated packet three targeted turn must issue at least one cancel request: {guard:#?}"
        );
        assert!(
            guard.execute_cancel_requests.iter().any(|request| {
                request.span_id == format!("member-turn-span-{member_participant_id}")
            }),
            "truncated packet three targeted turn must cancel the live member-turn span: {guard:#?}"
        );
...
#[test]
#[serial]
fn c3_targeted_world_turn_fails_closed_when_packet_three_stream_ends_without_exit() {
    let temp = temp_dir("substrate-c3-targeted-world-packet3-truncated-");
    let home = temp.path().join("home");
    let project = temp.path().join("project");
    let substrate_home = home.join(".substrate");
    fs::create_dir_all(&home).expect("create home");
    ...
    let sock_temp = short_socket_dir("sub-c3ws-targeted-world-packet3-truncated-");
    let sock = sock_temp.path().join("world.sock");
    let server = ReplWorldAgentStub::start_with_member_dispatch_scripts(
        &sock,
        StreamBehavior::Normal,
        vec![MemberDispatchStreamScript::ReadyAndHoldUntilCancel {
            session_handle_id: "session-targeted-packet3-truncated".to_string(),
            exit_code_on_cancel: 130,
        }],
    );
    let records = server.records();
    ...
    repl.send_line(&format!(
        "::cli:codex {PACKET_THREE_MEMBER_TURN_TRUNCATED_PROMPT_PREFIX}approval_request"
    ));
    wait_for_min_member_turn_submit_requests(&records, 1, Duration::from_secs(3));
    wait_for_min_execute_cancel_requests(&records, 1, Duration::from_secs(3));
    repl.wait_for_output(
        "world follow-up stream ended without a terminal exit frame",
        Duration::from_secs(3),
    )
    .expect("truncated packet three error");
    repl.wait_for_prompt(Duration::from_secs(3))
        .expect("prompt after truncated packet three error");
    ...
    let submit_run_id = {
        ...
        assert!(
            guard.execute_cancel_requests.len() >= 1,
            "truncated packet three targeted turn must cancel at least one submitted world follow-up"
        );
        assert!(
            guard.execute_cancel_requests.iter().any(|request| {
                request.span_id == format!("member-turn-span-{member_participant_id}")
            }),
            "truncated packet three targeted turn must cancel the member-turn span before failing closed"
        );
        guard.member_turn_submit_requests[0].run_id.clone()
    };
    let truncated_obligation_path = canonical_obligation_path(
        &substrate_home,
        &orchestration_session_id,
        &format!("obl_continue_{submit_run_id}_approval_required"),
    );
    assert!(
        !truncated_obligation_path.exists(),
        "truncated packet three targeted turn must fail before durable obligation persistence"
    );

...
fn manager_manifest_path() -> std::path::PathBuf {
    std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../config/manager_hooks.yaml")
}
...
impl PtyRepl {
    fn spawn(project_dir: &Path, home_dir: &Path, substrate_home: &Path) -> Self {
        ...
        #[cfg(unix)]
        let master_fd = master.as_raw_fd().expect("master fd");
        ...
        let reader_handle = std::thread::spawn(move || {
            ...
            loop {
                ...
                #[cfg(unix)]
                {
                    let rc = unsafe { libc::read(master_fd, buf.as_mut_ptr().cast(), buf.len()) };
                    if rc == 0 {
                        break;
                    }
                    if rc < 0 {
    ...
    fn disconnect_master(&mut self) {
        // Ensure we release the PTY master FD to simulate terminal teardown/hangup on Linux.
        self.stop_reader.store(true, Ordering::Relaxed);
        self.master.take();
    }
...
#[test]
#[serial]
fn repl_pty_master_disconnect_exits_1_on_reedline_path() {
    let temp = temp_dir("substrate-repl-tty-disconnect-");
    ...
    repl.wait_for_reedline_probe(Duration::from_secs(3))
        .expect("expected Reedline prompt traffic before master disconnect");

    repl.disconnect_master();

    let (code, out, exited_in_time) = repl.shutdown();

    assert!(
        exited_in_time,
        "async REPL did not exit within the bounded timeout after PTY master disconnect; output:\n{out}"
    );
    assert_eq!(
        code, 1,
    ...
    // Master disconnect can prevent post-disconnect diagnostics from reaching the harness; require
    // only that we don't emit repeated spam in the captured output.
    let abnormal_loss_count = out
        .to_ascii_lowercase()
        .matches("abnormal terminal loss")
        .count();
    assert!(
        abnormal_loss_count <= 1,
        "expected at most one abnormal-terminal-loss diagnostic fragment; output:\n{out}"
    );

    assert!(
        out.to_ascii_lowercase().contains("substrate"),
        "expected substrate output to be captured around the disconnect path; output:\n{out}"
    );
}

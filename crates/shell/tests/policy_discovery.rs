...
fn policy_current_show_uses_declared_prefix_under_conflicting_ambient_home() {
    ...
    let secure_parent = std::env::var_os("XDG_RUNTIME_DIR")
        ...
        .unwrap_or_else(|| {
            ...
            let account_home = unsafe { std::ffi::CStr::from_ptr(passwd.pw_dir) }
                .to_str()
                .expect("current account home is UTF-8");
            let account_home = PathBuf::from(account_home);
            assert!(

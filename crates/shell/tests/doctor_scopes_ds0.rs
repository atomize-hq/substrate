...
fn host_doctor_uses_installed_witness_under_conflicting_ambient() {
    ...
    let ambient_before = std::fs::read(&ambient_config).expect("snapshot ambient config");
    ...
    assert_eq!(
        std::fs::read(&ambient_config).expect("read unchanged ambient config"),
        ambient_before,
        "Host dispatch must not mutate conflicting ambient B"
    );
    ...
    assert_eq!(
        std::fs::read(&ambient_config).expect("read ambient config after rejected invocation"),
        ambient_before,
        "rejected direct invocation must fail before mutation"
    );
    assert!(

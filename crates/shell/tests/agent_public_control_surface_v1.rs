...
fn wait_for_obligation_record(
    ...
    let path = canonical_obligation_path(
...
#[cfg(target_os = "linux")]
fn enable_packet_three_worker_event_policy(fixture: &AgentControlFixture) {
    let policy_path = fixture.substrate_home.join("policy.yaml");
...
#[test]
#[serial]
fn public_turn_persists_packet_three_worker_event_obligations_for_world_follow_up() {
    let cases = [
    ...
    for case in cases {
        ...
        enable_packet_three_worker_event_policy(&fixture);

        let socket_home = tempfile::Builder::new()
            .prefix("sac-world-packet3-")
            .tempdir_in("/tmp")
            .expect("socket tempdir");
        let socket_path = socket_home.path().join("world.sock");
        let server = ReplWorldAgentStub::start_with_member_dispatch_scripts(
            ...
            StreamBehavior::Normal,
            vec![MemberDispatchStreamScript::ReadyAndHoldUntilCancel {
                session_handle_id: format!("session-public-packet3-{}", case.event_class),
                exit_code_on_cancel: 130,
            }],
        );
        let records = server.records();
        ...
        let owner_pid = fixture.load_orchestration_session(&orchestration_session_id)
            ["shell_owner_pid"]
            .as_u64()
            .expect("owner pid") as u32;
        let live_members = wait_for_live_world_member_count(
        ...
        let turn_output = fixture
            ...
            .output()
            .expect("run public packet three turn");
        assert!(
            turn_output.status.success(),
            "public packet three world turn must succeed for {}: stdout=\n{}\nstderr=\n{}",
            case.event_class,
            String::from_utf8_lossy(&turn_output.stdout),
        ...
        let submit = {
            ...
            assert_eq!(
                ...
                1,
                "public packet three turn must reuse the retained member instead of relaunching it: {guard:#?}"
            );
            assert_eq!(
                guard.member_turn_submit_requests.len(),
                1,
                "public packet three turn must submit exactly one typed member turn request: {guard:#?}"
            );
        ...
        assert_eq!(submit.world_id, world_id);
        assert_eq!(submit.world_generation, world_generation);
        assert_eq!(submit.prompt, prompt);
        ...
        assert_eq!(
            live_members_after[0]
                ...
                .and_then(Value::as_str),
            Some(member_participant_id.as_str()),
            "public packet three {} turn must not allocate a child worker",
            case.event_class
        );
...
#[test]
#[serial]
fn public_turn_fails_closed_when_packet_three_stream_ends_without_exit() {
    let fixture = AgentControlFixture::new();
    fixture.init_workspace();
    fixture.write_runtime_inventory_with_member_backend_and_world_dispatch(
        Some("codex_world"),
    ...
    let socket_home = tempfile::Builder::new()
        .prefix("sac-world-packet3-truncated-")
        .tempdir_in("/tmp")
        .expect("socket tempdir");
    let socket_path = socket_home.path().join("world.sock");
    let server = ReplWorldAgentStub::start_with_member_dispatch_scripts(
        ...
        StreamBehavior::Normal,
        vec![MemberDispatchStreamScript::ReadyAndHoldUntilCancel {
            session_handle_id: "session-public-packet3-truncated".to_string(),
            exit_code_on_cancel: 130,
        }],
    );
    let records = server.records();
    ...
    let prompt = format!("{PACKET_THREE_MEMBER_TURN_TRUNCATED_PROMPT_PREFIX}approval_request");
    let turn_output = fixture
        .command()
        .current_dir(&fixture.workspace_root)
        .env("SUBSTRATE_WORLD_SOCKET", &socket_path)
        ...
        .output()
        .expect("run truncated public packet three turn");
    assert!(
        !turn_output.status.success(),
        "truncated packet three world turn must fail closed: stdout=\n{}\nstderr=\n{}",
        String::from_utf8_lossy(&turn_output.stdout),
        String::from_utf8_lossy(&turn_output.stderr),
    );
    let surfaced_output = format!(
    ...
    assert!(
        surfaced_output.contains("world follow-up stream ended without a terminal exit frame"),
        "truncated packet three turn must surface the fail-closed stream error: {surfaced_output}"
    );
    ...
    assert!(
        ...
        "truncated packet three world turn must fail before durable obligation persistence"
    );

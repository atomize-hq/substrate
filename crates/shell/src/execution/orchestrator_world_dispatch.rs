...
async fn wait_for_supervised_ephemeral_terminal_truth(
    ...
    loop {
        ...
        if let Some(terminal) = observation.terminal {
            return Ok(world_task_terminal_state_from_exit_code(terminal.exit_code));
        }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match prepared.request.mode {
            ...
            WorldDispatchModeV1::Ephemeral => {
                ...
                let terminal_state = match wait_for_supervised_ephemeral_terminal_truth(
                    &resolved.execution_supervisor,
                    &resolved.acceptance_record_id,
                    &resolved.session.orchestration_session_id,
                    &resolved.task_run_id,
                )
                .await?
...
fn resolve_active_ephemeral_inspect_target(
    ...
    let observation = registry_authority.resolve_active_ephemeral_observation(
        &prepared.session.orchestration_session_id,
        prepared.caller_participant.participant_id(),
        Some(&prepared.request.target_backend_id),
        &world_id,
        world_generation,
        task_run_id,
    )?;
    ...
    Ok(ResolvedActiveEphemeralInspectTarget {
        ...
        target_backend_id: claim.target_backend_id,
    })
}
        ...
        .map_err(|err| {
            anyhow::anyhow!(
                ...
                format_continue_world_worker_fork_command_bootstrap_failure(
                    target_participant_id,
                    map_fork_world_dispatch_resolution_error(err).to_string(),
                )
            )
        })?;
    let workspace_root = PathBuf::from(&prepared.session.workspace_root);
    ...
    match &prepared.request.payload {
        ...
        WorldDispatchPayloadV1::WorkerContinueForkCommand(_) => {
            if !base_policy.world_dispatch_fork_commands_allowed() {
                return Err(steering_policy_denial(
                    WorldDispatchSteeringDenialV1::ActionNotAllowed,
                    "effective policy does not allow continue_world_worker fork_command payloads",
                ));
            }
            Ok(())
        }
...
async fn stop_world_worker(
    ...
    let mut used_private_stop_surface = true;
    let closeout = if detached_stop_world_worker_closeout_available(
        &prepared.store,
        &resolved,
        &transport_path,
        None,
...
async fn wait_for_fork_child_durable_publication(
    ...
    wait_for_fork_child_durable_publication_with_timeouts(
        ...
        FORK_CHILD_STOP_TRANSPORT_VISIBILITY_WAIT_TIMEOUT,
        FORK_CHILD_REGISTRATION_VISIBILITY_POLL_INTERVAL,
    )
    .await
}

#[cfg(target_os = "linux")]
async fn wait_for_fork_child_durable_publication_with_timeouts(
    store: &AgentRuntimeStateStore,
    orchestration_session_id: &str,
    child_participant_id: &str,
    child_visibility_wait_timeout: Duration,
    stop_transport_visibility_wait_timeout: Duration,
    poll_interval: Duration,
) -> Result<()> {
    let stop_transport_path =
        private_stop_transport_path(store, orchestration_session_id, child_participant_id);
    let started_at = tokio::time::Instant::now();
    let mut child_visible_at = None;

    loop {
        let child_visible = store.load_participant(child_participant_id)?.is_some();
        if child_visible && child_visible_at.is_none() {
            child_visible_at = Some(tokio::time::Instant::now());
        }
        ...
        let child_visibility_timed_out = started_at.elapsed() >= child_visibility_wait_timeout;
        let stop_transport_timed_out = child_visible_at
            .map(|observed_at| observed_at.elapsed() >= stop_transport_visibility_wait_timeout)
            .unwrap_or(false);
        if (!child_visible && child_visibility_timed_out)
            || (child_visible && !stop_transport_published && stop_transport_timed_out)
        {
...
#[cfg(target_os = "linux")]
fn detached_stop_world_worker_closeout_available(
    store: &AgentRuntimeStateStore,
    resolved: &crate::execution::agent_runtime::state_store::ResolvedInternalStopWorldDispatchTarget,
    transport_path: &Path,
    transport_error: Option<&anyhow::Error>,
) -> bool {
    let owner_continuity = store
        ...
        .ok();
    let detached_owner_unreachable = matches!(
        owner_continuity,
        Some(
            crate::execution::agent_runtime::state_store::HiddenOwnerHelperLaunchContinuity::DetachedReconciled(
                _
            ) | crate::execution::agent_runtime::state_store::HiddenOwnerHelperLaunchContinuity::StaleAttachedTruth
        )
    );
    if let Some(err) = transport_error {
        let refused_live_transport =
            transport_path.exists() && detached_stop_transport_is_unusable(err);
        if refused_live_transport && parked_retained_target_stop_transport_contradiction(resolved) {
            return true;
        }
        if !detached_owner_unreachable {
            return false;
        }
        return !transport_path.exists() || detached_stop_transport_is_unusable(err);
    }
...
#[cfg(target_os = "linux")]
fn detached_stop_world_worker_closeout_available_after_transport_failure(
    store: &AgentRuntimeStateStore,
    resolved: &crate::execution::agent_runtime::state_store::ResolvedInternalStopWorldDispatchTarget,
    transport_path: &Path,
...
#[cfg(target_os = "linux")]
fn parked_retained_target_stop_transport_contradiction(
    resolved: &crate::execution::agent_runtime::state_store::ResolvedInternalStopWorldDispatchTarget,
) -> bool {
    resolved.session.state == crate::execution::agent_runtime::orchestration_session::OrchestrationSessionState::Active
        && resolved.session.posture
            == crate::execution::agent_runtime::orchestration_session::OrchestrationSessionPosture::ParkedResumable
        && resolved.target_participant.handle.execution.scope == AgentExecutionScope::World
        && resolved.target_participant.handle.state.is_live()
        && resolved.target_participant.internal.terminal_observed_at.is_none()
        && !resolved.target_participant.is_resume_eligible()
        && resolved.target_participant.internal.control_owner_retained
        && resolved.target_participant.internal.event_stream_active
        && resolved
            .target_participant
            .internal
            .completion_observer_retained
        && resolved.target_participant.internal.ownership_valid
        && matches!(
            resolved.target_participant.internal.ownership_mode,
            Some(
                crate::execution::agent_runtime::session::AgentRuntimeOwnershipMode::MemberRuntime
            )
        )
}

#[cfg(target_os = "linux")]
fn persist_detached_stop_world_worker_closeout(
    store: &AgentRuntimeStateStore,
    resolved: &crate::execution::agent_runtime::state_store::ResolvedInternalStopWorldDispatchTarget,
) -> Result<RetainedWorkerStopCloseoutV1> {
    ...
    Ok(RetainedWorkerStopCloseoutV1 {
        participant_state: participant.handle.state,
        session_state: session.state,
    })
}
...
#[cfg(target_os = "linux")]
fn summarize_stop_world_worker_result(
    participant_id: &str,
    backend_id: &str,
    used_private_stop_surface: bool,
) -> String {
    if used_private_stop_surface {
        return format!(
...
pub(crate) mod tests {
    ...
    use crate::execution::agent_runtime::tool_invocation_contract::{
        normalize_inspect_world_worker_outcome_v1, normalize_stop_world_worker_outcome_v1,
    };
    use crate::execution::agent_runtime::tool_invocation_contract::{
    ...
    #[cfg(target_os = "linux")]
    fn sample_continue_fork_command_world_dispatch_request() -> WorldDispatchRequestV1 {
        WorldDispatchRequestV1 {
            request_id: Some("req_continue_fork_command".to_string()),
            idempotency_key: Some("idem_continue_fork_command".to_string()),
            orchestration_session_id: Some("sess_dispatch".to_string()),
            caller_participant_id: Some("orch_dispatch".to_string()),
            action: WorldDispatchActionV1::ContinueWorldWorker,
            mode: WorldDispatchModeV1::Retained,
            ...
            payload: WorldDispatchPayloadV1::WorkerContinueForkCommand(
                WorkerContinueForkCommandPayloadV1 {
                    ...
                    fork_reason: Some("parallelize_investigation".to_string()),
                    fork_strategy: Some("exact_source_retained".to_string()),
                    thread_id: Some("thread-fork-command-45".to_string()),
                },
            ),
        }
    }
    ...
    #[cfg(target_os = "linux")]
    pub(crate) fn seed_b21_ephemeral_observation(
        task_run_id: &str,
    ) -> (
        TempDir,
        WorldWorkReceiptRegistry,
        ...
        .expect("persist accepted B2.1 test task");
        let claim = claim.expect("B2.1 test task has durable claim");
        execution_supervisor
            .journal_frame(
                &claim,
                &start,
                &start
                    ...
                    .expect("canonical B2.1 test Start"),
            )
            .expect("journal B2.1 test Start");
        (root, receipt_registry, execution_supervisor, claim)
    }

    #[cfg(target_os = "linux")]
    pub(crate) fn b21_registry_authority_for_test(
        receipt_registry: WorldWorkReceiptRegistry,
        execution_supervisor: WorldWorkExecutionSupervisor,
        authority_store_id: &str,
    ) -> crate::execution::agent_runtime::state_store::ResolvedWorldWorkRegistryAuthorityV1 {
        crate::execution::agent_runtime::state_store::ResolvedWorldWorkRegistryAuthorityV1 {
            receipt_registry,
            execution_supervisor,
    ...
    #[cfg(target_os = "linux")]
    fn persist_stale_attached_continue_dispatch_state(
        store: &AgentRuntimeStateStore,
        workspace_root: &Path,
        world_id: &str,
        world_generation: u64,
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn continue_world_worker_submit_request_renders_typed_progress_ack_deterministically() {
        let prepared = PreparedOrchestratorWorldDispatch {
            store: sample_state_store(),
            request: sample_continue_progress_ack_world_dispatch_request()
                .validate()
                .expect("validated progress-ack continue request"),
            session: sample_session(),
            caller_participant: sample_orchestrator_participant(),
            target_participant: Some(sample_member_participant()),
            live_retained_worker_count: 1,
...
"#,
    ...
    async fn dispatch_contract_continue_world_worker_progress_ack_reaches_target_resolution_when_policy_enabled(
        ...
        .await
        .expect_err("progress_ack routing should continue past policy gating once enabled");
        let message = err.to_string();
    ...
    async fn dispatch_contract_continue_world_worker_progress_ack_submits_rendered_prompt_to_exact_retained_worker(
        ...
        let socket_home = tempdir().expect("socket tempdir");
        let socket_path = socket_home.path().join("progress-ack-success.sock");
        ...
        let store = AgentRuntimeStateStore::new().expect("state store");
        persist_authoritative_continue_dispatch_state(&store, workspace_root.path(), "world-17", 2);
        let request = sample_continue_progress_ack_world_dispatch_request();
        let expected_prompt = render_continue_world_worker_transport_prompt(
            &request
                .clone()
                .validate()
                .expect("validated progress-ack request")
                .payload,
        )
        .expect("render progress-ack prompt");

        let outcome = dispatch_orchestrator_world_request(&store, request)
            .await
            .expect("progress_ack delivery should succeed");
        let WorldDispatchOutcomeV1::ContinueWorldWorker(outcome) = outcome else {
        ...
        assert!(
            ...
            "typed progress_ack delivery may surface a generic reply, but must not imply a landed control_ack worker event: {:?}",
        ...
        assert!(
            ...
            "progress_ack summary must not imply completion: {}",
            outcome.summary
        );
        assert!(
            !outcome.summary.contains("control_directive"),
            "progress_ack summary must not imply control acknowledgement: {}",
            outcome.summary
        );
        assert!(
            !outcome.summary.contains("fork_command"),
            "progress_ack summary must not imply fork handling: {}",
            outcome.summary
        );
        ...
        assert_eq!(
            recorded.len(),
            1,
            "typed progress_ack should submit exactly one retained member turn: {recorded:?}"
        );
    ...
    async fn dispatch_contract_continue_world_worker_fork_command_denies_by_default_before_target_resolution(
        ...
        .await
        .expect_err("fork-command payloads must fail closed by default");
        let message = err.to_string();

        assert_eq!(
            message,
            "action_not_allowed: effective policy does not allow continue_world_worker fork_command payloads"
        );
        assert!(
            !message.contains("stale_linkage:"),
            "payload gate must not leak retained-worker lifecycle truth first: {message}"
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    #[serial]
    async fn dispatch_contract_continue_world_worker_fork_command_tolerates_late_child_durable_publication_after_authoritative_registration(
    ) {
        let _env_guard = world_env_guard();
        let _world_codex_guard = EnvVarGuard::set_path(
            "SUBSTRATE_WORLD_DEPS_GUEST_BIN_DIR",
        ...
        let socket_path = socket_home
            .path()
            .join("fork-command-late-child-publication.sock");
        let store = AgentRuntimeStateStore::new().expect("state store");
        persist_authoritative_continue_dispatch_state(&store, workspace_root.path(), "world-17", 2);
        let store_for_server = store.clone();
        let listener = UnixListener::bind(&socket_path).expect("bind stub world socket");
                        ...
                        &transport_api_types::ExecuteStreamFrame::Event {
                            ...
                            event: sample_continue_stream_uaa_event_for_run(
                                ...
                                json!({
                                    ...
                                    "item_type": "agent_message",
                                    "content": {
                                        "text": "fork command delivered before delayed child publication"
                                    }
                                }),
                            ),
                        },
    ...
    async fn dispatch_contract_continue_world_worker_fork_command_wraps_post_delivery_source_invalidation_as_bootstrap_failure(
                    ...
                    if first_line.starts_with("POST /v1/member_turn/stream ") {
                        ...
                        &transport_api_types::ExecuteStreamFrame::Event {
                            ...
                            event: sample_continue_stream_uaa_event_for_run(&parsed.run_id, json!({
                                ...
                                "item_type": "agent_message",
                                "content": {
                                    "text": "fork command delivered before source invalidation"
                                }
                            })),
                        },
                    )
        ...
        .await
        .expect_err("post-delivery source invalidation must fail inside bootstrap");
...
"#,
        ...
        .expect("write fork-command and fork-request policy");

        let socket_home = tempdir().expect("socket tempdir");
        let socket_path = socket_home
            .path()
            .join("fork-command-obligation-before-bootstrap-failure.sock");
        let listener = UnixListener::bind(&socket_path).expect("bind stub world socket");
        let store = AgentRuntimeStateStore::new().expect("state store");
        let store_for_server = store.clone();
        let server = tokio::spawn(async move {
            while let Ok((mut stream, _addr)) = listener.accept().await {
                ...
                if first_line.starts_with("POST /v1/member_turn/stream ") {
                    ...
                    let mut source = store_for_server
                        ...
                        .expect("load authoritative source")
                        .expect("authoritative source exists");
                    source.mark_terminal_state("worker invalidated after surfacing fork_request");
                    source.transition_state(AgentRuntimeSessionState::Invalidated);
                    store_for_server
                        .persist_participant(&source)
                        .expect("persist invalidated authoritative source");
        ...
        let obligation = store
            .load_obligation(
                "sess_dispatch",
                "obl_continue_req_continue_fork_command_fork_request",
            )
            .expect("load persisted fork-request obligation")
            .expect("fork-request obligation must persist before bootstrap failure returns");
        assert_eq!(obligation.kind, OrchestrationObligationKind::ForkRequest);
        assert_eq!(
            obligation.source_participant_id.as_deref(),
            Some("ash_member")
        ...
        assert_eq!(
            obligation
                ...
                .and_then(|payload| payload.pointer("/payload/message"))
                .and_then(serde_json::Value::as_str),
            Some("persist me before child bootstrap fails")
        );
    ...
    async fn dispatch_contract_inspect_world_worker_ephemeral_returns_authoritative_snapshot_without_mutation(
        ...
        let (_root, registry, supervisor, claim) = seed_b21_ephemeral_observation("task-run-47");
        let authority =
            b21_registry_authority_for_test(registry, supervisor, &claim.authority_store_id);
        let before = authority
            .execution_supervisor
            .inspect_observation_by_acceptance_id(&claim.acceptance_record_id)
            .expect("inspect before compatibility read");
        let observation = authority
            .resolve_active_ephemeral_observation(
                &claim.orchestration_session_id,
                &claim.caller_participant_id,
                Some(&claim.target_backend_id),
                &claim.world_id,
                claim.world_generation,
                "task-run-47",
            )
            .expect("resolve exact active task from receipt/supervisor truth");
        ...
        assert_eq!(
            authority
                .execution_supervisor
                .inspect_observation_by_acceptance_id(&observation.claim.acceptance_record_id)
                .expect("inspect after compatibility read"),
            before,
            "compatibility inspection must not mutate or rotate supervisor truth"
        );
    }
    ...
    async fn dispatch_contract_inspect_world_worker_ephemeral_resolves_exact_session_binding_when_task_run_id_is_reused(
        ...
        let (_root, registry, supervisor, claim) =
            seed_b21_ephemeral_observation("task-run-shared");
        let authority =
            b21_registry_authority_for_test(registry, supervisor, &claim.authority_store_id);
        assert!(authority
            .resolve_active_ephemeral_observation(
    ...
    {
        let (_root, registry, supervisor, claim) =
            seed_b21_ephemeral_observation("task-run-present");
        let authority =
            b21_registry_authority_for_test(registry, supervisor, &claim.authority_store_id);
        let err = authority
            .resolve_active_ephemeral_observation(
        ...
        assert_eq!(
            err.to_string(),
            format!(
                "active_task_not_found: orchestration session {} has no exact active ephemeral task task-run-missing",
                claim.orchestration_session_id
            )
        );
    }
    ...
    async fn dispatch_contract_inspect_world_worker_ephemeral_fails_closed_for_stale_linkage() {
        let (_root, registry, supervisor, claim) = seed_b21_ephemeral_observation("task-run-stale");
        let authority =
            b21_registry_authority_for_test(registry, supervisor, &claim.authority_store_id);
        let err = authority
            .resolve_active_ephemeral_observation(
            ...
            .expect_err("stale linkage must fail closed");
        assert!(err.to_string().starts_with("stale_linkage:"));
    }
    ...
    async fn dispatch_contract_inspect_world_worker_ephemeral_fails_closed_for_backend_mismatch() {
        let (_root, registry, supervisor, claim) =
            seed_b21_ephemeral_observation("task-run-backend");
        let authority =
            b21_registry_authority_for_test(registry, supervisor, &claim.authority_store_id);
        let err = authority
            .resolve_active_ephemeral_observation(
    ...
    async fn dispatch_contract_inspect_world_worker_ephemeral_fails_closed_for_world_binding_mismatch(
        ...
        let (_root, registry, supervisor, claim) =
            seed_b21_ephemeral_observation("task-run-binding");
        let authority =
            b21_registry_authority_for_test(registry, supervisor, &claim.authority_store_id);
        let err = authority
            .resolve_active_ephemeral_observation(
        ...
        assert!(err.to_string().starts_with("world_binding_mismatch:"));
    }
    ...
    fn register_active_ephemeral_world_task_rejects_duplicate_task_run_id() {
        ...
        fs::create_dir_all(&safe_parent).expect("create safe legacy-writer fixture parent");
        let substrate_home =
            tempfile::tempdir_in(safe_parent).expect("safe substrate home tempdir");
        fs::set_permissions(substrate_home.path(), fs::Permissions::from_mode(0o700))
            .expect("secure legacy-writer fixture root");
        let _substrate_home_guard = EnvVarGuard::set_path("SUBSTRATE_HOME", substrate_home.path());
        let store = AgentRuntimeStateStore::new().expect("state store");
        let _guard = store
    ...
    async fn dispatch_contract_inspect_world_worker_ephemeral_teardown_removes_routability() {
        let (_root, registry, supervisor, claim) =
            seed_b21_ephemeral_observation("task-run-terminal");
        let authority = b21_registry_authority_for_test(
            registry,
            supervisor.clone(),
            &claim.authority_store_id,
        );
        let terminal = b21_test_exit_frame("rts_shell_world_dispatch_fixture", "task-run-terminal");
        supervisor
            .journal_frame(
                &claim,
                &terminal,
        ...
        let err = authority
            ...
            .expect_err("terminal active task must not remain routable");
        assert!(err.to_string().starts_with("active_task_not_found:"));
    }
    ...
    async fn dispatch_contract_cancel_world_work_ephemeral_routes_exact_active_task_over_execute_cancel(
        ...
        let (_root, _registry, supervisor, claim) =
            seed_b21_ephemeral_observation("task-run-cancel-live");
        let socket_home = tempdir().expect("cancel socket tempdir");
        let socket_path = socket_home.path().join("world.sock");
        let listener = UnixListener::bind(&socket_path).expect("bind cancel socket");
        let (cancel_seen_tx, cancel_seen_rx) = tokio::sync::oneshot::channel();
        let server = tokio::spawn(async move {
            let (mut stream, _addr) = listener.accept().await.expect("accept cancel request");
            let (header, body) = read_http_request(&mut stream)
                .await
            ...
            assert!(header
                ...
                .next()
                .unwrap_or_default()
                .starts_with("POST /v1/execute/cancel "));
            let parsed: ExecuteCancelRequestV1 =
                serde_json::from_slice(&body).expect("decode exact cancel request");
            ...
            write_http_json(
                ...
                .expect("encode cancel response"),
            )
            .await;
            cancel_seen_tx.send(()).expect("signal cancel delivery");
        });
        let _socket_guard = EnvVarGuard::set_path("SUBSTRATE_WORLD_SOCKET", &socket_path);
        let waiter_supervisor = supervisor.clone();
        let acceptance_record_id = claim.acceptance_record_id.clone();
        let waiter = tokio::spawn(async move {
            wait_for_supervised_ephemeral_terminal_truth(
                &waiter_supervisor,
        ...
        let delivered = connect_world_agent_client()
            .expect("connect exact cancel client")
            .cancel_execute(ExecuteCancelRequestV1 {
                span_id: "task-run-cancel-live".to_string(),
                sig: "TERM".to_string(),
            })
            .await
            .expect("deliver exact execute cancel");
        assert!(delivered.delivered);
        cancel_seen_rx.await.expect("observe exact cancel delivery");
        tokio::task::yield_now().await;
        assert!(
            !waiter.is_finished(),
            "cancel remains blocking until exact terminal"
        );

        let mut terminal =
            b21_test_exit_frame("rts_shell_world_dispatch_fixture", "task-run-cancel-live");
        let transport_api_types::ExecuteStreamFrame::Exit { exit, .. } = &mut terminal else {
            unreachable!("cancel terminal fixture is Exit")
        };
        *exit = 130;
        supervisor
            .journal_frame(
                ...
                &terminal
                    .canonical_ndjson_bytes()
                    .expect("canonical cancel terminal"),
            )
            .expect("journal exact cancel terminal");
        assert_eq!(
            waiter
                .await
                .expect("join cancel waiter")
                .expect("resolve cancel terminal"),
            WorldTaskTerminalStateV1::Cancelled
        );
        server.await.expect("join exact cancel server");
    }
    #[cfg(target_os = "linux")]
    #[tokio::test(flavor = "current_thread")]
    #[serial]
    async fn dispatch_contract_cancel_world_work_ephemeral_retry_reuses_shared_terminal_truth() {
        let (_root, _registry, supervisor, claim) =
            seed_b21_ephemeral_observation("task-run-cancel-shared");
        let socket_home = tempdir().expect("retry cancel socket tempdir");
        let socket_path = socket_home.path().join("world.sock");
        let listener = UnixListener::bind(&socket_path).expect("bind retry cancel socket");
        let server = tokio::spawn(async move {
            for _ in 0..2 {
                let (mut stream, _addr) = listener.accept().await.expect("accept retry cancel");
                let (header, body) = read_http_request(&mut stream)
                    .await
                    .expect("read retry cancel request");
                assert!(header
                    .lines()
                    .next()
                    .unwrap_or_default()
                    .starts_with("POST /v1/execute/cancel "));
                let parsed: ExecuteCancelRequestV1 =
                    serde_json::from_slice(&body).expect("decode retry cancel request");
                assert_eq!(parsed.span_id, "task-run-cancel-shared");
                write_http_json(
                    &mut stream,
                    "200 OK",
                    &serde_json::to_string(&transport_api_types::ExecuteCancelResponseV1 {
                    ...
                    .expect("encode retry cancel response"),
                )
                .await;
            }
        });
        ...
        for _ in 0..2 {
            let delivered = connect_world_agent_client()
                .expect("connect retry cancel client")
                .cancel_execute(ExecuteCancelRequestV1 {
                    span_id: "task-run-cancel-shared".to_string(),
                    sig: "TERM".to_string(),
                })
                .await
                .expect("deliver retry execute cancel");
            assert!(delivered.delivered);
        }
        ...
        let mut terminal =
            b21_test_exit_frame("rts_shell_world_dispatch_fixture", "task-run-cancel-shared");
        let transport_api_types::ExecuteStreamFrame::Exit { exit, .. } = &mut terminal else {
            unreachable!("retry cancel terminal fixture is Exit")
        };
        *exit = 130;
        supervisor
            .journal_frame(
                ...
                &terminal
                    .canonical_ndjson_bytes()
                    .expect("canonical shared cancel terminal"),
            )
            .expect("journal shared cancel terminal");
        for waiter in waiters {
            assert_eq!(
                waiter
                    .await
                    .expect("join retry cancel waiter")
                    .expect("resolve shared cancel terminal"),
                WorldTaskTerminalStateV1::Cancelled
            );
        }
        server.await.expect("join retry cancel server");
    }
    #[cfg(target_os = "linux")]
    #[tokio::test(flavor = "current_thread")]
    #[serial]
    async fn dispatch_contract_cancel_world_work_ephemeral_fails_closed_when_execute_cancel_is_not_delivered(
        ...
        let (_root, _registry, supervisor, claim) =
            seed_b21_ephemeral_observation("task-run-cancel-race");
        let socket_home = tempdir().expect("not-delivered cancel socket tempdir");
        let socket_path = socket_home.path().join("world.sock");
        let listener = UnixListener::bind(&socket_path).expect("bind not-delivered cancel socket");
        let server = tokio::spawn(async move {
            let (mut stream, _addr) = listener
                .accept()
                .await
                .expect("accept not-delivered cancel");
            read_http_request(&mut stream)
                .await
                .expect("read not-delivered cancel request");
            write_http_json(
                &mut stream,
                "200 OK",
                &serde_json::to_string(&transport_api_types::ExecuteCancelResponseV1 {
                ...
                .expect("encode not-delivered response"),
            )
            .await;
        });
        let _socket_guard = EnvVarGuard::set_path("SUBSTRATE_WORLD_SOCKET", &socket_path);
        let delivered = connect_world_agent_client()
            .expect("connect not-delivered cancel client")
            .cancel_execute(ExecuteCancelRequestV1 {
                span_id: "task-run-cancel-race".to_string(),
                sig: "TERM".to_string(),
            })
            .await
            .expect("receive not-delivered cancel response");
        assert!(!delivered.delivered);
        let observation = supervisor
            .inspect_observation_by_acceptance_id(&claim.acceptance_record_id)
            .expect("inspect after not-delivered cancel")
            .expect("accepted observation survives cancel race");
        assert!(observation.terminal.is_none());
        assert_eq!(observation.durable_frame_cursor, Some(1));
        server.await.expect("join not-delivered cancel server");
    }
    ...
    async fn active_ephemeral_terminal_wait_allows_multiple_waiters_to_observe_same_terminal_truth()
    {
        ...
        let mut terminal =
            b21_test_exit_frame("rts_shell_world_dispatch_fixture", "task-run-shared");
        let transport_api_types::ExecuteStreamFrame::Exit { exit, .. } = &mut terminal else {
            unreachable!("shared terminal fixture is Exit")
        };
        *exit = 130;
        supervisor
            .journal_frame(
                ...
                &terminal
                    .canonical_ndjson_bytes()
                    .expect("canonical shared terminal"),
            )
            .expect("journal shared terminal");
        for waiter in waiters {
            assert_eq!(
                waiter
                    .await
                    .expect("join shared waiter")
                    .expect("resolve shared terminal"),
                WorldTaskTerminalStateV1::Cancelled
            );
        }
    }
    ...
    async fn active_ephemeral_terminal_wait_registration_guard_releases_non_happy_path_registrations(
        ...
        let (_root, _registry, supervisor, claim) =
            seed_b21_ephemeral_observation("task-run-cleanup");
        let waiter_supervisor = supervisor.clone();
        let acceptance_record_id = claim.acceptance_record_id.clone();
        let waiter = tokio::spawn(async move {
            wait_for_supervised_ephemeral_terminal_truth(
        ...
        tokio::task::yield_now().await;
        waiter.abort();
        assert!(waiter.await.expect_err("waiter is aborted").is_cancelled());
        assert!(supervisor
            .inspect_observation_by_acceptance_id(&claim.acceptance_record_id)
            .expect("inspect after waiter drop")
            .is_some());
    }
    ...
    async fn active_ephemeral_terminal_truth_guard_publishes_failed_terminal_truth_on_drop_after_start(
        ...
        let (_root, _registry, supervisor, claim) =
            seed_b21_ephemeral_observation("task-run-stream-failure");
        let observation = supervisor
            .inspect_observation_by_acceptance_id(&claim.acceptance_record_id)
            .expect("inspect interrupted observation")
            .expect("accepted observation survives local drop");
        assert_eq!(observation.durable_frame_cursor, Some(1));
        assert!(
            observation.terminal.is_none(),
            "local failure/drop must not fabricate terminal truth"
        );
    }
    ...
    async fn dispatch_contract_stop_world_worker_waits_for_late_private_stop_transport_publication_after_recovered_owner_path(
        ...
        let stop_owner = tokio::spawn(async move {
            ...
            let mut stop_transport =
                ...
                .await
                .expect("register late private stop transport");
            let request = tokio::time::timeout(Duration::from_secs(3), stop_rx.recv())
                .await
                .expect("timed out waiting for late private stop request")
                .expect("late private stop request");
            let mut session = store_for_task
                .load_orchestration_session("sess_dispatch")
                .expect("load orchestration session for late stop closeout")
                .expect("authoritative orchestration session for late stop closeout");
            let mut participant = store_for_task
                .load_participant("ash_member")
                .expect("load retained participant for late stop closeout")
                .expect("authoritative retained participant for late stop closeout");
            participant.transition_state(AgentRuntimeSessionState::Stopped);
    ...
    async fn dispatch_contract_stop_world_worker_persists_parked_resumable_closeout_without_private_transport(
        ...
        assert!(
            !transport_path.exists(),
            "parked resumable retained workers should not depend on a private stop transport"
        );
        ...
        let outcome = dispatch_prepared_orchestrator_world_request(prepared)
            .await
            .expect("dispatch parked resumable stop request");
        ...
        assert!(
            ...
            "summary should disclose the no-live-owner retained stop path: {}",
            outcome.summary
        );

        let participant_after = store
            .load_participant("ash_member")
            .expect("load retained participant after parked stop")
            .expect("retained participant after parked stop");
        assert_eq!(
            participant_after.handle.state,
            AgentRuntimeSessionState::Stopped
        );
    ...
    async fn dispatch_contract_stop_world_worker_persists_parked_resumable_closeout_when_private_stop_socket_is_stale(
        ...
        assert!(
            transport_path.exists(),
            "stale private stop socket path should remain present without a listener"
        );
        ...
        let outcome = dispatch_prepared_orchestrator_world_request(prepared)
            .await
            .expect("dispatch parked resumable stop request with stale private socket");
        ...
        assert!(
            outcome.summary.contains("detached durable closeout"),
            "stale private stop socket should route through detached durable closeout: {}",
            outcome.summary
        );

        let participant_after = store
            .load_participant("ash_member")
            .expect("load retained participant after stale-socket stop")
            .expect("retained participant after stale-socket stop");
        assert_eq!(
            participant_after.internal.termination_reason.as_deref(),
            Some("stopped")
        );
    ...
    fn detached_stop_world_worker_closeout_availability_rejects_live_active_attached_refused_transport(
        ...
        assert!(
            ...
            "live active_attached sessions must stay fail-closed on ECONNREFUSED instead of taking detached stop fallback"
        );
    }
    ...
    fn detached_stop_world_worker_closeout_availability_accepts_parked_refused_transport_when_target_runtime_truth_is_stale(
        ...
        let mut orchestrator = store
            .load_participant("orch_dispatch")
            .expect("load parked orchestrator")
            .expect("parked orchestrator");
        orchestrator.internal.uaa_session_id = None;
        store
            .persist_participant(&orchestrator)
            .expect("persist parked orchestrator without internal session id");
        ...
        let resolved = store
            ...
            .expect("resolve parked stop target with stale target ownership truth");
        let continuity = store
            .classify_hidden_owner_helper_launch_continuity("sess_dispatch", "orch_dispatch", true)
            .expect("classify parked owner continuity");
        assert!(
            matches!(
                continuity,
                crate::execution::agent_runtime::state_store::HiddenOwnerHelperLaunchContinuity::Pending
            ),
            "fixture should shut the detached-owner continuity gate so the stale target-ownership override is the only fallback path"
        );
        assert!(
            parked_retained_target_stop_transport_contradiction(&resolved),
            "target fixture must retain stale member-runtime ownership truth before refused stop transport invalidates it"
        );
        ...
        assert!(
            ...
            "parked retained stop fallback should treat ECONNREFUSED on the exact target private stop socket as authoritative proof that stale member-runtime ownership truth must be overridden"
        );
    }
    ...
    fn detached_stop_world_worker_closeout_availability_rechecks_fresh_session_truth_after_refused_transport(
        ...
        assert!(
            ...
            "stale pre-error session truth should not unlock detached stop fallback before the authoritative posture flips"
        );
        ...
        store
            .persist_orchestration_session(&session)
            .expect("persist refreshed parked session");
    ...
    fn detached_stop_world_worker_closeout_availability_recheck_stays_closed_for_missing_socket_not_found(
        ...
        assert!(
            !transport_path.exists(),
            "missing-socket regression fixture must exercise the path-absent transport seam"
        );
        ...
        let mut session = store
            ...
            .expect("load authoritative session")
            .expect("authoritative session");
        session.mark_parked_resumable("owner detached after missing private stop transport");
        store
            .persist_orchestration_session(&session)
            .expect("persist refreshed parked session");
    ...
    async fn dispatch_contract_stop_world_worker_persists_detached_recoverable_closeout_for_stale_active_attached_owner(
        ...
        assert!(
            !transport_path.exists(),
            "stale active_attached owner-loss should not need a private stop transport to persist durable closeout"
        );
        ...
        let outcome = dispatch_prepared_orchestrator_world_request(prepared)
            .await
            .expect("dispatch stale active_attached stop request");
        ...
        assert!(
            outcome.summary.contains("detached durable closeout"),
            "stale owner-loss should route through detached durable closeout: {}",
            outcome.summary
        );

        let participant_after = store
            .load_participant("ash_member")
            .expect("load retained participant after stale-owner stop")
            .expect("retained participant after stale-owner stop");
        assert_eq!(
            participant_after.handle.state,
            AgentRuntimeSessionState::Stopped
        );
    ...
    async fn dispatch_contract_stop_world_worker_prefers_private_stop_surface_for_parked_resumable_with_live_transport(
        ...
        let mut stop_transport =
            ...
            .await
            .expect("register private stop transport for parked resumable session");
        let store_for_task = store.clone();
        let stop_owner = tokio::spawn(async move {
            let request = tokio::time::timeout(Duration::from_secs(3), stop_rx.recv())
                .await
                .expect("timed out waiting for parked private stop request")
                .expect("parked private stop request");
            let mut session = store_for_task
                .load_orchestration_session("sess_dispatch")
                .expect("load orchestration session for parked stop closeout")
                .expect("authoritative orchestration session for parked stop closeout");
            let mut participant = store_for_task
                .load_participant("ash_member")
                .expect("load retained participant for parked stop closeout")
                .expect("authoritative retained participant for parked stop closeout");
            participant.transition_state(AgentRuntimeSessionState::Stopped);
            participant.mark_terminal_state("worker stopped through private parked transport");
            participant.touch_heartbeat();
            session.touch_active();
            crate::execution::agent_runtime::control::persist_runtime_snapshots(
                &store_for_task,
            ...
            .expect("persist retained parked stop closeout");
            let _ = request
                .response_tx
                .send(crate::execution::agent_runtime::control::PrivateStopOutcome::Accepted);
        });
        ...
        let outcome = dispatch_prepared_orchestrator_world_request(prepared)
            .await
            .expect("dispatch parked resumable stop request should still use private transport");
        ...
        assert!(
            outcome
                .summary
                .contains("existing private owner stop surface"),
            "parked resumable session must keep using the live private stop surface: {}",
            outcome.summary
        );
        assert!(
            !outcome.summary.contains("detached durable closeout"),
            "detached closeout must stay gated behind actual private-surface loss: {}",
            outcome.summary
        );

        let participant_after = store
            .load_participant("ash_member")
            .expect("load retained participant after parked private stop")
            .expect("retained participant after parked private stop");
        assert_eq!(
            participant_after.internal.termination_reason.as_deref(),
            Some("worker stopped through private parked transport")
        );

        stop_owner
            .await
            .expect("parked private stop owner task should join");
        stop_transport.close().await;
    }
    ...
    fn persist_detached_stop_world_worker_closeout_rejects_backend_drift() {
        ...
        let resolved = store
            ...
            .expect("resolve detached stop target before backend drift");
        let mut participant = store
            .load_participant("ash_member")
            .expect("load retained participant before backend drift")
            .expect("retained participant before backend drift");
        participant.handle.backend_id = "cli:other-world".to_string();
        store
            .persist_participant(&participant)
            .expect("persist backend drift");
        ...
        assert_eq!(
            err.to_string(),
            "backend_mismatch: orchestration session sess_dispatch retained worker ash_member backend is cli:other-world not cli:codex-world"
        );
    }
    ...
    fn persist_detached_stop_world_worker_closeout_rejects_terminal_target_race() {
        ...
        let resolved = store
            ...
            .expect("resolve detached stop target before terminal race");
        ...
        let err = persist_detached_stop_world_worker_closeout(&store, &resolved)
            .expect_err("terminal target race must fail closed");
    ...
    fn persist_detached_stop_world_worker_closeout_rejects_stale_owner_successor_drift() {
        ...
        let resolved = store
            ...
            .expect("resolve stale-owner stop target before successor drift");
        ...
        let err = persist_detached_stop_world_worker_closeout(&store, &resolved)
            .expect_err("successor drift must fail closed");

        assert_eq!(
            err.to_string(),
            "stale_linkage: orchestration session sess_dispatch retained worker ash_member is not linked to authoritative orchestrator orch_successor"
        );

        let participant_after = store
            .load_participant("ash_member")
            .expect("load retained participant after successor drift")
            .expect("retained participant after successor drift");
        assert!(
            participant_after.handle.state.is_live(),
            "fallback must not mutate retained worker state after successor drift"
        );
        assert!(
            participant_after.internal.termination_reason.is_none(),
    ...
    fn persist_detached_stop_world_worker_closeout_rejects_stale_owner_linkage_drift() {
        ...
        let resolved = store
            ...
            .expect("resolve stale-owner stop target before linkage drift");

        let mut participant = store
            .load_participant("ash_member")
            .expect("load stale-owner retained participant")
            .expect("stale-owner retained participant");
        participant.handle.orchestrator_participant_id = Some("orch_unrelated".to_string());
        store
            .persist_participant(&participant)
            .expect("persist stale-owner linkage drift");

        let err = persist_detached_stop_world_worker_closeout(&store, &resolved)
            .expect_err("linkage drift must fail closed");

        assert_eq!(
            err.to_string(),
            "stale_linkage: orchestration session sess_dispatch retained worker ash_member is not linked to authoritative orchestrator orch_dispatch"
        );

        let participant_after = store
            .load_participant("ash_member")
            .expect("load retained participant after linkage drift")
            .expect("retained participant after linkage drift");
        assert!(
            participant_after.handle.state.is_live(),
            "fallback must not mutate retained worker state after linkage drift"
        );
        assert!(
            participant_after.internal.termination_reason.is_none(),
            "fallback must not persist stop closeout after linkage drift"
        );
    }
    ...
    fn detached_stop_world_worker_closeout_availability_fails_closed_for_non_refused_transport_errors(
        ...
        let resolved = store
            ...
            .expect("resolve detached stop target");
        let transport_path = private_stop_transport_path(&store, "sess_dispatch", "ash_member");
        let transport_parent = transport_path
            .parent()
            .expect("private stop transport path parent");
        std::fs::create_dir_all(transport_parent).expect("create private stop transport parent");
        std::fs::write(&transport_path, b"stale inode")
            .expect("persist placeholder transport path");
        ...
        assert!(
            ...
            "parked resumable detached fallback must stay fail-closed for non-ECONNREFUSED transport errors"
        );
    }
    ...
    fn detached_stop_world_worker_closeout_availability_rejects_active_missing_transport_without_observed_unavailability(
        ...
        assert!(
            !transport_path.exists(),
            "active authoritative test fixture should start before private stop transport publication"
        );
        ...
        assert!(
            ...
            "active authoritative retained targets must wait for late private transport publication instead of taking detached fallback on a missing path alone"
        );
    }
    ...
    fn detached_stop_world_worker_closeout_availability_rejects_active_missing_transport_after_not_found_error(
        ...
        assert!(
            ...
            "late-publication timeout must stay fail-closed for live active_attached sessions when the private stop socket never appears"
        );
    }
    ...
    fn continue_world_worker_progress_ack_summary_stays_seen_progress_only() {
        ...
        assert!(
            failure.contains("status 17"),
            "non-zero progress_ack summary must preserve terminal status truth: {failure}"
        );
        assert!(
            failure.contains("host saw the worker's recent progress"),
            "non-zero progress_ack summary must stay limited to seen-progress acknowledgement: {failure}"
        );
        assert!(
            failure.contains("downstream completion remains worker-defined"),

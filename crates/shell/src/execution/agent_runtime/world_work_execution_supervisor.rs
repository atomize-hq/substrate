    ...
    fn restart_recovery_rotates_observer_at_exact_cursor_and_replay_converges() {
        with_supervisor(|supervisor, root_path| {
            ...
            let old = WorldWorkExecutionSupervisor::bind_for_test_observer(
                &root_path,
                &crate::execution::agent_runtime::host_session_authority::facade::HostSessionAuthority::open(&root_path)
            ...
            let root = crate::execution::agent_runtime::host_session_authority::facade::HostSessionAuthority::open(&root_path)
            ...
            let after_terminal = WorldWorkExecutionSupervisor::bind_for_test_observer(
                &root_path,
                &root.bootstrap_home,
                &root.authority_store_id,
                "observer-after-terminal",
            )
            .expect("bind post-terminal observer");
            assert!(after_terminal
                .recover_records(std::slice::from_ref(&record))
                .expect("terminal recovery is immutable")
                .is_empty());
            assert_eq!(
                after_terminal
                    .inspect_observation_by_acceptance_id(&record.acceptance_record_id)
                    .expect("inspect immutable terminal"),

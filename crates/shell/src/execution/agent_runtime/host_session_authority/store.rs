...
pub(crate) fn bind_world_work_receipt_registry_storage(
    path: &Path,
    expected_root: &CanonicalDirectoryV1,
    expected_authority_store_id: &str,
) -> Result<WorldWorkReceiptRegistryStorageV1, BootstrapError> {
...
mod platform {
    ...
    impl WorldWorkReceiptRegistryTransactionV1<'_> {
        ...
        pub(crate) fn replace_registry(&mut self, _bytes: &[u8]) -> Result<(), BootstrapError> {
            Err(BootstrapError(
                "B1 receipt-registry storage is unsupported on this platform",
            ))
        }

...
pub(crate) const WORKSPACE_DISABLED_FILENAME: &str = "workspace.disabled";
...
pub(crate) fn workspace_disabled_marker_path(workspace_root: &Path) -> PathBuf {
    workspace_root
        .join(SUBSTRATE_DIR_NAME)
        .join(WORKSPACE_DISABLED_FILENAME)
}
...
pub(crate) fn find_workspace_root(start: &Path) -> Option<PathBuf> {
    ...
    for dir in start.ancestors() {
        ...
        if marker.is_file() && !workspace_disabled_marker_path(dir).exists() {
...
pub(crate) fn find_workspace_root_any(start: &Path) -> Option<PathBuf> {
    let start = start.canonicalize().unwrap_or_else(|_| start.to_path_buf());

...
#[cfg(feature = "graph")]
mod kuzu_integration {
    use super::*;
    use kuzu::{Connection, Database};
    use std::sync::Arc;
    
    static GRAPH_DB: Lazy<RwLock<Option<Arc<Database>>>> = Lazy::new(|| RwLock::new(None));
    
    pub fn init_graph_db(path: impl AsRef<Path>) -> Result<()> {
        let db = Arc::new(Database::new(path, 0)?);
        
        let conn = Connection::new(&db)?;
        
        conn.query(
            "CREATE NODE TABLE IF NOT EXISTS spans (
        ...
        conn.query(
            "CREATE REL TABLE IF NOT EXISTS edges (
                FROM spans TO spans,
                edge_type STRING,
                metadata STRING
            )"
        ...
        *GRAPH_DB.write() = Some(db);
        Ok(())
    }
    
    pub fn ingest_span(span: &Span) -> Result<()> {
        let db = GRAPH_DB.read()
            .as_ref()
            .cloned()
            .ok_or_else(|| anyhow::anyhow!("Graph DB not initialized"))?;
        ...
        conn.query(&format!(
            ...
            span.agent_id,
            span.cmd.replace('\'', "''"),
            span.exit.unwrap_or(-1),
            span.ts.to_rfc3339()
        ))?;
        
        if let Some(parent) = &span.parent_span {
            conn.query(&format!(
                "MATCH (p:spans {{span_id: '{}'}}), (c:spans {{span_id: '{}'}})
                 CREATE (p)-[:edges {{edge_type: 'parent_child'}}]->(c)",
                parent, span.span_id
            ))?;
        }
        
        if let Some(edges) = &span.graph_edges {
            for edge in edges {
                conn.query(&format!(
                    "MATCH (f:spans {{span_id: '{}'}}), (t:spans {{span_id: '{}'}})
                     CREATE (f)-[:edges {{edge_type: '{}', metadata: '{}'}}]->(t)",
                    edge.from_span,
                    edge.to_span,
    ...
    pub fn query_graph(query: &str) -> Result<Vec<serde_json::Value>> {
        let db = GRAPH_DB.read()
            .as_ref()
            .cloned()
            .ok_or_else(|| anyhow::anyhow!("Graph DB not initialized"))?;

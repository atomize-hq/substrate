...
impl RetainedWorkerLaunchAuthorityProofV1 {
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (field, value) in [
            ...
            ("world_binding.world_id", self.world_binding.world_id.as_str()),
            ("current_policy_ref_id", self.current_policy_ref_id.as_str()),
            (
                "current_policy_revision",
                self.current_policy_revision.as_str(),
            ),
            ("retained_worker_ref_id", self.retained_worker_ref_id.as_str()),
        ] {
            validate_non_empty_request_field(
        ...
        self.registration_commitment.validate(
            "retained_worker_launch_authority.registration_commitment",
        )?;
        self.authority_record_commitment_after.validate(
            "retained_worker_launch_authority.authority_record_commitment_after",
        )?;
        self.retained_worker_commitment.validate(
            "retained_worker_launch_authority.retained_worker_commitment",
        )?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn retained_worker_launch_authority_proof_is_closed_and_strictly_validated() {
        ...
        assert!(serde_json::from_value::<RetainedWorkerLaunchAuthorityProofV1>(changed_algorithm)
            .is_err());

        let mut uppercase_digest = json.clone();
        uppercase_digest["canonical_spawn_fingerprint"]["digest_hex"] =
            serde_json::json!("A".repeat(64));
        assert!(serde_json::from_value::<RetainedWorkerLaunchAuthorityProofV1>(uppercase_digest)
            .is_err());

        let mut unknown_field = json;
        unknown_field["unexpected"] = serde_json::json!(true);
        assert!(serde_json::from_value::<RetainedWorkerLaunchAuthorityProofV1>(unknown_field)
            .is_err());
    }

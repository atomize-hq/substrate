...
impl NetNs {
    pub fn new(name: &str) -> Self {
        Self { name: name.to_string(), active: false }
    }
    ...
    pub fn ip_available() -> bool {
        Command::new("sh").arg("-lc").arg("command -v ip >/dev/null 2>&1").status().map(|s| s.success()).unwrap_or(false)
    }

    /// Create the named netns.
    pub fn add(&mut self) -> Result<bool> {
        let status = Command::new("ip").args(["netns","add", &self.name]).status();
        match status {
            Ok(s) if s.success() => { self.active = true; Ok(true) },
    ...
    pub fn lo_up(&self) -> Result<bool> {
        if !self.active { return Err(anyhow::anyhow!("netns not active")); }
        let status = Command::new("ip").args(["-n", &self.name, "link", "set", "lo", "up"]).status();
        match status {
            Ok(s) if s.success() => Ok(true),
            Ok(s) => Err(anyhow::anyhow!("ip -n {} link set lo up exited with status {}", self.name, s)),

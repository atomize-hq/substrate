...
if (-not $IsWindows) {
  Write-Host "SKIP: NASP windows smoke (not Windows)"
  exit 0
}
...
$tmpRoot = if ($env:SUBSTRATE_SMOKE_ROOT -and $env:SUBSTRATE_SMOKE_ROOT.Trim() -ne "") { $env:SUBSTRATE_SMOKE_ROOT } else { New-TempDir "nasp-smoke" }
...
"@ | Set-Content -Path (Join-Path $substrateHome "deps\\packages\\smoke-pacman-b.yaml") -NoNewline
  ...
  try {
    ...
    & $SubstrateExe world deps workspace add smoke-hello smoke-pacman-a smoke-pacman-b | Out-Null
    ...
    $null = Invoke-Substrate -Label "world doctor" -ExpectedExit 0 -Args @("world", "doctor")

    Write-Host "== Case B: runtime current sync fails early for pacman-backed items =="
    $r = Invoke-Substrate -Label "deps current sync --dry-run" -ExpectedExit 4 -Args @("world", "deps", "current", "sync", "--dry-run")
    Require-LineOrder $r.Stdout "nasp-pacman-a" "nasp-pacman-b"
    Require-Contains $r.Stderr "substrate world enable --provision-deps"
    Require-Contains $r.Stderr "unsupported on Windows"

    Write-Host "== Case C: current install explicit args do not add enabled items implicitly =="
    $null = Invoke-Substrate -Label "deps current install smoke-hello" -ExpectedExit 0 -Args @("world", "deps", "current", "install", "smoke-hello")

    Write-Host "== Case D: current install fails early for explicit pacman-backed items =="
    $r = Invoke-Substrate -Label "deps current install smoke-pacman-a --dry-run" -ExpectedExit 4 -Args @("world", "deps", "current", "install", "smoke-pacman-a", "--dry-run")
    Require-LineOrder $r.Stdout "nasp-pacman-a" "nasp-pacman-b"
    Require-Contains $r.Stderr "substrate world enable --provision-deps"
  } finally {
    Pop-Location
  ...
  Write-Host "OK: NASP windows smoke"
  exit 0
} finally {
  if (-not $keep) {

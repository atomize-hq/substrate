...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "SKIP: NASP linux smoke (uname=$(uname -s))"
  exit 0
fi
...
"$SUBSTRATE_BIN" world deps workspace add smoke-hello smoke-pacman-a smoke-pacman-b >/dev/null
...
if ! "$SUBSTRATE_BIN" world doctor >/dev/null 2>&1; then
  echo "NASP linux smoke: world backend not healthy; run 'substrate world doctor' and retry" >&2
  exit 4
fi

echo "== Case B: runtime current sync fails early for pacman-backed items =="

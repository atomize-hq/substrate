# Threading - dev-install-world-service-staging
...
- **Contract ID**: `C-01`
  ...
  - **Definition**: In the standard version-dir flow, `substrate world enable` resolves `<home>/bin/substrate`, derives the standard version dir from the canonical binary target, checks `<version_dir>/bin/world-service` before `<version_dir>/bin/linux/world-service`, and treats either accepted executable path as sufficient to continue.
  - **Versioning / compat**: the root `bin/world-service` path wins when both exist; any change to derivation, accepted path set, sufficiency rule, or override carve-out forces downstream revalidation.
...
- **Thread ID**: `THR-03`
  ...
  - **Purpose**: publish the actual staged `world-service` bridge layout, selected-profile mapping, and refresh posture that the checkpoint evidence must consume.
  - **State**: `closed`
...
flowchart LR
  ...
  S1 -->|THR-01 / C-01 + C-03| S2[SEAM-2 Linux dev-install world-service staging]
...
2. `SEAM-2` revalidates against `SEAM-1` closeout and then lands Linux `world-service` staging, selected-profile mapping, and `ln -sfn` refresh behavior.

...
- On Linux, `scripts/substrate/dev-install-substrate.sh --no-world --profile <debug|release>` stages executable links at:
  1. `target/bin/world-service`
  2. `target/bin/linux/world-service`
- Both staged links must point at `target/<profile>/world-service` for the selected profile.
- Either staged link location must satisfy the runtime preflight contract published by `SEAM-1`; when both exist, the root `target/bin/world-service` path remains the runtime-preferred path.

...
- **Scope (in/out)**:
  - In:
    - stage both accepted `world-service` links from the selected profile
...
- **Acceptance criteria**:
  - debug-profile installs stage both links to `target/debug/world-service`
  - release-profile installs stage both links to `target/release/world-service`
...
#### S2.T1 - Stage both accepted `world-service` links

...
- **User/system value**: `substrate world enable` becomes deterministic in the standard version-dir flow: it can only proceed when an accepted staged `world-service` exists, and `--dry-run` truthfully reports readiness without mutating state.
- **Scope (in/out)**:
...
- **Acceptance criteria**:
  - With override unset, preflight checks `<version_dir>/bin/world-service` then `<version_dir>/bin/linux/world-service`.
...
- **Outcome**: A single function answers: “is there an accepted staged `world-service`, and which path wins?”
- **Inputs/outputs**:
...
- **Acceptance criteria**: When both exist, selects `<version_dir>/bin/world-service`.

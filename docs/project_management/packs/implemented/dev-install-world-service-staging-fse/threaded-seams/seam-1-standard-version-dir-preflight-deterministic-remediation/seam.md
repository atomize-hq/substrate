...
- **Goal / value**: Make `substrate world enable` behave deterministically in the standard version-dir flow by either finding an accepted staged `world-service` artifact or failing early with one operator-visible remediation before helper launch, privileged work, or state mutation.

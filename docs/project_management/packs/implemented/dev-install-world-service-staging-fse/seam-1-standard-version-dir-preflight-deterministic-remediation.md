...
- **Key invariants / rules**:
  - The accepted staged path search order is fixed: `<version_dir>/bin/world-service`, then `<version_dir>/bin/linux/world-service`.
  ...
  - The missing-artifact remediation must name both accepted paths, `scripts/substrate/dev-install-substrate.sh --no-world`, and `cargo build -p world-service`.

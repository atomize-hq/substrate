---
pack_id: dev-install-world-service-staging-seam-pack
pack_version: v1
pack_status: extracted
source_ref: dev-install-world-service-staging.zip
execution_horizon:
  active_seam: null
  next_seam: null
---

# Scope Brief - dev-install-world-service-staging
...
- **In-scope**:
  ...
  - the accepted staged path set and search order: `<version_dir>/bin/world-service`, then `<version_dir>/bin/linux/world-service`
  - the sufficiency rule that either accepted staged executable path is enough to continue
  - deterministic exit `3` missing-artifact behavior with one remediation block that names both accepted paths, `scripts/substrate/dev-install-substrate.sh --no-world`, and `cargo build -p world-service`
  ...
  - Linux `dev-install-substrate.sh --no-world` staging of `world-service` into both accepted staged paths from the selected `--profile`, with `ln -sfn` refresh semantics
...
- **Success criteria**:
  ...
  - Linux `scripts/substrate/dev-install-substrate.sh --no-world --profile <debug|release>` stages both accepted `world-service` bridges to `target/<profile>/world-service`, keeps `world.enabled: false`, skips provisioning, and refreshes stale links with `ln -sfn`

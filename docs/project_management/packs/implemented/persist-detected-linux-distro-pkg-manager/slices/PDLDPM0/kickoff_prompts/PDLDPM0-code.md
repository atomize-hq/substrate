...
1. Confirm the worktree is `wt/persist-detected-linux-distro-pkg-manager-pdldpm0-code` on branch `persist-detected-linux-distro-pkg-manager-pdldpm0-code` and `.taskmeta.json` exists at the worktree root.
2. Read `docs/project_management/packs/draft/persist-detected-linux-distro-pkg-manager/plan.md`, `docs/project_management/packs/draft/persist-detected-linux-distro-pkg-manager/tasks.json`, `docs/project_management/packs/draft/persist-detected-linux-distro-pkg-manager/session_log.md`, the PDLDPM0 spec, and this prompt.
...
## Requirements
- Implement only the production changes required by the PDLDPM0 AC set.
- Keep `pkg_manager.selected` and `pkg_manager.source` as verbatim copies of the upstream detection outputs.
...
2. Run `make triad-task-finish TASK_ID="PDLDPM0-code"` from inside the worktree.

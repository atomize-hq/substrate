# dev-install-world-service-staging — contract surface
...
- `scripts/substrate/install-substrate.sh` remains authoritative for production bundle install behavior outside the accepted `world-service` path rule stated here.
...
- **Accepted staged path set**:
  1. `<version_dir>/bin/world-service`
  2. `<version_dir>/bin/linux/world-service`
...
- The script builds the selected profile and stages the `world-service` bridge from `target/<profile>/world-service` into both accepted staged paths.
...
- Re-running the script refreshes both staged world-service links with `ln -sfn`.
...
- The missing-artifact failure prints one remediation block that names both accepted paths, `scripts/substrate/dev-install-substrate.sh --no-world`, and `cargo build -p world-service`.
...
- The accepted staged path set for the standard version-dir flow is:
  1. `<version_dir>/bin/world-service`
  2. `<version_dir>/bin/linux/world-service`
- The search order is fixed. The root `bin/world-service` path wins when both paths exist.
- Linux dev-install uses the selected profile output under `target/<profile>/world-service` as the source for both staged bridge links.
...
- `3`
  - In the standard version-dir flow, neither accepted staged world-service path exists.
...
- Linux `dev-install-substrate.sh --no-world` stages `world-service` for the enable-later workflow.

...
SMOKE_NAME="dev-install-world-service-staging-linux-smoke"
...
log "Case 1: dev-install --no-world stages absolute world-service bridge targets for debug profile"
scripts/substrate/dev-install-substrate.sh --prefix "${SUBSTRATE_HOME}" --profile debug --no-world --no-shims

expected_debug_world_service="${REPO_ROOT}/target/debug/world-service"
assert_link_points_to "target/bin/world-service" "${expected_debug_world_service}"
assert_link_points_to "target/bin/linux/world-service" "${expected_debug_world_service}"
...
log "Case 2: repeated dev-install refreshes staged world-service bridge (debug -> release)"
scripts/substrate/dev-install-substrate.sh --prefix "${SUBSTRATE_HOME}" --profile release --no-world --no-shims
expected_release_world_service="${REPO_ROOT}/target/release/world-service"
assert_link_points_to "target/bin/world-service" "${expected_release_world_service}"
assert_link_points_to "target/bin/linux/world-service" "${expected_release_world_service}"
...
if [[ "${dry_run_status}" -ne 3 ]]; then
  fatal "Expected world enable --dry-run to exit 3 when staged world-service is missing (got ${dry_run_status})"
fi

assert_contains "${dry_run_output}" "target/bin/world-service"
assert_contains "${dry_run_output}" "target/bin/linux/world-service"
assert_contains "${dry_run_output}" "scripts/substrate/dev-install-substrate.sh --no-world"
assert_contains "${dry_run_output}" "cargo build -p world-service"

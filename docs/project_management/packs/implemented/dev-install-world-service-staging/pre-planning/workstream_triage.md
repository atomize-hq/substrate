# dev-install-world-service-staging — workstream triage (pre-planning)
...
- `docs/project_management/packs/draft/dev-install-world-service-staging/pre-planning/minimal_spec_draft.md`
...
- `docs/project_management/packs/draft/dev-install-world-service-staging/logs/spec-manifest/last_message.md`
- `docs/project_management/packs/draft/dev-install-world-service-staging/logs/impact-map/last_message.md`
- `docs/project_management/packs/draft/dev-install-world-service-staging/logs/min-spec-draft/last_message.md`
- `docs/project_management/packs/draft/dev-install-world-service-staging/logs/CI-checkpoint/last_message.md`

## Lift evidence (v1; advisory)

- Intake/ADR lift: `lift_score=10`, `estimated_slices=1`, `confidence=low` (missing inputs). Evidence: `docs/project_management/packs/draft/dev-install-world-service-staging/logs/workstream-triage/pm_lift_intake.txt`.
- Pack lift (impact-map-derived): `lift_score=66`, `estimated_slices=6`, `confidence=low`; triggers include `split_required:estimated_slices>3`, `likely_split:lift_score>24`, `likely_split:touch_files_sum>12`. Evidence: `docs/project_management/packs/draft/dev-install-world-service-staging/logs/workstream-triage/pm_lift_pack.txt`.
...
- Staged `world-service` path sufficiency rule (accept either; search order pinned).
...
Goal:
- Specify DIWAS0’s acceptance criteria and contracts for “fail fast when staged `world-service` is missing” (before any privileged steps).

Owns:
- `docs/project_management/packs/draft/dev-install-world-service-staging/slices/DIWAS0/DIWAS0-spec.md`
- `docs/project_management/packs/draft/dev-install-world-service-staging/slices/DIWAS0/kickoff_prompts/DIWAS0-{code,test,integ}.md`
...
### DIWAS-PWS-slice_spec_diwas1 — DIWAS1 slice spec (dev-install `--no-world` stages `world-service`)
...
- `docs/project_management/packs/draft/dev-install-world-service-staging/slices/DIWAS1/kickoff_prompts/DIWAS1-{code,test,integ}.md`
...
- `docs/project_management/packs/draft/dev-install-world-service-staging/tasks.json` (populate `tasks`; keep `meta.checkpoint_boundaries=["DIWAS1"]` aligned to checkpoint plan)
...
- High-churn seams (must be decided early; otherwise slice specs churn):
  - Staged artifact path set (`bin/world-service` vs `bin/linux/world-service`, BOTH vs ONE).

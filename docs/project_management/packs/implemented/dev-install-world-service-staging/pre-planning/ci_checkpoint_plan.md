# dev-install-world-service-staging — CI checkpoint plan (pre-planning)

This file defines **when** cross-platform CI gates run for this feature.

The canonical checkpoint-plan path for this pack is `docs/project_management/packs/draft/dev-install-world-service-staging/pre-planning/ci_checkpoint_plan.md`; the extracted `-fse` seam docs are planning inputs only.
...
- `docs/project_management/packs/draft/dev-install-world-service-staging/pre-planning/minimal_spec_draft.md` (draft slice skeleton + invariants)
- Slice specs: see `docs/project_management/packs/draft/dev-install-world-service-staging/pre-planning/spec_manifest.md`.
...
- **Contract completion seam:** the user-visible “dev install with `--no-world`, enable later” workflow is only coherent once BOTH are true:
  - `DIWAS1` stages `world-service` into the contract-defined version-dir layout, and
...
- Missing-`world-service` enable preflight behavior (ordering + exit code + remediation; contract-defined).
...
- Path canonicalization:
  - The canonical checkpoint-plan path is `docs/project_management/packs/draft/dev-install-world-service-staging/pre-planning/ci_checkpoint_plan.md`.
...
- Mechanical validity (when slice tasks exist in `tasks.json`):
  ...
  - Run (must pass): `python3 docs/project_management/system/scripts/planning/validate_ci_checkpoint_plan.py --feature-dir "docs/project_management/packs/draft/dev-install-world-service-staging"`.

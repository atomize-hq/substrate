...
- Dev-install staging and enable preflight MUST agree on the staged `world-service` path set; `contract.md` MUST pin the exact rule.

## Failure posture + invariants

### Fail-closed posture (missing staged artifact)
- On Linux, if the staged `world-service` artifact is missing, `substrate world enable` MUST fail before any privileged/systemd/socket/readiness actions.
...
- This feature requires no new exit codes; `contract.md` MUST map all externally visible failures (including missing staged `world-service` and unsupported platforms) to taxonomy codes deterministically.
...
- Preflight orthogonality: missing-`world-service` preflight MUST remain independent of any `world enable` dependency-provisioning flags so overlapping queued work can evolve without breaking the remediation contract.
...
- Decision Register requirements:
  ...
  - DR-0003: pin `world-service` staging profile mapping (release-only vs match `dev-install --profile`).
  - DR-0004: pin overwrite/idempotency rules for staged `bin/(linux/)world-service` paths.
- Contract determinism:
  - Pin the exact staged artifact path rule (require BOTH `bin/world-service` and `bin/linux/world-service`, or accept ONE).
...
- slice_id: DIWAS0
  - name: Fail fast on missing staged `world-service`
...
- slice_id: DIWAS1
  - name: Stage `world-service` during dev-install `--no-world`

# dev-install-world-service-staging — decision register
...
- Decision statement:
  - choose the implementation locus for the operator-visible missing-`world-service` preflight in the standard version-dir flow
- Option A:
...
- Option A:
  - `--no-world` skips provisioning only and still stages `world-service`
- Option B:
...
- Tradeoff:
  - Linux dev-install keeps the `world-service` build cost even when the world remains disabled after install.
- Impacted surfaces:
  - `contract.md`
...
## DR-0003 — Profile mapping for staged `world-service`
- Decision statement:
  - choose whether staging always targets `release` or mirrors the selected dev-install profile
- Option A:
  - stage `world-service` from the selected dev-install profile
- Option B:
  - stage `world-service` from `release` regardless of the dev-install profile
- Selected option:
  - Option A
- Why:
...
- Decision statement:
  - choose whether repeated dev installs refresh staged `world-service` links or keep any existing bridge until it is absent
- Option A:
...
- Option A:
  - accept either path as sufficient; search order remains `bin/world-service`, then `bin/linux/world-service`
- Option B:
  - require BOTH `bin/world-service` and `bin/linux/world-service` to exist before provisioning
- Selected option:
  - Option A
- Why:

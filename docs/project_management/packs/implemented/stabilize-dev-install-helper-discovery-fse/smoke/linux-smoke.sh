...
SMOKE_NAME="stabilize-dev-install-helper-discovery-linux-smoke"
...
log "Linux smoke is authoritative for the closeout-backed helper-order and cleanup/refusal contract."
log "This proof surface consumes C-01 through C-04 without widening platform scope."

run_step \
  "world_enable_rejects_prefix_flag" \
  cargo test -p shell world_enable_rejects_prefix_flag -- --exact --nocapture
...
run_step \
  "world_enable_reports_staged_prefix_guidance_when_helper_missing" \
  cargo test -p shell world_enable_reports_staged_prefix_guidance_when_helper_missing -- --exact --nocapture

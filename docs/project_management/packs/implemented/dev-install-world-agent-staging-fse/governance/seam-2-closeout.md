...
- **Source artifact**: `../threaded-seams/seam-2-linux-dev-install-world-agent-staging/slice-3-seam-exit-gate.md`
- **Landed evidence**:
...
- **Review-surface delta**: the Linux dev-install path now stages the accepted `world-agent` bridge layout for `debug` and `release`, while `scripts/substrate/install-substrate.sh` remains reference-only for this seam.

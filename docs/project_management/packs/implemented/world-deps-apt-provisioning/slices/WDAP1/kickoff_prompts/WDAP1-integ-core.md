# Kickoff: WDAP1-integ-core (integration core)
...
1. Verify you are in the task worktree `wt/world-deps-apt-provisioning-wdap1-integ-core` on branch `world-deps-apt-provisioning-wdap1-integ-core` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" TASK_ID="WDAP1-integ-core"`
...
- This slice is the end of the checkpoint group for CP2; finish this task, then run `CP2-ci-checkpoint` from the orchestration checkout.
...
1. Ensure merged state is committed and local integration gates are green:
   - From inside the worktree, run: `make triad-task-finish TASK_ID="WDAP1-integ-core"`

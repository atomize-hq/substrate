# Kickoff: WDAP0-integ (integration final — cross-platform merge)
...
1. Verify you are in the task worktree `wt/world-deps-apt-provisioning-wdap0-integ` on branch `world-deps-apt-provisioning-wdap0-integ` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" TASK_ID="WDAP0-integ"`

## Requirements
- Merge the relevant integration branches for this slice:
  - core integration branch: `WDAP0-integ-core`
  - platform-fix branches: `WDAP0-integ-linux`, `WDAP0-integ-macos`, `WDAP0-integ-windows` (when they produced commits)
- Run:
  - `cargo fmt`
...
- Verify `CP1-ci-checkpoint` is completed and that run ids/URLs are recorded in `docs/project_management/packs/draft/world-deps-apt-provisioning/session_log.md`.

## End Checklist
1. Ensure merged state is committed and local integration gates are green:
   - From inside the worktree, run: `make triad-task-finish TASK_ID="WDAP0-integ"`
...
Naming note:
- The task id for the final aggregator is `WDAP0-integ`.

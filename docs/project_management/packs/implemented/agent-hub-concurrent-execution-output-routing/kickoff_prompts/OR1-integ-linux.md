...
- Run advisory audit and record outputs:
  - `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/packs/active/agent-hub-concurrent-execution-output-routing/logs/OR1/ci-audit/ledger.jsonl" --kind feature-smoke --orch-branch "feat/agent-hub-concurrent-execution-output-routing" --required-platforms linux`
- Dispatch linux smoke via CI and record the run id/URL in session_log.md:
  - `make feature-smoke FEATURE_DIR="docs/project_management/packs/active/agent-hub-concurrent-execution-output-routing" PLATFORM=linux SMOKE_SLICE_ID="OR1" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/agent-hub-concurrent-execution-output-routing" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=1`

...
1. Verify you are in the task worktree `wt/world-disabled-reason-attribution-wdra0-test` on branch `world-disabled-reason-attribution-wdra0-test` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   ...
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/world-disabled-reason-attribution" TASK_ID="WDRA0-test"`
...
- Targeted test focus:
  - helper precedence and redaction coverage inside `crates/shell/tests/replay_world.rs`

## End Checklist
1. Run targeted tests and capture outputs.
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WDRA0-test"`

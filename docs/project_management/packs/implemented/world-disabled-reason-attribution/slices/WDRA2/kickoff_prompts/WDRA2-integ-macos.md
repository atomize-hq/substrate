# Kickoff: WDRA2-integ-macos (integration platform-fix — macOS)

## Scope
- Ensure WDRA2 is green for macOS.
...
1. Run this task on a machine that matches macOS.
2. Verify you are in the task worktree `wt/world-disabled-reason-attribution-wdra2-integ-macos` on branch `world-disabled-reason-attribution-wdra2-integ-macos` and that `.taskmeta.json` exists at the worktree root.
...
4. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/world-disabled-reason-attribution" TASK_ID="WDRA2-integ-macos" TASK_PLATFORM=macos`
...
- Run:
  ...
  - bash "docs/project_management/packs/draft/world-disabled-reason-attribution/smoke/macos-smoke.sh"
...
## End Checklist
1. Ensure the required gate is green for macOS and capture the run id or local evidence.
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WDRA2-integ-macos"`

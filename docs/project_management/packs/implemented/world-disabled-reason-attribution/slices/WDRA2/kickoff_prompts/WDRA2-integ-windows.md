# Kickoff: WDRA2-integ-windows (integration platform-fix — Windows)

## Scope
- Ensure WDRA2 is green for Windows.
...
1. Run this task on a machine that matches Windows.
2. Verify you are in the task worktree `wt/world-disabled-reason-attribution-wdra2-integ-windows` on branch `world-disabled-reason-attribution-wdra2-integ-windows` and that `.taskmeta.json` exists at the worktree root.
...
4. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/world-disabled-reason-attribution" TASK_ID="WDRA2-integ-windows" TASK_PLATFORM=windows`
...
- Run:
  ...
  - pwsh -File "docs/project_management/packs/draft/world-disabled-reason-attribution\smoke\windows-smoke.ps1"
...
## End Checklist
1. Ensure the required gate is green for Windows and capture the run id or local evidence.
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WDRA2-integ-windows"`

# Kickoff: WDD2-integ-core (integration core)
...
1. Verify you are in the task worktree `wt/world-disabled-diagnostics-wdd2-integ-core` on branch `world-disabled-diagnostics-wdd2-integ-core` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/world-disabled-diagnostics" TASK_ID="WDD2-integ-core"`
...
If `docs/project_management/packs/draft/world-disabled-diagnostics/smoke/` exists and this machine matches a behavior platform for this feature, run the matching smoke script locally before finishing this task.

Determine behavior platforms:
- `jq -r '.meta.behavior_platforms_required // [] | join(\",\")' \"docs/project_management/packs/draft/world-disabled-diagnostics/tasks.json\"`
...
bash "docs/project_management/packs/draft/world-disabled-diagnostics/smoke/linux-smoke.sh"
```
...
cargo build --bin substrate
export PATH="$PWD/target/debug:$PATH"
bash "docs/project_management/packs/draft/world-disabled-diagnostics/smoke/macos-smoke.sh"
```
...
pwsh -File "docs/project_management/packs/draft/world-disabled-diagnostics\\smoke\\windows-smoke.ps1"
```
...
1. Ensure your merged state is committed and local integration gates are green:
   - From inside the worktree, run: `make triad-task-finish TASK_ID="WDD2-integ-core"`

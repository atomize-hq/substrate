...
1. Verify you are in the task worktree `wt/world-sync-ws7-code` on branch `world-sync-ws7-code` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start-pair FEATURE_DIR="docs/project_management/packs/active/world-sync" SLICE_ID="WS7"` (preferred; starts code+test in parallel; `WS7` is `WS7-code` without `-code`)
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/active/world-sync" TASK_ID="WS7-code"` (single task only)
...
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WS7-code"`

...
4. Determine the exact commit that this checkpoint validates:
   ...
   - Compute `CHECKOUT_SHA` from `tasks.json` without checking out the branch:
     - `CORE_BRANCH="$(jq -r --arg id "WS5-integ-core" '.tasks[] | select(.id==$id) | .git_branch' "docs/project_management/packs/active/world-sync/tasks.json")"`
...
- Ledger path (not committed): `docs/project_management/packs/active/world-sync/logs/WS5/ci-audit/ledger.jsonl`
- CI Testing audit:
...
- Feature Smoke audit:
- `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/packs/active/world-sync/logs/WS5/ci-audit/ledger.jsonl" --kind feature-smoke --orch-branch "feat/world-sync" --feature-dir "docs/project_management/packs/active/world-sync"`
...
- `make feature-smoke FEATURE_DIR="docs/project_management/packs/active/world-sync" PLATFORM=behavior SMOKE_SLICE_ID="WS5" SMOKE_CHECKOUT_REF="$CHECKOUT_SHA" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/world-sync" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=0`
...
- Single multi-platform smoke run (`PLATFORM=behavior`):
  ...
  - `make triad-task-start-platform-fixes-from-smoke FEATURE_DIR="docs/project_management/packs/active/world-sync" SLICE_ID="WS5" SMOKE_RUN_ID="$SMOKE_RUN_ID" LAUNCH_CODEX=1`
- Per-platform smoke runs:
  - `make triad-task-start-platform-fixes FEATURE_DIR="docs/project_management/packs/active/world-sync" SLICE_ID="WS5" PLATFORMS="linux,macos" LAUNCH_CODEX=1`

...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start-pair FEATURE_DIR="docs/project_management/packs/active/world-sync" SLICE_ID="WS4"` (preferred; starts code+test in parallel; `WS4` is `WS4-test` without `-test`)
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/active/world-sync" TASK_ID="WS4-test"` (single task only)

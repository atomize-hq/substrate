...
candidate_subslices: []
---
### S3 - world-service consumes Snapshot V3 `net_allowed` and `WorldSpec` (remove broker coupling)
...
- **Acceptance criteria**:
  - world-service forwards `isolate_network` and `allowed_domains` to the world backend consistently for all execute paths.
  - Snapshot V3 `net_allowed` is the sole source of truth for allowlists inside the world-service request pipeline.
...
- **Acceptance criteria**:
  - No broker-derived allowlist lookups remain on the world-service execution path.

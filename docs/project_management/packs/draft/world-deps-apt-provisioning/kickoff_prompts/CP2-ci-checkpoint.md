...
- Feature Smoke audit (per authoritative platform gate):
  - Linux:
  ...
  - macOS:
    - `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/packs/draft/world-deps-apt-provisioning/logs/WDAP1/ci-audit/ledger.jsonl" --kind feature-smoke --orch-branch "feat/world-deps-apt-provisioning" --required-platforms macos`
  - Windows:
    - `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/packs/draft/world-deps-apt-provisioning/logs/WDAP1/ci-audit/ledger.jsonl" --kind feature-smoke --orch-branch "feat/world-deps-apt-provisioning" --required-platforms windows`
...
2) Linux behavioral smoke (authoritative Linux gate; GitHub-hosted Ubuntu):
- `make feature-smoke FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" PLATFORM=linux SMOKE_SLICE_ID="WDAP1" SMOKE_CHECKOUT_REF="$CHECKOUT_SHA" RUNNER_KIND=github-hosted WORKFLOW_REF="feat/world-deps-apt-provisioning" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=0`

3) macOS behavioral smoke (self-hosted):
- `make feature-smoke FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" PLATFORM=macos SMOKE_SLICE_ID="WDAP1" SMOKE_CHECKOUT_REF="$CHECKOUT_SHA" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/world-deps-apt-provisioning" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=0`

4) Windows behavioral smoke (self-hosted):
- `make feature-smoke FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" PLATFORM=windows SMOKE_SLICE_ID="WDAP1" SMOKE_CHECKOUT_REF="$CHECKOUT_SHA" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/world-deps-apt-provisioning" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=0`

Notes:
- `SMOKE_SLICE_ID` is optional but recommended; the workflow exports `SUBSTRATE_SMOKE_SLICE_ID` for slice-scoped smoke scripts.
- For `WDAP1`, do not treat a self-hosted Linux smoke run as the authoritative Linux gate.

## If smoke fails

Start only failing platform-fix tasks (from orchestration checkout):
- CP2 dispatches smoke per platform for this slice:
  - `make triad-task-start-platform-fixes FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" SLICE_ID="WDAP1" PLATFORMS="<csv failing platforms>" LAUNCH_CODEX=1`

## End Checklist

1. Record run ids/URLs (compile parity + linux/macos/windows smoke, and any CI Testing runs) in `docs/project_management/packs/draft/world-deps-apt-provisioning/session_log.md`.

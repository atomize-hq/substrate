...
3. Confirm which slice id this checkpoint is validating (per `ci_checkpoint_plan.md`).
   - For this pack, CP1 validates the checkpoint-boundary slice `WDAP0`.
   - Schema v4+ note: confirm `WDAP0` is listed in `tasks.json` `meta.checkpoint_boundaries`.
4. Determine the exact commit that this checkpoint validates:
   - This checkpoint validates the core integration branch for the checkpoint slice (`WDAP0-integ-core`).
   - Compute `CHECKOUT_SHA` from `tasks.json` without checking out the branch:
     - `CORE_BRANCH="$(jq -r --arg id \"WDAP0-integ-core\" '.tasks[] | select(.id==$id) | .git_branch' \"docs/project_management/packs/draft/world-deps-apt-provisioning/tasks.json\")"`
...
- Ledger path (not committed): `docs/project_management/packs/draft/world-deps-apt-provisioning/logs/WDAP0/ci-audit/ledger.jsonl`
- CI Testing audit:
  - `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/packs/draft/world-deps-apt-provisioning/logs/WDAP0/ci-audit/ledger.jsonl" --kind ci-testing --orch-branch "feat/world-deps-apt-provisioning"`
- Feature Smoke audit:
  - `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/packs/draft/world-deps-apt-provisioning/logs/WDAP0/ci-audit/ledger.jsonl" --kind feature-smoke --orch-branch "feat/world-deps-apt-provisioning" --feature-dir "docs/project_management/packs/draft/world-deps-apt-provisioning"`
...
- `make feature-smoke FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" PLATFORM=behavior SMOKE_SLICE_ID="WDAP0" SMOKE_CHECKOUT_REF="$CHECKOUT_SHA" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/world-deps-apt-provisioning" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=0`
...
- Single multi-platform smoke run (`PLATFORM=behavior`):
  - `make triad-task-start-platform-fixes-from-smoke FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" SLICE_ID="WDAP0" SMOKE_RUN_ID="<run-id>" LAUNCH_CODEX=1`
- Per-platform smoke runs:
  - `make triad-task-start-platform-fixes FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" SLICE_ID="WDAP0" PLATFORMS="<csv>" LAUNCH_CODEX=1`

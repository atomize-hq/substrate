# Kickoff: WDAP1-integ-windows (integration platform-fix — windows)
...
2. Verify you are in the task worktree `wt/world-deps-apt-provisioning-wdap1-integ-windows` on branch `world-deps-apt-provisioning-wdap1-integ-windows` and that `.taskmeta.json` exists at the worktree root.
...
4. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" TASK_ID="WDAP1-integ-windows" TASK_PLATFORM=windows`
...
- Feature Smoke (repeat after fixes):
  - `make feature-smoke FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" PLATFORM=windows SMOKE_SLICE_ID="WDAP1" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/world-deps-apt-provisioning" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=1`

## End Checklist
1. Ensure the required gate is green for windows and capture the run id/URL.
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WDAP1-integ-windows"`

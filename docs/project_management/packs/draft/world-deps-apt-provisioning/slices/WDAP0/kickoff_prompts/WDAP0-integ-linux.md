# Kickoff: WDAP0-integ-linux (integration platform-fix — linux)
...
2. Verify you are in the task worktree `wt/world-deps-apt-provisioning-wdap0-integ-linux` on branch `world-deps-apt-provisioning-wdap0-integ-linux` and that `.taskmeta.json` exists at the worktree root.
...
4. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" TASK_ID="WDAP0-integ-linux" TASK_PLATFORM=linux`
...
- Advisory CI audit (ledger not committed):
  ...
  - Feature Smoke audit:
    - `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/packs/draft/world-deps-apt-provisioning/logs/WDAP0/ci-audit/ledger.jsonl" --kind feature-smoke --orch-branch "feat/world-deps-apt-provisioning" --required-platforms linux`
...
- Feature Smoke (repeat after fixes):
  - `make feature-smoke FEATURE_DIR="docs/project_management/packs/draft/world-deps-apt-provisioning" PLATFORM=linux SMOKE_SLICE_ID="WDAP0" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/world-deps-apt-provisioning" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=1`

## End Checklist
1. Ensure the required gate is green for linux and capture the run id/URL.
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WDAP0-integ-linux"`

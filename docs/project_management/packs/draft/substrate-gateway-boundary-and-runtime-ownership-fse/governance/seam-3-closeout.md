...
basis:
  ...
  required_threads:
    ...
    - THR-04
  stale_triggers:
    - revalidate `SEAM-4` if typed world-agent lifecycle/status endpoint ownership or request/response shape changes
...
- **Landed evidence**:
  - `8c0bd439` - completed S1 typed runtime boundary across shared API types, shared client helpers, world-agent gateway routes, and shell gateway consumption/tests
  ...
  - `docs/WORLD.md`
  - Validation commands:
    - `cargo test -p agent-api-client -- --nocapture`
  ...
  - Validation readback result:
    - `cargo test -p agent-api-client -- --nocapture` passed `13/13` tests plus `0` doc tests.
    - `cargo test -p world-agent --test gateway_runtime_parity -- --nocapture` completed successfully and the `3/3` target-local route-shape tests passed; the runtime-dependent service cases self-skipped on this host after `WorldAgentService::new()` reported Linux/VM-only support.
...
- **Downstream stale triggers raised**:
  - typed world-agent lifecycle/status endpoint ownership or request/response shape changes
...
- **Remediation disposition**: `REM-001` resolved; the contract baseline, runtime implementation, and parity evidence all landed, and the targeted verification reruns passed on the current tree with the `world-agent` host-local runtime cases explicitly self-skipping outside Linux/VM support.

...
## Scope
- Run the checkpoint gate for the group ending at `AHCSITC2`.
...
3. Confirm the checkpoint slice is `AHCSITC2` and the checkpoint task depends on `AHCSITC2-integ-core`.
4. Compute the checkpoint checkout SHA from the `AHCSITC2-integ-core` branch before dispatching CI.
...
2. Mark `CP1-ci-checkpoint` completed in `tasks.json` and add the END entry to `session_log.md`.

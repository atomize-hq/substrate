...
1. Verify you are in `wt/agent-hub-core-successor-identity-tuple-compatible-ahcsitc1-code` on branch `agent-hub-core-successor-identity-tuple-compatible-ahcsitc1-code` and `.taskmeta.json` exists.
...
## Requirements
- Implement exactly the behaviors required by `AC-AHCSITC1-01` through `AC-AHCSITC1-06`.
- Keep the slice boundary on the session protocol and placement surfaces owned by `AHCSITC1`.
...
2. From inside the worktree, run `make triad-task-finish TASK_ID="AHCSITC1-code"`.

...
1. Verify you are in `wt/agent-hub-core-successor-identity-tuple-compatible-ahcsitc2-code` on branch `agent-hub-core-successor-identity-tuple-compatible-ahcsitc2-code` and `.taskmeta.json` exists.
...
## Requirements
- Implement exactly the behaviors required by `AC-AHCSITC2-01` through `AC-AHCSITC2-06`.
- Keep the slice boundary on the fail-closed routing and telemetry publication surfaces owned by `AHCSITC2`.
...
2. From inside the worktree, run `make triad-task-finish TASK_ID="AHCSITC2-code"`.

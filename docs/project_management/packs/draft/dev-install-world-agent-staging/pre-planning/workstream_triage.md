...
- Full planning should explicitly reconcile the lift “split” triggers with the 2-slice skeleton (either justify the 2-slice shape or split the ADR into multiple items).
...
- Staged `world-agent` path rule: require BOTH `bin/world-agent` + `bin/linux/world-agent` vs accept ONE.
...
- DR-0001..DR-0004 (implementation locus; meaning of `--no-world`; profile mapping; overwrite/idempotency).
...
- Lift split trigger:
  - Pack lift flags `estimated_slices>3`; planning should either justify 2 slices explicitly or split ADR scope into multiple items.

# dev-install-world-agent-staging — impact map (pre-planning)
...
- `docs/project_management/packs/draft/dev-install-world-agent-staging/slices/DIWAS0/kickoff_prompts/DIWAS0-code.md`
- `docs/project_management/packs/draft/dev-install-world-agent-staging/slices/DIWAS0/kickoff_prompts/DIWAS0-integ.md`
- `docs/project_management/packs/draft/dev-install-world-agent-staging/slices/DIWAS0/kickoff_prompts/DIWAS0-test.md`
- `docs/project_management/packs/draft/dev-install-world-agent-staging/slices/DIWAS1/DIWAS1-spec.md`
- `docs/project_management/packs/draft/dev-install-world-agent-staging/slices/DIWAS1/kickoff_prompts/DIWAS1-code.md`
- `docs/project_management/packs/draft/dev-install-world-agent-staging/slices/DIWAS1/kickoff_prompts/DIWAS1-integ.md`
- `docs/project_management/packs/draft/dev-install-world-agent-staging/slices/DIWAS1/kickoff_prompts/DIWAS1-test.md`
...
### CLI / UX
- Change: Linux `scripts/substrate/dev-install-substrate.sh --no-world` becomes “enable later ready” by still building/staging `world-agent` (no provisioning).
  - Direct impact:
    - The “install with `--no-world`, enable later” dev workflow stops requiring a manual `cargo build -p world-agent ...` step.
    - Dev install runtime increases on Linux when `--no-world` is used (adds a `world-agent` build step).
  - Cascading impact:
...
- Change: `substrate world enable` fails early on Linux when the staged `world-agent` artifact is missing, with a single actionable remediation.
  - Direct impact:
...
- Change: Make the staged artifact path set under the inferred dev “version dir” (`<repo>/target/`) a contract-bearing surface for enable.
  ...
  - Cascading impact:
    - The contract must pin:
      - the authoritative staged path set (`bin/world-agent`, `bin/linux/world-agent`),
...
- Change: Fail-closed preflight prevents privileged side effects when the staged `world-agent` artifact is missing.
  - Direct impact:
...
- ADR: `docs/project_management/adrs/draft/ADR-0034-staging-beaver.md`
  ...
  - Resolution (explicit):
    - Keep contracts non-overlapping: ADR-0034 owns helper discovery robustness across `cargo clean`; ADR-0035 owns staging/proving the `world-agent` artifact exists for enable provisioning.
    - Sequence to reduce merge conflict risk: land ADR-0034 dev-installer refactors first (helper staging), then land ADR-0035 staging changes (world-agent build/stage) as a smaller follow-on edit.
...
- ADR: `docs/project_management/adrs/draft/ADR-0030-provisioning-otter.md`
  ...
  - Resolution (explicit):
    - Keep missing-`world-agent` preflight independent of any `--provision-deps` flag work so provisioning UX can evolve without breaking the “artifact missing” remediation contract.
...
- Planning Pack: `docs/project_management/packs/draft/stabilize-dev-install-helper-discovery/`
  ...
  - Resolution (explicit):
    - Sequence boundary: land helper discovery/staging first, then land world-agent staging. Share/align overwrite + ownership guard decisions to avoid divergent dev-install contracts.
...
- Decision Register entries required:
  - DR-0001 — Decide where missing-`world-agent` preflight is implemented (Rust runner vs helper script vs installer helper), given default helper output suppression and the “fail before privileged steps” invariant.
  ...
  - DR-0003 — Define profile mapping for staging `world-agent` (`release` only vs match `dev-install --profile`), and reconcile any UX/defaults drift.
  - DR-0004 — Define overwrite/idempotency rules for staged `bin/(linux/)world-agent` paths under `<repo>/target/bin/`.

...
## Case 1 — `--no-world` stages `world-agent` for the selected profile
- Command:
  - `scripts/substrate/dev-install-substrate.sh --prefix "$SUBSTRATE_HOME" --profile debug --no-world`
- Verify:
  - `test -x target/bin/world-agent`
  - `test -x target/bin/linux/world-agent`
...
- Verify:
  - `echo "$status"`
  - stderr names `target/bin/world-agent`
  - stderr names `target/bin/linux/world-agent`
  - stderr names `scripts/substrate/dev-install-substrate.sh --no-world`
  - stderr names `cargo build -p world-agent`
- Expected result:
  - exit code is `3`

...
- Boundary rationale: isolate the highest-churn seams into separate planning lanes for contract, machine schema, policy, typed world-agent transport plus parity, and final docs-validation alignment.
...
```json
{
  ...
  "pws": [
    ...
    {
      "id": "SGBRO-PWS-world_agent_profile",
      "role": "implementation",
      "depends_on": [
        "SGBRO-PWS-contract",
      ...
      "assumes": [
        "Impact-map option A stays selected: Substrate exposes a typed world-agent lifecycle and status surface instead of shell-side exec probing."
      ],
      "owns": [
...
### SGBRO-PWS-world_agent_profile — typed runtime and parity seam
- Goal: lock the typed world-agent lifecycle/status path and the Linux/macOS/Windows guarantees that the CLI and status schema consume.
- Owned surfaces: `platform-parity-spec.md`.
- Dependencies: `SGBRO-PWS-contract`, `SGBRO-PWS-schema_inventory`.
- Proposed slices/triads: `SGBRO3-{code,test,integ}` for world-agent transport, shell integration, and parity evidence.
...
- Dependencies: `SGBRO-PWS-contract`, `SGBRO-PWS-schema_inventory`, `SGBRO-PWS-world_agent_profile`.
...
- Dependencies: `SGBRO-PWS-contract`, `SGBRO-PWS-schema_inventory`, `SGBRO-PWS-world_agent_profile`, `SGBRO-PWS-docs_validation`, `SGBRO-PWS-implementation_seams`.
...
- Hard ordering: `SGBRO-PWS-world_agent_profile` lands after `SGBRO-PWS-contract` and `SGBRO-PWS-schema_inventory`.
...
- Gate B: lock the `status --json` envelope and policy decision flow before any world-agent endpoint or shell status task references them.
- Gate C: lock typed world-agent transport and parity guarantees before final docs-validation or task-graph wiring.
...
- High-churn seam: `gateway-status-schema-spec.md`, `crates/agent-api-types/src/lib.rs`, `crates/agent-api-client/src/lib.rs`, and `crates/world-agent/src/handlers.rs` all need the same status envelope and `client_wiring.*` semantics.
- High-churn seam: `policy-spec.md`, `contract.md`, and world-placement behavior in `crates/shell` and `crates/world-agent` need one fail-closed decision taxonomy for exit codes `2`, `3`, `4`, and `5`.
...
- `SGBRO3`: typed world-agent lifecycle/status transport, shell integration seam, and Linux/macOS/Windows parity guarantees.

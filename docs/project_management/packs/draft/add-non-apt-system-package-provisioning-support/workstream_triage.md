...
### WS-A — Contract + decision register (operator-facing truth)
...
- Dependencies: must reconcile with WS-F (cross-pack contract/schema) before finalizing runtime behavior language.
...
### WS-B — Pack backbone (plan + tasks + checkpoints wiring)
...
- Dependencies: depends on slice skeleton update (see “Slice skeleton recommendations”) and WS-A for deterministic validation commands.
...
### WS-C — ANS0: in-world OS/manager probe determinism
...
- Dependencies: WS-A must pin precedence rules (`/etc/os-release` vs manager presence) and ambiguity handling.
- Proposed slices/triads: `ANS0-code`, `ANS0-test` (classification edge cases), `ANS0-integ` (probe result drives manager selection).

### WS-D — Schema + inventory validation + derivation (candidate split from ANS1)
...
- Dependencies: WS-F (cross-pack schema/contract authority) and WS-A (mismatch policy + runtime/provisioning invariants).
- Proposed slices/triads: `ANS1-code` (schema/validation/derivation), `ANS1-test`, `ANS1-integ` (enabled-set → derived requirement sets).

### WS-E — Provisioning-time execution wiring (candidate split from ANS1)
...
- Dependencies: WS-C (probe semantics), WS-D (schema + derivation), WS-A (exit codes + output invariants).
- Proposed slices/triads: `ANS2-code`, `ANS2-test`, `ANS2-integ` (unsupported backend behavior + deterministic remediation).

### WS-F — Runtime fail-early + health guidance (candidate split from ANS2)
...
- Dependencies: WS-A must pin fail-early scope (enabled set vs explicit args) and remediation invariants.
- Proposed slices/triads: `ANS3-code`, `ANS3-test`, `ANS3-integ` (mixed enabled sets; ensure no dead-end workflows).

### WS-G — Operator docs + scripts + smoke/manual validation (candidate split from ANS2)
...
- Dependencies: WS-A contract wording and WS-E/WS-F behavior must be stable enough to document.

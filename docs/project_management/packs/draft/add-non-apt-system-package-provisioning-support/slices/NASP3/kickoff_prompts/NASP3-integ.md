# Kickoff: NASP3-integ (integration)
...
1. Verify worktree `wt/add-non-apt-system-package-provisioning-support-nasp3-integ` on branch `add-non-apt-system-package-provisioning-support-nasp3-integ`.
2. Read `plan.md`, `tasks.json`, `session_log.md`, the slice spec, and this prompt.
3. If `.taskmeta.json` is missing, ask the operator to run `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/add-non-apt-system-package-provisioning-support" TASK_ID="NASP3-integ"`.

## Requirements
- Merge `NASP3-code` and `NASP3-test`.
...
## End Checklist
1. Run `make triad-task-finish TASK_ID="NASP3-integ"` inside the worktree.

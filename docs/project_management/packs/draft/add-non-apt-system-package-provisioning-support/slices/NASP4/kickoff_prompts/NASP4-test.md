...
1. Verify worktree `wt/add-non-apt-system-package-provisioning-support-nasp4-test` on branch `add-non-apt-system-package-provisioning-support-nasp4-test`.
...
## Requirements
- Add or update tests that enforce the NASP4 acceptance criteria.
- Run `cargo fmt` and the targeted tests you add or touch.

## End Checklist
1. Run `make triad-task-finish TASK_ID="NASP4-test"` inside the worktree.

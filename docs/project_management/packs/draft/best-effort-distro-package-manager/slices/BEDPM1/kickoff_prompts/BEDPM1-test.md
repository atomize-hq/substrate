...
1. Confirm the worktree is `wt/best-effort-distro-package-manager-bedpm1-test` on branch `best-effort-distro-package-manager-bedpm1-test` and `.taskmeta.json` exists at the worktree root.
...
## Requirements
- Add or update tests that enforce the BEDPM1 AC set.
...
2. Run `make triad-task-finish TASK_ID="BEDPM1-test"` from inside the worktree.

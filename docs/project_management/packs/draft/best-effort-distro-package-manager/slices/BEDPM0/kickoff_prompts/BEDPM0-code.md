...
1. Confirm the worktree is `wt/best-effort-distro-package-manager-bedpm0-code` on branch `best-effort-distro-package-manager-bedpm0-code` and `.taskmeta.json` exists at the worktree root.
...
## Requirements
- Implement only the production changes required by the BEDPM0 AC set.
...
2. Run `make triad-task-finish TASK_ID="BEDPM0-code"` from inside the worktree.

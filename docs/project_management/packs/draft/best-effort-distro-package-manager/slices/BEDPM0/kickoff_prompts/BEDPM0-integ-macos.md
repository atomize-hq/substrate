# Kickoff: BEDPM0-integ-macos (integration macos)

## Scope
- Resolve CP1 parity drift for BEDPM0 on macOS.
...
2. Confirm the worktree is `wt/best-effort-distro-package-manager-bedpm0-integ-macos` on branch `best-effort-distro-package-manager-bedpm0-integ-macos` and `.taskmeta.json` exists at the worktree root.
...
4. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/best-effort-distro-package-manager" TASK_ID="BEDPM0-integ-macos"`.
...
2. Run `make triad-task-finish TASK_ID="BEDPM0-integ-macos"` from inside the worktree.

# Kickoff: BEDPM2-integ-windows (integration Windows)

## Scope
- Resolve CP1 parity drift for BEDPM2 on Windows.
...
2. Confirm the worktree is `wt/best-effort-distro-package-manager-bedpm2-integ-windows` on branch `best-effort-distro-package-manager-bedpm2-integ-windows` and `.taskmeta.json` exists at the worktree root.
...
4. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/best-effort-distro-package-manager" TASK_ID="BEDPM2-integ-windows"`.
...
2. Run `make triad-task-finish TASK_ID="BEDPM2-integ-windows"` from inside the worktree.

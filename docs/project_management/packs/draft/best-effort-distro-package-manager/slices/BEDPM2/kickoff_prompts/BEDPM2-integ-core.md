# Kickoff: BEDPM2-integ-core (integration core)

## Scope
- Merge BEDPM2 code and test branches and make the core slice green before CP1.
...
1. Confirm the worktree is `wt/best-effort-distro-package-manager-bedpm2-integ-core` on branch `best-effort-distro-package-manager-bedpm2-integ-core` and `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/best-effort-distro-package-manager" TASK_ID="BEDPM2-integ-core"`.

## Requirements
- Merge `BEDPM2-code` and `BEDPM2-test`.
...
2. Run `make triad-task-finish TASK_ID="BEDPM2-integ-core"` from inside the worktree.

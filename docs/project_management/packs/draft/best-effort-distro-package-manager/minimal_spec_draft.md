...
- Spec manifest: `docs/project_management/packs/draft/best-effort-distro-package-manager/spec_manifest.md`
- Impact map: `docs/project_management/packs/draft/best-effort-distro-package-manager/impact_map.md`

## Scope + authority

This draft defines only:
- Cross-cutting defaults, precedence, and invariants that all slice specs and `contract.md` must align on.
- Shared vocabulary (allowed values, stable enum values, stable one-liner shape).
- Planning follow-ups that must be resolved during full planning.

This draft does not define:
- Slice-level behavior details, detailed schemas, or implementation tasks.
- Exact prose for non-normative logs/errors (except where the ADR requires exact text).

## Defaults + precedence

### Platform scope
- Linux installer behavior changes for this feature.
- macOS/Windows behavior remains unchanged by this feature.
...
- `/etc/os-release` contents are parsed as plain text; the implementation does not `source`/eval/execute file content.
- Distro detection performs no network calls.
- Operator-facing outputs introduced/changed by this feature are limited to:
  - the required decision one-liner fields (`<id>`, `<id_like>`, `<pkg_manager>`, `source`), and
  - the PATH ambiguity warning content elements (other detected supported managers plus override mechanism).

## Exit-code posture

- Exit code taxonomy: `docs/project_management/system/standards/shared/EXIT_CODE_TAXONOMY.md`
- This work introduces no new exit codes.
...
### `/etc/os-release` family mapping (initial locked intent from ADR)
...
### Cross-pack non-overlap boundary (queued conflict)
- This pack defines detection + selection outputs and explicitly does not persist an `install_state.json`.
- The pack `docs/project_management/packs/draft/persist-detected-linux-distro-pkg-manager/` persists outputs and treats this pack’s `contract.md` as authoritative for detection/selection semantics.

## Follow-ups for full planning

- Reconcile ADR “Related Docs” link drift (`detecting-badger/*` vs `best-effort-distro-package-manager/*`).
- DR-0001: Decide `/etc/os-release` parsing + canonicalization rules (quotes/whitespace, duplicate keys, comments, case-sensitivity).
- DR-0002: Decide PATH probe fixed precedence order and the complete warning content requirements.
...
- Pin when the decision one-liner is emitted for the “no prereqs needed” path to preserve the “exactly once” invariant (including when `install-substrate.sh` is sourced by other scripts).
- Explicitly scope exit-code meanings as Linux-only vs global to avoid accidental macOS/Windows behavior changes.
...
- slice_id: `BEDPM0`
  - name: Detect distro and report decision
...
- slice_id: `BEDPM1`
  ...
  - intent: Stabilize the precedence pipeline (`--pkg-manager`/`PKG_MANAGER`/mapping/PATH probe), failure posture, and exit-code mapping for override/selection failures.
  - likely touch surfaces:
    - `scripts/substrate/install-substrate.sh`
    - `docs/project_management/packs/draft/best-effort-distro-package-manager/contract.md`
...
- slice_id: `BEDPM2`
  ...
  - intent: Stabilize a hermetic test harness that asserts precedence, one-liner content, and exit codes using a stubbed `PATH` and fake os-release input with no host mutation.
  - likely touch surfaces:
    ...
    - `docs/project_management/packs/draft/best-effort-distro-package-manager/slices/`

Downstream notes:
- `docs/project_management/packs/draft/best-effort-distro-package-manager/logs/CI-checkpoint/` uses this slice list as the default input when populating the machine-readable slice list in `ci_checkpoint_plan.md` (no mechanical validation until slice tasks exist in `tasks.json`).
- `docs/project_management/packs/draft/best-effort-distro-package-manager/logs/workstream-triage/` records proposed edits to this slice skeleton as recommendations in `workstream_triage.md`; this file remains unchanged.

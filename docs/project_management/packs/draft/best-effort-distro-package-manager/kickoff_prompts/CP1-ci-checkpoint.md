...
## Scope
- Run compile parity and CI Testing quick for the `BEDPM0` checkpoint boundary.
...
2. Determine `CHECKOUT_SHA` from the `BEDPM0-integ-core` branch:

```bash
CORE_BRANCH="$(jq -r --arg id "BEDPM0-integ-core" '.tasks[] | select(.id==$id) | .git_branch' "docs/project_management/packs/draft/best-effort-distro-package-manager/tasks.json")"
CHECKOUT_SHA="$(git rev-parse "$CORE_BRANCH")"
```

## Requirements
- Use the CI audit ledger path `docs/project_management/packs/draft/best-effort-distro-package-manager/logs/BEDPM0/ci-audit/ledger.jsonl`.
- Run `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/packs/draft/best-effort-distro-package-manager/logs/BEDPM0/ci-audit/ledger.jsonl" --kind ci-testing --orch-branch "feat/best-effort-distro-package-manager"`.

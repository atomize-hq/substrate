...
- `SEAM-1` (active, integration): Snapshot V3 `net_allowed` contract + host→world-agent plumbing (remove in-guest broker state).
- `SEAM-2` (next, platform/risk): `crates/world` netfilter enforcement is fail-closed and applies to every executed process (cgroup attach invariants).
- `SEAM-3` (future, capability): Config opt-in `world.net.filter` + CLI set/reset + env override/export + docs.

...
1. Confirm the worktree is `wt/persist-detected-linux-distro-pkg-manager-pdldpm1-code` on branch `persist-detected-linux-distro-pkg-manager-pdldpm1-code` and `.taskmeta.json` exists at the worktree root.
...
## Requirements
- Implement only the production changes required by the PDLDPM1 AC set.
- Keep successful Linux hosted and dev installs on the accepted write matrix.
- Preserve `--dry-run` as a no-write branch and metadata failure as warning-only behavior.
...
2. Run `make triad-task-finish TASK_ID="PDLDPM1-code"` from inside the worktree.

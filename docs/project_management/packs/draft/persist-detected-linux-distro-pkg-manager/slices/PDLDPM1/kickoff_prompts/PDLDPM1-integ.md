...
## Scope
- Merge code and tests, resolve drift to the spec, and make PDLDPM1 green.
...
1. Verify you are in `wt/persist-detected-linux-distro-pkg-manager-pdldpm1-integ` on branch `persist-detected-linux-distro-pkg-manager-pdldpm1-integ` and `.taskmeta.json` exists.
2. Read `plan.md`, `tasks.json`, `session_log.md`, `compatibility-spec.md`, `contract.md`, the slice spec, and this prompt.
...
## Requirements
- Merge `PDLDPM1-code` and `PDLDPM1-test`, then reconcile code and tests to the spec.
- Run:
  - `cargo fmt`
...
2. From inside the worktree, run `make triad-task-finish TASK_ID="PDLDPM1-integ"`.

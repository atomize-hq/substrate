...
## Scope
- Merge PDLDPM0 code and test branches and make the slice green.
...
1. Confirm the worktree is `wt/persist-detected-linux-distro-pkg-manager-pdldpm0-integ` on branch `persist-detected-linux-distro-pkg-manager-pdldpm0-integ` and `.taskmeta.json` exists at the worktree root.
...
## Requirements
- Merge `PDLDPM0-code` and `PDLDPM0-test`.
...
2. Run `make triad-task-finish TASK_ID="PDLDPM0-integ"` from inside the worktree.

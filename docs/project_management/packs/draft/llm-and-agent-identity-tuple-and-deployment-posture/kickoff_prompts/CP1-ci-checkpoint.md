...
3. Confirm `LAITDP1` is listed in `tasks.json` `meta.checkpoint_boundaries`.
4. Compute the checkpoint checkout SHA from `LAITDP1-integ-core`:
   - `CORE_BRANCH="$(jq -r --arg id "LAITDP1-integ-core" '.tasks[] | select(.id==$id) | .git_branch' docs/project_management/packs/draft/llm-and-agent-identity-tuple-and-deployment-posture/tasks.json)"`
...
2. Run feature smoke:
   - `make feature-smoke FEATURE_DIR="docs/project_management/packs/draft/llm-and-agent-identity-tuple-and-deployment-posture" PLATFORM=behavior SMOKE_SLICE_ID="LAITDP1" SMOKE_CHECKOUT_REF="$CHECKOUT_SHA" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/llm-and-agent-identity-tuple-and-deployment-posture" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=0`
...
3. If Linux or macOS smoke fails, or if CI parity fails on any platform, start only the matching platform-fix tasks for `LAITDP1`.
...
2. Mark `CP1-ci-checkpoint` completed in `docs/project_management/packs/draft/llm-and-agent-identity-tuple-and-deployment-posture/tasks.json`.
3. Do not begin `LAITDP2` until this task is green.

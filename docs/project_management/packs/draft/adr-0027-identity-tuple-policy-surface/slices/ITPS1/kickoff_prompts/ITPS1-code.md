...
1. Verify the worktree is `wt/adr-0027-identity-tuple-policy-surface-itps1-code` on branch `adr-0027-identity-tuple-policy-surface-itps1-code`.
...
## Requirements
- Implement only the production behaviors required by `AC-ITPS1-01` through `AC-ITPS1-08`.
- Run targeted baseline tests before and after the change.
- Run `cargo fmt` and `cargo clippy --workspace --all-targets -- -D warnings`.

## End Checklist
1. Run `make triad-task-finish TASK_ID="ITPS1-code"` inside the worktree.

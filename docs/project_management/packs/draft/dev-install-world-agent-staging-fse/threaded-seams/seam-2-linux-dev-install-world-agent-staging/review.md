...
---
# Review Bundle - SEAM-2 Linux dev-install world-agent staging
...
- Can `scripts/substrate/dev-install-substrate.sh --no-world` still skip staging one of the accepted `world-agent` paths or stage the wrong profile target?
...
```mermaid
flowchart LR
  DI["dev-install-substrate.sh --no-world --profile <debug|release>"] --> BUILD["target/<profile>/world-agent"]
  BUILD --> LINK1["target/bin/world-agent -> target/<profile>/world-agent"]
  BUILD --> LINK2["target/bin/linux/world-agent -> target/<profile>/world-agent"]
...
flowchart TB
  ...
  SEAM2 --> LINKS["target/bin/world-agent + target/bin/linux/world-agent"]

...
- Can `substrate world enable` still launch the helper or write *any* state (config, env exports, logs, socket activation, provisioning) when the accepted staged `world-agent` artifact set is missing in the standard version-dir flow?
...
flowchart LR
  ...
  VDIR --> P1{"<version_dir>/bin/world-agent exists?"}
  P1 -->|yes| RUN["Launch helper / verify health"]
  P1 -->|no| P2{"<version_dir>/bin/linux/world-agent exists?"}
  P2 -->|yes| RUN

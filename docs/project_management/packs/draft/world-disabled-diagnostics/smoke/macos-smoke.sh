...
if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "SKIP: world-disabled-diagnostics macOS smoke (not macOS)"
  exit 0
fi
...
PY
  ...
  # Force-enabled, intentionally broken connectivity via nonexistent world socket (bypasses Lima detection).
  local sock

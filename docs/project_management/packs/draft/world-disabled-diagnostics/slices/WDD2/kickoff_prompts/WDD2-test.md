...
1. Verify you are in the task worktree `wt/world-disabled-diagnostics-wdd2-test` on branch `world-disabled-diagnostics-wdd2-test` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   ...
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/draft/world-disabled-diagnostics" TASK_ID="WDD2-test"` (single task only)
...
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WDD2-test"`

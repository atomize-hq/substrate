...
- The wrapper/final gate MUST run CI Testing; if CI Testing is green, then mark platform-fix tasks `completed` as no-ops to unblock the final aggregator’s `depends_on`:
  - `make triad-mark-noop-platform-fixes-completed FEATURE_DIR="docs/project_management/packs/active/warn-config-global-show-workspace-overrides" SLICE_ID="<slice>"` (optional: add `SMOKE_RUN_ID="<run-id>"` for logging)

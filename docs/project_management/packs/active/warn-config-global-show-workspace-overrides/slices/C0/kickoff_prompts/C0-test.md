...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start-pair FEATURE_DIR="docs/project_management/packs/active/warn-config-global-show-workspace-overrides" SLICE_ID="<slice>"` (preferred; starts code+test in parallel; `<slice>` is `C0-test` without `-test`)
   - `make triad-task-start FEATURE_DIR="docs/project_management/packs/active/warn-config-global-show-workspace-overrides" TASK_ID="C0-test"` (single task only)

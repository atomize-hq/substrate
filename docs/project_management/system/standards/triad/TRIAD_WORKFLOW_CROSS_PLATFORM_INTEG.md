...
- The dispatch scripts are hardened with timeouts to avoid indefinite hangs; default max wait is **2 hours**. If you hit infra slowness, you can override via `FEATURE_SMOKE_WATCH_TIMEOUT_SECS` / `CI_TESTING_WATCH_TIMEOUT_SECS` (see `docs/project_management/system/standards/ci/PLATFORM_INTEGRATION_AND_CI.md`).
...
Exception:
- Create `X-integ-wsl` only when the rubric in `docs/project_management/system/standards/ci/PLATFORM_INTEGRATION_AND_CI.md` justifies separate ownership.

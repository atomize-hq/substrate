...
Options:
  ...
  --launch-codex-missing   With --launch-codex: only launch Codex for tasks missing CODEX_LAST_MESSAGE_PATH (safe resume)
...
Notes:
  ...
  - Uses the same underlying automation as single-task start (`make triad-orch-ensure` and `make triad-task-start`).
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --launch-codex)
            ...
            shift 1
            ;;
        --launch-codex-missing)
            LAUNCH_CODEX_MISSING=1
            shift 1
            ;;
        --codex-profile)
            CODEX_PROFILE="${2:-}"
...
if [[ "${LAUNCH_CODEX_MISSING}" -eq 1 && "${LAUNCH_CODEX}" -ne 1 ]]; then
    die "--launch-codex-missing requires --launch-codex"
fi
...
TRIAD_SCRIPTS_DIR="${PM_SYSTEM_ROOT}/scripts/triad"
...
if [[ "${DRY_RUN}" -eq 1 ]]; then
    "${TRIAD_SCRIPTS_DIR}/orch_ensure.sh" --feature-dir "${FEATURE_DIR_ABS}" --dry-run >/dev/null
else
    "${TRIAD_SCRIPTS_DIR}/orch_ensure.sh" --feature-dir "${FEATURE_DIR_ABS}" >/dev/null
fi
...
code_args=("${TRIAD_SCRIPTS_DIR}/task_start.sh" --feature-dir "${FEATURE_DIR_ABS}" --task-id "${CODE_TASK_ID}")
...
test_args=("${TRIAD_SCRIPTS_DIR}/task_start.sh" --feature-dir "${FEATURE_DIR_ABS}" --task-id "${TEST_TASK_ID}")
...
launch_codex_one() {
    ...
    write_missing_last_message_stub() {
        ...
        mkdir -p "$(dirname "${last_message}")" >/dev/null 2>&1 || true
        {
            printf '# Generated task summary (Codex last message missing)\n\n'
            printf 'This file was generated by `make triad-task-start-pair` because Codex did not write `--output-last-message`.\n'
            printf 'This commonly indicates the Codex process was interrupted or crashed.\n\n'
            printf -- '- Task: `%s`\n' "${task_id}"
            printf -- '- Worktree: `%s`\n' "${worktree}"
            printf -- '- Exit code: `%s`\n' "${exit_code}"
            printf -- '- Codex stderr log: `%s`\n' "${stderr}"
            printf -- '- Codex events log: `%s`\n' "${events}"
        } >"${last_message}" 2>/dev/null || true
    }

    wait_for_existing_codex_if_running() {
        if [[ ! -f "${pid_path}" ]]; then
            return 0
        fi
        ...
        if ! printf '%s\n' "${cmd}" | grep -qiE -- '(^|[[:space:]/])codex([[:space:]]|$)'; then
            log "WARN: stale codex.pid for ${task_id} (pid=${pid}) does not look like Codex; removing pid file"
            log "WARN: cmd=${cmd}"
            rm -f "${pid_path}" >/dev/null 2>&1 || true
            return 0
        fi

        log "Waiting for existing Codex to exit: task=${task_id} pid=${pid}"
    ...
    if [[ "${LAUNCH_CODEX_MISSING}" -eq 1 && -f "${last_message}" ]]; then
        return 0
    fi
...
if [[ "${LAUNCH_CODEX}" -eq 1 ]]; then
    ...
    if [[ "${DRY_RUN}" -eq 1 ]]; then
        ...
        if [[ "${LAUNCH_CODEX_MISSING}" -eq 1 ]]; then
            log "Launching Codex headless for missing tasks only (in parallel where applicable)"
        else
            log "Launching Codex headless for both tasks (in parallel)"
        fi
        set +e
        launch_codex_one "${CODE_WORKTREE}" "${CODE_KICKOFF}" "${CODEX_CODE_OUT_DIR}" "${CODEX_CODE_LAST_MESSAGE_PATH}" "${CODEX_CODE_EVENTS_PATH}" "${CODEX_CODE_STDERR_PATH}" "${CODE_TASK_ID}" &
        pid_code=$!
        launch_codex_one "${TEST_WORKTREE}" "${TEST_KICKOFF}" "${CODEX_TEST_OUT_DIR}" "${CODEX_TEST_LAST_MESSAGE_PATH}" "${CODEX_TEST_EVENTS_PATH}" "${CODEX_TEST_STDERR_PATH}" "${TEST_TASK_ID}" &
        pid_test=$!

...
usage() {
    cat <<'USAGE'
Usage:
  make triad-task-start-complete FEATURE_DIR=<path> SLICE_ID=<slice> [CODEX_PROFILE=<p>] [CODEX_MODEL=<m>] [CODEX_JSONL=1] [DRY_RUN=1]
...
if [[ "${DRY_RUN}" -eq 1 ]]; then
    "${TRIAD_SCRIPTS_DIR}/orch_ensure.sh" --feature-dir "${FEATURE_DIR_ABS}" --dry-run 1>>"${LOG_PATH}" 2>>"${LOG_PATH}"
else
    "${TRIAD_SCRIPTS_DIR}/orch_ensure.sh" --feature-dir "${FEATURE_DIR_ABS}" 1>>"${LOG_PATH}" 2>>"${LOG_PATH}"
fi
...
ensure_last_message_or_stub() {
    ...
    mkdir -p "$(dirname "${last_message_path}")" >/dev/null 2>&1 || true
    {
        ...
        printf 'This file was generated by `make triad-task-start-complete` because the expected Codex `--output-last-message` file was missing.\n'
...
relaunch_codex_for_task_if_needed() {
    ...
    start_args=("${TRIAD_SCRIPTS_DIR}/task_start.sh" --feature-dir "${FEATURE_DIR_ABS}" --task-id "${task_id}" --launch-codex)
...
PY
    ...
    pair_args=("${TRIAD_SCRIPTS_DIR}/task_start_pair.sh" --feature-dir "${FEATURE_DIR_ABS}" --slice-id "${SLICE_ID}" --launch-codex)
    ...
    if [[ "${CODE_TEST_STATUS}" != "completed" ]]; then
        ...
        if [[ ! -f "${CODEX_CODE_LAST_MESSAGE_PATH}" || ! -f "${CODEX_TEST_LAST_MESSAGE_PATH}" ]]; then
            ...
            pair_resume_args=("${TRIAD_SCRIPTS_DIR}/task_start_pair.sh" --feature-dir "${FEATURE_DIR_ABS}" --slice-id "${SLICE_ID}" --launch-codex --launch-codex-missing)
...
PY
    ...
    integ_args=("${TRIAD_SCRIPTS_DIR}/task_start.sh" --feature-dir "${FEATURE_DIR_ABS}" --task-id "${INTEG_TASK_ID}" --launch-codex)

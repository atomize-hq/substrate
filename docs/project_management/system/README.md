...
- `PM_PACKS_ROOT`: planning packs root (typically `docs/project_management/packs/`)
- `PM_DEFAULT_PACK_BUCKET`: default bucket name under packs root (typically `active`)

Path resolver helper:

- `pm_paths.py` (under `${PM_SYSTEM_SCRIPTS}/planning/`)
  - `print-roots`
  - `resolve-feature-dir`

...
- Minimum viable “full isolation” rootfs:
...
- Capability/privilege detection:
  - If `world_fs.isolation=full` cannot be created, Substrate must fail closed with a clear diagnostic (I1).

## Acceptance
- With `world_fs.isolation=full`, commands cannot write to or read from arbitrary host paths outside the allowed mounts.

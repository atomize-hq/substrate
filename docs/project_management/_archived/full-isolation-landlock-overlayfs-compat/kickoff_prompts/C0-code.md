...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start-pair FEATURE_DIR="docs/project_management/_archived/full-isolation-landlock-overlayfs-compat" SLICE_ID="<slice>"` (preferred; starts code+test in parallel; `<slice>` is `C0-code` without `-code`)
   - `make triad-task-start FEATURE_DIR="docs/project_management/_archived/full-isolation-landlock-overlayfs-compat" TASK_ID="C0-code"` (single task only)

...
- Closeout report: `docs/project_management/_archived/workspace-config-policy-unification/WCU1-closeout_report.md`
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/_archived/workspace-config-policy-unification" TASK_ID="WCU1-integ"`
...
- Dispatch behavioral smoke via CI from this worktree’s `HEAD`:
  - `make feature-smoke FEATURE_DIR="docs/project_management/_archived/workspace-config-policy-unification" PLATFORM=behavior SMOKE_SLICE_ID="WCU1" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/workspace-config-policy-unification" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=1`

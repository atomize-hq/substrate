# Kickoff – M5b-integ (Host detection parity)
...
2. Set `M5b-integ` to `in_progress` in `tasks.json` (orchestration branch only).
3. Add START entry to `session_log.md`; commit docs (`docs: start M5b-integ`).
4. Create branch `mp-m5b-world-deps-host-detect-integ`, then worktree: `git worktree add wt/mp-m5b-world-deps-host-detect-integ mp-m5b-world-deps-host-detect-integ`.

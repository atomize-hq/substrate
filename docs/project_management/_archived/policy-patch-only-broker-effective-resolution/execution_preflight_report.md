...
- Planning quality gate is `ACCEPT` (`docs/project_management/_archived/policy-patch-only-broker-effective-resolution/quality_gate_report.md`): VERIFIED
- ADR accepted and still matches intent (`docs/project_management/adrs/implemented/ADR-0013-policy-patch-only-broker-canonical-effective-resolution.md`): VERIFIED (Status: Approved)
...
From `docs/project_management/_archived/policy-patch-only-broker-effective-resolution/tasks.json` meta:
...
Smoke scripts:
- Linux smoke: `docs/project_management/_archived/policy-patch-only-broker-effective-resolution/smoke/linux-smoke.sh`
- macOS smoke: `docs/project_management/_archived/policy-patch-only-broker-effective-resolution/smoke/macos-smoke.sh`
- Windows smoke: `docs/project_management/_archived/policy-patch-only-broker-effective-resolution/smoke/windows-smoke.ps1`
...
Integration task dispatch commands live in `docs/project_management/_archived/policy-patch-only-broker-effective-resolution/tasks.json` integration checklists.
...
- `make -n feature-smoke FEATURE_DIR="docs/project_management/_archived/policy-patch-only-broker-effective-resolution" PLATFORM=behavior SMOKE_SLICE_ID="C0" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/policy-patch-only-broker-effective-resolution" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=1` → exit `0`
...
Verified:
- `make triad-orch-ensure FEATURE_DIR="docs/project_management/_archived/policy-patch-only-broker-effective-resolution"` → exit `0`

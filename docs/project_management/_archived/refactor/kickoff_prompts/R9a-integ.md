# Task R9a-integ (Routing split: dispatch & builtins) – INTEGRATION
...
3. Confirm `R9a-code` and `R9a-test` are completed.
4. Set `R9a-integ` to `in_progress` in `tasks.json`, add a START entry to
   `session_log.md`, and commit the docs update (`git commit -am "docs: start R9a-integ"`).
5. Create the task branch and worktree:
   ```
   git checkout -b cr-r9a-routing-integ
   git worktree add wt/cr-r9a-routing-integ cr-r9a-routing-integ
   cd wt/cr-r9a-routing-integ
   ```
   Do not edit docs/tasks/session logs from the worktree.

## Spec
- Merge the R9a code and test branches for routing dispatch/builtin split.
...
3. Return to repo root, merge the integration branch:
   ```
   git checkout cr-r9a-routing-integ
   git merge --ff-only wt/cr-r9a-routing-integ   # if needed
   git checkout feat/crate-refactor
   git pull --ff-only
   git merge --ff-only cr-r9a-routing-integ
   ```
...
5. Commit docs/log updates on `feat/crate-refactor`
   (`git commit -am "docs: finish R9a-integ"`).
6. Remove the worktree (`git worktree remove wt/cr-r9a-routing-integ`) if done and

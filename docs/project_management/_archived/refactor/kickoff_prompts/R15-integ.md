# Task R15-integ (Shell integration test split) – INTEGRATION
...
3. Confirm `R15-code` and `R15-test` are completed.
4. Set `R15-integ` to `in_progress` in `tasks.json`, add a START entry to
   `session_log.md`, and commit the docs update (`git commit -am "docs: start R15-integ"`).
5. Create the task branch and worktree:
   ```
   git checkout -b cr-r15-shell-integration-integ
   git worktree add wt/cr-r15-shell-integration-integ cr-r15-shell-integration-integ
   cd wt/cr-r15-shell-integration-integ
   ```
   Do not edit docs/tasks/session logs from the worktree.

## Spec
- Merge the R15 code and test branches for shell integration test splits.
- Resolve conflicts, ensure module paths/fixtures align, and keep behavior unchanged.
...
3. Return to repo root, merge the integration branch:
   ```
   git checkout cr-r15-shell-integration-integ
   git merge --ff-only wt/cr-r15-shell-integration-integ   # if needed
   git checkout feat/crate-refactor
   git pull --ff-only
   git merge --ff-only cr-r15-shell-integration-integ
   ```
...
5. Commit docs/log updates on `feat/crate-refactor`
   (`git commit -am "docs: finish R15-integ"`).
6. Remove the worktree (`git worktree remove wt/cr-r15-shell-integration-integ`) if done and

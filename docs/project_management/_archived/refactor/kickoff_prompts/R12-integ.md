# Task R12-integ (Routing builtin/world runner slimming) – INTEGRATION
...
3. Confirm `R12-code` and `R12-test` are completed.
4. Set `R12-integ` to `in_progress` in `tasks.json`, add a START entry to
   `session_log.md`, and commit the docs update (`git commit -am "docs: start R12-integ"`).
5. Create the task branch and worktree:
   ```
   git checkout -b cr-r12-routing-builtin-integ
   git worktree add wt/cr-r12-routing-builtin-integ cr-r12-routing-builtin-integ
   cd wt/cr-r12-routing-builtin-integ
   ```
   Do not edit docs/tasks/session logs from the worktree.

## Spec
- Merge the R12 code and test branches for routing builtin/world runner slimming.
...
3. Return to repo root, merge the integration branch:
   ```
   git checkout cr-r12-routing-builtin-integ
   git merge --ff-only wt/cr-r12-routing-builtin-integ   # if needed
   git checkout feat/crate-refactor
   git pull --ff-only
   git merge --ff-only cr-r12-routing-builtin-integ
   ```
...
5. Commit docs/log updates on `feat/crate-refactor`
   (`git commit -am "docs: finish R12-integ"`).
6. Remove the worktree (`git worktree remove wt/cr-r12-routing-builtin-integ`) if done and

...
## Metadata
- Feature directory: `docs/project_management/_archived/next/macos_world_backend_vf/`
...
```bash
export FEATURE_DIR="docs/project_management/_archived/next/macos_world_backend_vf"

# Mechanical lint (required)
make planning-lint FEATURE_DIR="$FEATURE_DIR"
# exit=2
# == Planning lint: docs/project_management/_archived/next/macos_world_backend_vf ==
# Missing required path: docs/project_management/_archived/next/macos_world_backend_vf/plan.md

# tasks.json validation (required by lint checklist)
make planning-validate FEATURE_DIR="$FEATURE_DIR"
# exit=2
# docs/project_management/_archived/next/macos_world_backend_vf/tasks.json: missing
...
rg -n "macos_world_backend_vf" docs/project_management/packs/sequencing.json
# exit=1 (no matches)
...
- `docs/project_management/packs/sequencing.json`: `NO` (validated + searched; not read end-to-end)
- Standards:
...
- Result: `FAIL`
- Evidence:
  - `docs/project_management/_archived/next/macos_world_backend_vf/05_policy_model.md:58` (“should ship”)
  - `docs/project_management/_archived/next/macos_world_backend_vf/05_policy_model.md:65` (“likely requires”)
  - `docs/project_management/_archived/next/macos_world_backend_vf/07_testing_plan.md:37-40` (“should fail”)
  - `docs/project_management/_archived/next/macos_world_backend_vf/01_problem_and_goals.md:3` (“etc.”)
...
- Result: `FAIL`
- Evidence:
  - Missing `docs/project_management/_archived/next/macos_world_backend_vf/decision_register.md`
...
- Result: `FAIL`
- Evidence:
  - Proposed command `substrate world create...` in `docs/project_management/_archived/next/macos_world_backend_vf/04_architecture_and_flows.md:41`, but current CLI has no `world create` subcommand (`crates/shell/src/execution/cli.rs:522-533`).
  - Proposed env var `SUBSTRATE_WORLD_BACKEND` in `docs/project_management/_archived/next/macos_world_backend_vf/08_rollout_plan.md:9`, but it does not exist in code (repo search returns only this doc).
...
- Result: `FAIL`
- Evidence:
  - `docs/project_management/packs/sequencing.json` has no entries for `macos_world_backend_vf` (search returned no matches).
...
- Status: `DEFECT`
- Evidence:
  - `make planning-lint` fails with missing `docs/project_management/_archived/next/macos_world_backend_vf/plan.md`.
  - `docs/project_management/_archived/next/macos_world_backend_vf/` contains only narrative docs + ADR (no `plan.md`, `spec_manifest.md`, `tasks.json`, `session_log.md`, `kickoff_prompts/`, slice specs).
...
- Status: `DEFECT`
- Evidence: `make planning-lint FEATURE_DIR="docs/project_management/_archived/next/macos_world_backend_vf"` exits non-zero (missing required files).
...
- Status: `DEFECT`
- Evidence:
  - `docs/project_management/_archived/next/macos_world_backend_vf/04_architecture_and_flows.md:41` references `substrate world create...`; current world CLI actions are `Doctor|Enable|Deps|Cleanup|Verify` (`crates/shell/src/execution/cli.rs:522-533`).
  - `docs/project_management/_archived/next/macos_world_backend_vf/08_rollout_plan.md:9` references `SUBSTRATE_WORLD_BACKEND`, which is absent in code (repo search finds only this doc).
...
- Status: `DEFECT`
- Evidence:
  - Proposed placeholder shadow-tree model in `docs/project_management/_archived/next/macos_world_backend_vf/05_policy_model.md:19-48`.

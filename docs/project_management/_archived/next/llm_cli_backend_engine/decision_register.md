...
- The preferred mechanism going forward is a host→world secret-channel payload delivered to the in-world gateway/manager via an inherited one-time FD/pipe auth bundle (no secret-bearing env vars in-world by default). See `docs/project_management/_archived/next/llm_gateway_in_world/decision_register.md` (DR-0018) and `docs/project_management/_archived/next/llm_gateway_in_world/specs/env_injection.md`.
...
- However, a host-run CLI/wrapper MAY still be compatible with the enforcement story if it is operating as a _client_ of the in-world gateway (i.e., “frontend mode”), so that **all** network egress happens at the gateway inside the world boundary. That mode is distinct from “CLI provider backend engine” behavior.
- Concrete example (Codex):
  - If the Unified Agent Wrapper is pointed at the in-world gateway and the gateway performs the outbound request to the OpenAI/Codex endpoint, then the auth material (e.g., from `~/.codex/auth.json`) must still be made available to the component that performs outbound egress (gateway/engine), either:
...
**Date:** 2026-02-09  
**Status:** Accepted  
**Related docs:** ADR-0024, ADR-0023, ADR-0027, `docs/project_management/_archived/next/llm_gateway_in_world/specs/env_injection.md`
...
**Date:** 2026-02-13  
**Status:** Accepted  
**Related docs:** `docs/project_management/standards/SECRETS_DELIVERY_CHANNEL_RUBRIC.md`, `docs/project_management/_archived/next/llm_gateway_in_world/decision_register.md` (DR-0018)

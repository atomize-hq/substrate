...
- **Deny-by-default allowlist.** If `llm.allowed_backends=[]`, routing to CLI backends is denied by default.
  - Credential reuse note: subscription auth state MAY be forwarded/mounted into the world (policy-gated; no secrets in Substrate YAML). See `/Users/spensermcconnell/__Active_Code/atomize-hq/substrate/docs/project_management/_archived/next/llm_cli_backend_engine/decision_register.md` (DR-0004).
...
- Streaming semantics (true stream vs synthetic stream vs non-stream) are finalized via `docs/project_management/_archived/next/llm_cli_backend_engine/decision_register.md`.

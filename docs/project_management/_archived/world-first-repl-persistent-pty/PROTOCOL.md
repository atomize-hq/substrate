...
Host rendering note:
- Because `stdout` is a raw byte stream (may be non-UTF8) and may arrive while the host line editor is active, host rendering MUST follow `docs/project_management/_archived/world-first-repl-persistent-pty/STATE_MACHINE.md` (`Idle` → “Out-of-band world PTY output”).
...
- Before executing a new REPL command, Substrate MUST compute, for the current `world_cwd`:
  ...
  - the effective workspace root (if any; see `docs/project_management/_archived/world-first-repl-persistent-pty/decision_register.md` DR-09).

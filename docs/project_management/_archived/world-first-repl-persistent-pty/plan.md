...
- Record each CI run to the per-slice evidence ledger (gitignored): `docs/project_management/_archived/world-first-repl-persistent-pty/logs/<slice>/ci-audit/ledger.jsonl`.
...
- Authoritative spec pack:
  ...
  - `docs/project_management/_archived/world-first-repl-persistent-pty/RESEARCH.md` (historical)
...
- Kickoff prompts: `docs/project_management/_archived/world-first-repl-persistent-pty/kickoff_prompts/`
...
- Implement the in-world driver loop owned by world-agent:
  ...
  - See `docs/project_management/_archived/world-first-repl-persistent-pty/PROTOCOL.md` (“Internal Driver Loop (World-Agent Owned)”) for the v1 in-process/no-OS-FD-pipes constraint.
...
- Add targeted tests for protocol robustness and security invariants:
  ...
  - Output ordering: prompt/resume input must not occur before all foreground PTY stdout for that command has been forwarded (no “late stdout after command_complete” for non-backgrounded commands). See DR-23 and `docs/project_management/_archived/world-first-repl-persistent-pty/PROTOCOL.md` (“Output ordering / drain guarantee”, watermark barrier).

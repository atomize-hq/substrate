...
4. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/_archived/policy_and_config_precedence" TASK_ID="PCP0-integ-windows" TASK_PLATFORM=windows LAUNCH_CODEX=1`
...
- Validate Windows smoke via CI (repeat until green if you make fixes):
  - `make feature-smoke FEATURE_DIR="docs/project_management/_archived/policy_and_config_precedence" PLATFORM=windows RUNNER_KIND=self-hosted WORKFLOW_REF="feat/policy_and_config_precedence" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=1`

...
- Planning references: `docs/project_management/_archived/next/world-deps-sync.md`, `docs/project_management/_archived/next/substrate_isolated_shell_plan.md` (§4), and `docs/project_management/_archived/next/substrate_isolated_shell_data_map.md` (env vars for manifests + overrides).
...
Reminders:
- Start from `feat/isolated-shell-plan`: follow `AI_AGENT_START_HERE.md`, mark C2-test as `in_progress` in `docs/project_management/_archived/next/tasks.json`, and append a START entry to the session log before jumping into `wt/c2-world-deps-test`. Leave coordination files untouched in the test worktree.

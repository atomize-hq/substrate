...
- `CHANGELOG.md`, `docs/project_management/_archived/next/substrate_isolated_shell_execution_plan.md`
- Coordination references in `docs/project_management/_archived/next/session_log.md` + `tasks.json` (already updated on `feat/isolated-shell-plan`)
...
Reminders:
- Start at `AI_AGENT_START_HERE.md`, mark B3-integ as `in_progress` in `docs/project_management/_archived/next/tasks.json`, and add a START entry to `docs/project_management/_archived/next/session_log.md` before touching the integration worktree.
...
- Capture the markdownlint output: MD013 line-length warnings already exist across legacy docs (`docs/project_management/_archived/next/substrate_isolated_shell_data_map.md`, plan, README); document the known warnings rather than reflowing large sections unless instructed otherwise.

...
- Planning docs: `docs/project_management/_archived/next/substrate_isolated_shell_plan.md` (§5.3, §5.6), `.../substrate_isolated_shell_data_map.md` (config metadata + env vars), `.../substrate_isolated_shell_execution_plan.md` (Phase C details).
...
Reminders:
- Begin on `feat/isolated-shell-plan`: read `AI_AGENT_START_HERE.md`, set this task to `in_progress` in `docs/project_management/_archived/next/tasks.json`, and append a START entry in `session_log.md` before switching into `wt/c1-world-enable-code`.

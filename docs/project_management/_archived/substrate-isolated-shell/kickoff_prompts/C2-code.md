...
Summary:
- Add the `substrate world deps` command group with `status`, `install`, and `sync` actions that read a manifest of host/guest tool definitions, detect what’s available on the host vs. inside the world backend, and run the installer snippets in the guest when requested. Follow the workflow outlined in `docs/project_management/_archived/next/world-deps-sync.md` and the execution plan (§4 "World Dependency Sync").
...
- Planning docs: `docs/project_management/_archived/next/substrate_isolated_shell_plan.md` (§4, §5.5), `docs/project_management/_archived/next/substrate_isolated_shell_data_map.md` (env + config fields), `docs/project_management/_archived/next/world-deps-sync.md`, and `docs/project_management/_archived/next/substrate_isolated_shell_execution_plan.md` (Phase C milestones).
...
Reminders:
- Begin on `feat/isolated-shell-plan`: read `AI_AGENT_START_HERE.md`, flip C2-code to `in_progress` in `docs/project_management/_archived/next/tasks.json`, and append a START entry in `session_log.md` before editing. Do all code work inside `wt/c2-world-deps-code`; don’t touch coordination artifacts from that worktree.

...
Today, the world-agent also links `crates/broker` and performs policy resolution internally.

## Problem

### 1) Policy home mismatch (global patch inconsistency)

The world-agent is typically deployed as a systemd service (often running as `root`). Its process
...
- The world-agent can resolve a different effective policy (often missing the global patch), causing
...
shell/shim/world-agent.

### 2) Request concurrency bug (cross-request policy contamination)

The world-agent runs a multithreaded Tokio runtime and services requests concurrently.
...
- Ensure world-agent enforcement uses the same effective policy semantics as the CLI.
- Eliminate cross-request policy contamination under concurrency.
- Clarify what “global policy” means in a world-agent deployment:
  - single-user workstation,
  - multi-user host with shared world-agent,
...
- The world-agent calls broker APIs per request and relies on a shared broker handle.
- Full isolation writability depends on `world_fs.write_allowlist` being resolved by the world-agent
  and translated into mount-time remounts (via `SUBSTRATE_WORLD_FS_WRITE_ALLOWLIST`).

## Options Considered

### Option A: Serialize world-agent policy resolution with a lock
...
- Keeps world-agent self-contained.

Cons:
- Still relies on local filesystem + `SUBSTRATE_HOME` semantics.
- More CPU/io (parsing YAML per request) unless cached carefully.

### Option C: Host-resolved policy snapshot passed to world-agent (preferred direction)

Move effective policy resolution to the host-side Substrate CLI (the “authoritative policy
resolver”), and pass a policy snapshot (or the subset needed for enforcement) to world-agent as part
of the request.
...
- Removes world-agent dependence on `$SUBSTRATE_HOME` semantics.
...
- Requires careful threat modeling: a privileged world-agent must not accept untrusted “policy”
...
1. **Short-term correctness:** ensure the world-agent deployment has an explicit, intended policy
...
2. **Medium-term correctness:** change world-agent to use per-request policy snapshots rather than

...
# Shared behavior smoke for world-fs-granular-allow-deny.
#
# Caller responsibilities:
# - Ensure host prerequisites for the intended platform (Linux world-agent socket, or macOS Lima VM).
...
case "$mode" in
  schema)
    schema_smoke
    echo "OK: world-fs-granular-allow-deny smoke passed (schema-only; slice=$slice_id)"
    exit 0
    ;;
  wfgad3|wfgad4|full)
    world_preflight

...
1. Verify you are in the task worktree `wt/world-fs-granular-allow-deny-c0-integ` on branch `world-fs-granular-allow-deny-c0-integ` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/next/world-fs-granular-allow-deny" TASK_ID="C0-integ"`
...
- Record evidence in `docs/project_management/next/world-fs-granular-allow-deny/session_log.md`.

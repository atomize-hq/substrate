...
- Existing behavior:
  - `substrate world deps current install|sync` did not execute `install.method=apt` and could not reliably apply apt+script inventory end-to-end per the contract.
  - Hardening/cage denials could be surfaced as “backend unavailable” exit `3` in some paths.

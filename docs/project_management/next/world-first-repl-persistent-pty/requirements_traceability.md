...
| R-029 | MUST/SHOULD | Policy snapshot drift restart: before each command, host MUST compute effective snapshot hash + workspace root; on drift, MUST restart session; SHOULD request cwd continuity; MUST report cwd change if continuity fails | `C3-code` | `C3-test` |

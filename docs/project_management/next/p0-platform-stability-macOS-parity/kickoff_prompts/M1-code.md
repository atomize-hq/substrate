...
5. Create branch `mp-m1-migration-code` from `feat/p0-platform-stability-macOS-parity`; run `git worktree add wt/mp-m1-migration-code mp-m1-migration-code`.
...
2. Inside `wt/mp-m1-migration-code`, commit M1-code changes to branch `mp-m1-migration-code` (no docs/tasks/session_log.md edits).
3. From outside the worktree, ensure branch `mp-m1-migration-code` contains the worktree commit (fast-forward if needed); do **not** merge into `feat/p0-platform-stability-macOS-parity`.

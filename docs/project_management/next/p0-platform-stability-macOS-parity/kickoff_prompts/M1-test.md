...
5. Create branch `mp-m1-migration-test` from `feat/p0-platform-stability-macOS-parity`; run `git worktree add wt/mp-m1-migration-test mp-m1-migration-test`.
...
2. Inside `wt/mp-m1-migration-test`, commit M1-test changes to branch `mp-m1-migration-test` (no docs/tasks/session_log.md edits).
3. From outside the worktree, ensure branch `mp-m1-migration-test` contains the worktree commit (fast-forward if needed); do **not** merge into `feat/p0-platform-stability-macOS-parity`.

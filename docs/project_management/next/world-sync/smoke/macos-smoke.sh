...
case "$SLICE_ID" in
  WS2)
    ...
    if [[ $rc -ne 0 ]]; then
      if should_skip_for_output "$out"; then
        echo "SKIP: world-sync macOS smoke ($SLICE_ID): missing/old world backend prerequisites (workspace sync --verbose)"
        printf '%s\n' "$out"
        exit 0
      fi
      echo "FAIL: world-sync macOS smoke ($SLICE_ID): workspace sync --verbose (exit=$rc)" >&2
      printf '%s\n' "$out" >&2
      exit "$rc"
    fi
  ...
  WS5)
    ...
    if [[ $rc -ne 0 ]]; then
      if should_skip_for_output "$out"; then
        echo "SKIP: world-sync macOS smoke ($SLICE_ID): missing/old world backend prerequisites (workspace sync both --verbose)"
        printf '%s\n' "$out"
        exit 0
      fi
      echo "FAIL: world-sync macOS smoke ($SLICE_ID): workspace sync both --verbose (exit=$rc)" >&2
      printf '%s\n' "$out" >&2
      exit "$rc"
    fi

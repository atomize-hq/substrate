# Kickoff: WS7-integ-windows (integration platform-fix — windows)
...
2. Verify you are in the task worktree `wt/world-sync-ws7-integ-windows` on branch `world-sync-ws7-integ-windows` and that `.taskmeta.json` exists at the worktree root.
...
4. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/next/world-sync" TASK_ID="WS7-integ-windows" TASK_PLATFORM=windows`
...
- Use the advisory CI audit to avoid redundant dispatch:
  - Ledger path (recommended; not committed): `docs/project_management/next/world-sync/logs/WS7/ci-audit/ledger.jsonl`
  - Feature Smoke audit (for a single platform):
    - `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/next/world-sync/logs/WS7/ci-audit/ledger.jsonl" --kind feature-smoke --orch-branch "feat/world-sync" --required-platforms windows`
  ...
  - CI Testing audit (if you are about to dispatch CI Testing):
    - `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/next/world-sync/logs/WS7/ci-audit/ledger.jsonl" --kind ci-testing --orch-branch "feat/world-sync"`
- If this is a behavioral platform (P3-008), run a local behavioral smoke preflight before dispatching CI smoke:
  ...
  - If `windows` is in `behavior_platforms_required`, run Feature Smoke for this platform (repeat until green if you make fixes):
    - `make feature-smoke FEATURE_DIR="docs/project_management/next/world-sync" PLATFORM=windows SMOKE_SLICE_ID="WS7" SMOKE_CHECKOUT_REF="$(git rev-parse HEAD)" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/world-sync" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=1`
    ...
    - After dispatch completes, record evidence (recommended):
      ...
      - `scripts/ci-audit/ci_audit_record.sh --ledger-path "docs/project_management/next/world-sync/logs/WS7/ci-audit/ledger.jsonl" --kind feature-smoke --orch-branch "feat/world-sync" --run-id "$RUN_ID" --tested-sha "$(git rev-parse HEAD)" --required-platforms windows`
    - If this is the Linux task and WSL coverage is required (see `tasks.json` meta: `wsl_required` + `wsl_task_mode`):
      - Bundled (default): dispatch with `RUN_WSL=1`:
        - `make feature-smoke FEATURE_DIR="docs/project_management/next/world-sync" PLATFORM=linux RUN_WSL=1 SMOKE_SLICE_ID="WS7" SMOKE_CHECKOUT_REF="$(git rev-parse HEAD)" RUNNER_KIND=self-hosted WORKFLOW_REF="feat/world-sync" REMOTE=origin CLEANUP=1 RUN_INTEG_CHECKS=1`
        - After dispatch completes, record evidence (recommended):
          - Set `RUN_ID` to the numeric id in the Actions run URL (example: `.../actions/runs/123456789`).
          - `scripts/ci-audit/ci_audit_record.sh --ledger-path "docs/project_management/next/world-sync/logs/WS7/ci-audit/ledger.jsonl" --kind feature-smoke --orch-branch "feat/world-sync" --run-id "$RUN_ID" --tested-sha "$(git rev-parse HEAD)" --required-platforms linux,wsl`
  - Otherwise, do **not** run Feature Smoke for this platform. Instead, treat this task as CI parity-only:
    ...
    - After dispatch completes, record evidence (recommended):
      - Set `RUN_ID` to the numeric id in the Actions run URL (example: `.../actions/runs/123456789`).
      - `scripts/ci-audit/ci_audit_record.sh --ledger-path "docs/project_management/next/world-sync/logs/WS7/ci-audit/ledger.jsonl" --kind ci-testing --orch-branch "feat/world-sync" --run-id "$RUN_ID" --tested-sha "$(git rev-parse HEAD)"`
...
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WS7-integ-windows"`

...
1. Verify you are in the task worktree `wt/world-sync-ws2-test` on branch `world-sync-ws2-test` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start-pair FEATURE_DIR="docs/project_management/next/world-sync" SLICE_ID="WS2"` (preferred; starts code+test in parallel; `WS2` is `WS2-test` without `-test`)
   - `make triad-task-start FEATURE_DIR="docs/project_management/next/world-sync" TASK_ID="WS2-test"` (single task only)
...
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WS2-test"`

...
1. Verify you are in the task worktree `wt/world-sync-ws6-code` on branch `world-sync-ws6-code` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start-pair FEATURE_DIR="docs/project_management/next/world-sync" SLICE_ID="WS6"` (preferred; starts code+test in parallel; `WS6` is `WS6-code` without `-code`)
   - `make triad-task-start FEATURE_DIR="docs/project_management/next/world-sync" TASK_ID="WS6-code"` (single task only)
...
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WS6-code"`

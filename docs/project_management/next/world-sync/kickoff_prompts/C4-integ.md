...
- Run `cargo fmt`, `cargo clippy --workspace --all-targets -- -D warnings`, relevant PTY tests, then `make integ-checks`.
...
5. Update tasks.json (C4-integ status), add END entry to session_log.md (commands/results/blockers).
6. Commit docs (`docs: finish C4-integ`). Remove worktree if done.

...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start-pair FEATURE_DIR="docs/project_management/next/world-sync" SLICE_ID="WS3"` (preferred; starts code+test in parallel; `WS3` is `WS3-code` without `-code`)
   - `make triad-task-start FEATURE_DIR="docs/project_management/next/world-sync" TASK_ID="WS3-code"` (single task only)

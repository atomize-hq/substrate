# Kickoff: WS3-integ (integration)

## Scope
- Merge code + tests, resolve drift to spec, and make the slice green.
- Spec: `docs/project_management/next/world-sync/WS3-spec.md`
- Execution workflow standard: `docs/project_management/standards/TASK_TRIADS_WORKTREE_EXECUTION_STANDARD.md`

## Start Checklist
Do not edit planning docs inside the worktree.

1. Verify you are in the task worktree `wt/world-sync-ws3-integ` on branch `world-sync-ws3-integ` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/next/world-sync" TASK_ID="WS3-integ"`
...
- Complete the slice closeout gate report:
  - `docs/project_management/next/world-sync/WS3-closeout_report.md` (e.g., `docs/project_management/next/world-sync/WS3-closeout_report.md`)

### CI checkpoints (when applicable)

If this is a cross-platform automation pack (and `docs/project_management/next/world-sync/ci_checkpoint_plan.md` exists):
...
2. From inside the worktree, run: `make triad-task-finish TASK_ID="WS3-integ"`

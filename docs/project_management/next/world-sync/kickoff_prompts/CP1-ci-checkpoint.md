...
4. Determine the exact commit that this checkpoint validates:
   ...
   - Compute `CHECKOUT_SHA` from `tasks.json` without checking out the branch:
     - `CORE_BRANCH="$(jq -r --arg id "WS2-integ-core" '.tasks[] | select(.id==$id) | .git_branch' "docs/project_management/next/world-sync/tasks.json")"`
...
- Ledger path (not committed): `docs/project_management/next/world-sync/logs/WS2/ci-audit/ledger.jsonl`
- CI Testing audit:
...
- Feature Smoke audit:
- `scripts/ci-audit/ci_audit.sh --ledger-path "docs/project_management/next/world-sync/logs/WS2/ci-audit/ledger.jsonl" --kind feature-smoke --orch-branch "feat/world-sync" --feature-dir "docs/project_management/next/world-sync"`

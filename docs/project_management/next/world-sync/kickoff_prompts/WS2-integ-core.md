# Kickoff: WS2-integ-core (integration core)
...
1. Verify you are in the task worktree `wt/world-sync-ws2-integ-core` on branch `world-sync-ws2-integ-core` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/next/world-sync" TASK_ID="WS2-integ-core"`
...
If `docs/project_management/next/world-sync/smoke/` exists and this machine matches a behavior platform for this feature, run the matching smoke script locally before finishing this task.
...
bash "docs/project_management/next/world-sync/smoke/linux-smoke.sh"
```
...
1. Ensure your merged state is committed and local integration gates are green:
   - From inside the worktree, run: `make triad-task-finish TASK_ID="WS2-integ-core"`

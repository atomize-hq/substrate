# Kickoff: WS7-integ (integration final — cross-platform merge)
...
1. Verify you are in the task worktree `wt/world-sync-ws7-integ` on branch `world-sync-ws7-integ` and that `.taskmeta.json` exists at the worktree root.
...
3. If `.taskmeta.json` is missing or mismatched, stop and ask the operator to run:
   - `make triad-task-start FEATURE_DIR="docs/project_management/next/world-sync" TASK_ID="WS7-integ"`
...
- Verify that the checkpoint(s) that cover this slice are completed and that run ids/URLs are recorded in `docs/project_management/next/world-sync/session_log.md`.
- Complete the slice closeout gate report:
  - `docs/project_management/next/world-sync/WS7-closeout_report.md` (e.g., `docs/project_management/next/world-sync/WS7-closeout_report.md`)

## End Checklist
1. Ensure your merged state is committed and local integration gates are green:
   - From inside the worktree, run: `make triad-task-finish TASK_ID="WS7-integ"`
...
Naming note:
- The task id for the final aggregator is `WS7-integ` (this prompt’s `WS7-integ`). The helper command to start it is named `triad-task-start-integ-final`.

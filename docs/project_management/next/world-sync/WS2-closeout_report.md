...
- Existing behavior: `substrate workspace sync --dry-run --direction from_world` could preview pending diffs, but apply semantics for `--direction from_world` (without `--dry-run`) were not yet implemented.
...
- Links:
  ...
  - CI checkpoint evidence recorded in `docs/project_management/next/world-sync/session_log.md`
...
Notes:
- Checkpoint CP1 gates (compile parity + Feature Smoke + ci-testing quick) were validated for `CHECKOUT_SHA=136d6814a650066c58e09c43a1d849da1cdbbb8f` and recorded in `docs/project_management/next/world-sync/session_log.md`.

# Task R9c-integ (Routing split: world/agent flows) – INTEGRATION
...
3. Confirm `R9c-code` and `R9c-test` are completed.
4. Set `R9c-integ` to `in_progress` in `tasks.json`, add a START entry to
   `session_log.md`, and commit the docs update (`git commit -am "docs: start R9c-integ"`).
5. Create the task branch and worktree:
   ```
   git checkout -b cr-r9c-routing-integ
   git worktree add wt/cr-r9c-routing-integ cr-r9c-routing-integ
   cd wt/cr-r9c-routing-integ
   ```
   Do not edit docs/tasks/session logs from the worktree.

## Spec
- Merge the R9c code and test branches for routing world/agent/platform split.
...
3. Return to repo root, merge the integration branch:
   ```
   git checkout cr-r9c-routing-integ
   git merge --ff-only wt/cr-r9c-routing-integ   # if needed
   git checkout feat/crate-refactor
   git pull --ff-only
   git merge --ff-only cr-r9c-routing-integ
   ```
...
5. Commit docs/log updates on `feat/crate-refactor`
   (`git commit -am "docs: finish R9c-integ"`).
6. Remove the worktree (`git worktree remove wt/cr-r9c-routing-integ`) if done and

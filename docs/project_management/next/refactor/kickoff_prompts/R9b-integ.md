# Task R9b-integ (Routing split: path/env/cwd helpers) – INTEGRATION
...
3. Confirm `R9b-code` and `R9b-test` are completed.
4. Set `R9b-integ` to `in_progress` in `tasks.json`, add a START entry to
   `session_log.md`, and commit the docs update (`git commit -am "docs: start R9b-integ"`).
5. Create the task branch and worktree:
   ```
   git checkout -b cr-r9b-routing-integ
   git worktree add wt/cr-r9b-routing-integ cr-r9b-routing-integ
   cd wt/cr-r9b-routing-integ
   ```
   Do not edit docs/tasks/session logs from the worktree.

## Spec
- Merge the R9b code and test branches for routing path/cwd/env helper split.
...
3. Return to repo root, merge the integration branch:
   ```
   git checkout cr-r9b-routing-integ
   git merge --ff-only wt/cr-r9b-routing-integ   # if needed
   git checkout feat/crate-refactor
   git pull --ff-only
   git merge --ff-only cr-r9b-routing-integ
   ```
...
5. Commit docs/log updates on `feat/crate-refactor`
   (`git commit -am "docs: finish R9b-integ"`).
6. Remove the worktree (`git worktree remove wt/cr-r9b-routing-integ`) if done and

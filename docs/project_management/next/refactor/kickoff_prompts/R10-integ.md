# Task R10-integ (PTY IO module slicing) – INTEGRATION
...
3. Confirm `R10-code` and `R10-test` are completed.
4. Set `R10-integ` to `in_progress` in `tasks.json`, add a START entry to
   `session_log.md`, and commit the docs update (`git commit -am "docs: start R10-integ"`).
5. Create the task branch and worktree:
   ```
   git checkout -b cr-r10-pty-integ
   git worktree add wt/cr-r10-pty-integ cr-r10-pty-integ
   cd wt/cr-r10-pty-integ
   ```
   Do not edit docs/tasks/session logs from the worktree.

## Spec
- Merge the R10 code and test branches for the pty/io module split.
...
3. Return to repo root, merge the integration branch:
   ```
   git checkout cr-r10-pty-integ
   git merge --ff-only wt/cr-r10-pty-integ   # if needed
   git checkout feat/crate-refactor
   git pull --ff-only
   git merge --ff-only cr-r10-pty-integ
   ```
...
5. Commit docs/log updates on `feat/crate-refactor`
   (`git commit -am "docs: finish R10-integ"`).
6. Remove the worktree (`git worktree remove wt/cr-r10-pty-integ`) if done and

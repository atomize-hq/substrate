...
4. Set `C0-integ-windows` status to `in_progress` in tasks.json; add START entry to session_log.md; commit docs (`docs: start C0-integ-windows`).
5. Create branch `c0-integ-windows`; create worktree `wt/tmp-make-scaffold-c0-integ-windows`.

## Requirements
- First, validate platform smoke via CI for this platform:
  - `scripts/ci/dispatch_feature_smoke.sh --feature-dir "docs/project_management/next/tmp-make-scaffold" --runner-kind self-hosted --platform windows --cleanup`
...
4. Update tasks.json + session_log.md END entry; commit docs (`docs: finish C0-integ-windows`); remove worktree.

...
4. Set `C0-integ-macos` status to `in_progress` in tasks.json; add START entry to session_log.md; commit docs (`docs: start C0-integ-macos`).
5. Create branch `c0-integ-macos`; create worktree `wt/tmp-make-scaffold-c0-integ-macos`.

## Requirements
- First, validate platform smoke via CI for this platform:
  - `scripts/ci/dispatch_feature_smoke.sh --feature-dir "docs/project_management/next/tmp-make-scaffold" --runner-kind self-hosted --platform macos --cleanup`
...
4. Update tasks.json + session_log.md END entry; commit docs (`docs: finish C0-integ-macos`); remove worktree.

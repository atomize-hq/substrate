# Task I7-integ (Manual playbook alignment) – INTEGRATION
...
3. Set `I7-integ` to `in_progress`, append START entry to `session_log.md`, commit docs (`docs: start I7-integ`).
4. Create branch/worktree:
   ```
   git checkout -b ahih-i7-playbook-align-integ
   git worktree add wt/ahih-i7-playbook-align-integ ahih-i7-playbook-align-integ
   cd wt/ahih-i7-playbook-align-integ
   ```
5. Do not edit docs/tasks/session_log.md inside the worktree.

## Duties
- Merge `ahih-i7-playbook-align-code` and `ahih-i7-playbook-align-test`.
- Reconcile any drift so behavior matches `I7-spec.md`.
...
4. Update `tasks.json` + `session_log.md` (END entry) and commit docs (`docs: finish I7-integ`).
5. Remove worktree.

...
- Interactive REPL (`substrate` with no `--command`/`-c`):
  ...
  - `exit` / `quit`: exits the REPL; Substrate shuts down the world session as part of cleanup.
    ...
    - If the interactive terminal is revoked/disconnected after REPL startup (for example, controlling-TTY loss during async REPL input), the REPL process MUST terminate with exit code `1` because this is an abnormal runtime failure, not a normal operator exit.
    - On that abnormal path, Substrate MAY attempt one best-effort stderr diagnostic before exit. The landed regression proof for this contract is the macOS revoke harness in `crates/shell/tests/repl_tty_disconnect_macos.rs`; this ADR does not claim broader platform proof beyond that landed evidence.
...
- Exit codes:
  ...
  - That abnormal path may emit one best-effort stderr diagnostic, but the operator-facing contract remains the exit-code split rather than a platform-wide guarantee about how the diagnostic is surfaced.

...
- Slices `39` through `41` keep retained-worker approval/fork/follow-up/blocked obligation bootstrap and typed host approval-response bootstrap on that same internal surface through six deny-by-default keys: `agents.world_dispatch.obligations.approval_allowed`, `agents.world_dispatch.obligations.approval_response_allowed`, `agents.world_dispatch.obligations.follow_up_allowed`, `agents.world_dispatch.obligations.blocked_allowed`, `agents.world_dispatch.fork.requests_allowed`, and `agents.world_dispatch.fork.recommendations_allowed`.
...
- All six approval/fork/follow-up/blocked gates default to `false`. When the worker-event keys are `false`, retained-worker `approval_request`, `fork_request`, `fork_recommendation`, `follow_up_question`, and `blocked` events emitted during `continue_world_worker` fail closed even if `continue_world_worker` itself is allowlisted. When `approval_response_allowed=false`, typed `continue_world_worker` `approval_response` payloads fail closed even if generic prompt-based continue remains allowlisted.

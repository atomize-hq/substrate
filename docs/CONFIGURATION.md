...
- Slice `48` lands the first internal host-side router-owned local-session auto-attach execution boundary. It remains deny-by-default: `workflow.router.enabled` must be true, and the claimed obligation kind must also be allowed by the existing `agents.world_dispatch.obligations.approval_allowed`, `agents.world_dispatch.obligations.follow_up_allowed`, `agents.world_dispatch.obligations.blocked_allowed`, or `agents.world_dispatch.fork.requests_allowed` gate as applicable.
- Slice `49` adds no new policy keys, but it widens the local obligation envelope with bounded `origin_host_id` and `target_host_id` truth, stamps exact local host identity on locally produced obligations when available, and makes the router-owned auto-attach path fail closed with `wrong_target_host` if a local obligation explicitly targets a different host. This remains local-session-only and does not land `SUBSTRATE_HOME/host_inbox/`, remote ingress materialization, or cross-host delivery.
- Slice `50` adds no new policy keys, but it widens that same local obligation envelope with ingress-ready identity and causation fields: `ingress_source_kind`, `ingress_source_id`, `ingress_received_at`, `causation_event_id`, `causation_message_id`, and `causation_request_id`. The current local retained-worker producer stamps exact `local_runtime` ingress classification plus exact `request.run_id` receipt/request causation when known, preserves exact event/message ids only when the worker payload surfaces them directly, and keeps `thread_id` as payload-only metadata rather than promoting it to canonical message identity. This remains local-session-only and does not land `SUBSTRATE_HOME/host_inbox/`, remote ingress materialization, or cross-host delivery.
- Slice `51` adds no new policy keys, but it does land the first bounded `SUBSTRATE_HOME/host_inbox/` layer above the canonical local obligation ledger. Host-inbox records persist exact host-targeting, ingress, and causation truth plus durable `pending` / `materialized` / `failed_closed` materialization state, and a bounded host-side materialization pre-pass converts exact local records into canonical local obligations before router-owned auto-attach discovery runs.
- Slice `51` preserves the same hard boundaries: local obligations remain the canonical session-local deferred-work truth, router-owned auto-attach still consumes obligations only, wrong-host or otherwise invalid host-inbox records fail closed without creating local obligations, and this slice does not widen into new public CLI or daemon UX, new policy keys, remote sync, lease coordination, or broader cross-host delivery/federation mechanics.
...
- Slice `40` adds one more deny-by-default host-response gate under that same patch surface: `agents.world_dispatch.obligations.approval_response_allowed`.
- Slice `41` adds two more deny-by-default retained-worker obligation gates under that same patch surface: `agents.world_dispatch.obligations.follow_up_allowed` and `agents.world_dispatch.obligations.blocked_allowed`.
- Slice `42` adds one more deny-by-default host-response gate under that same patch surface: `agents.world_dispatch.obligations.clarification_response_allowed`.
- Slice `43` adds one more deny-by-default host-control gate under that same patch surface: `agents.world_dispatch.control.control_directives_allowed`.
- Slice `46` adds one more deny-by-default host progress-ack gate under that same patch surface: `agents.world_dispatch.control.progress_acks_allowed`.
- Slice `45` adds one more deny-by-default host-fork-command gate under that same patch surface: `agents.world_dispatch.fork.commands_allowed`.
- Slice `47` adds no new policy keys, but it does widen the live internal dispatch contract: active `run_world_task` now surfaces exact runtime-owned `task_run_id` truth, `inspect_world_worker` accepts `mode=ephemeral` only with that exact `task_run_id`, and `cancel_world_work` accepts `mode=ephemeral` only with that exact `task_run_id`; retained inspect/cancel still require exact `target_participant_id`.
- Slice `48` adds no new policy keys, but it does land the first internal host-side router-owned local-session auto-attach boundary. That path remains deny-by-default and requires `workflow.router.enabled=true` plus the relevant existing retained-obligation gate (`agents.world_dispatch.obligations.approval_allowed`, `agents.world_dispatch.obligations.follow_up_allowed`, `agents.world_dispatch.obligations.blocked_allowed`, or `agents.world_dispatch.fork.requests_allowed` as applicable). It is Linux-only in v1, local-session-only, continuity-first on persisted attach truth, and does not widen into a public router daemon UX, cross-host inbox, worker continuation, or review-state resolution.
- Slice `49` adds no new policy keys, but it does widen that same local-session obligation boundary with bounded `origin_host_id` and `target_host_id` truth and an exact local-host eligibility check on router-owned auto-attach. Foreign-targeted local obligations now fail closed with an explanation-ready `wrong_target_host` outcome before claim or launch; untargeted and same-host obligations preserve the landed Slice `48` behavior. This remains Linux-only in v1, local-session-only, and does not widen into `host_inbox`, remote ingress, or broader federation mechanics.
- Slice `50` adds no new policy keys, but it does widen that same local-session obligation boundary with ingress-ready identity and causation truth: `ingress_source_kind`, `ingress_source_id`, `ingress_received_at`, `causation_event_id`, `causation_message_id`, and `causation_request_id`. On the current Linux retained-worker producer path, locally persisted obligations stamp exact `local_runtime` ingress classification, exact `request.run_id` receipt/request-causation truth, and preserve event/message causation only when exact ids are surfaced directly in worker payload metadata. Missing exact event/message ids remain absent, `thread_id` remains payload-only thread identity, and this slice does not widen into `host_inbox`, remote ingress, or broader federation mechanics.
- Slice `51` adds no new policy keys, but it does land the first bounded host-global inbox layering seam under `SUBSTRATE_HOME/host_inbox/`. Host-inbox records remain a host-level ingress/materialization layer above canonical local obligations, persist durable `pending` / `materialized` / `failed_closed` outcome truth, and are materialized for the exact local host before router-owned auto-attach discovery evaluates canonical obligations.
- All ten approval/fork/follow-up/blocked/clarification/control/progress-ack gates default to `false`. When the worker-event keys are `false`, retained-worker `approval_request`, `fork_request`, `fork_recommendation`, `follow_up_question`, and `blocked` events emitted during `continue_world_worker` fail closed even if `continue_world_worker` itself is allowlisted. When `approval_response_allowed=false`, typed `continue_world_worker` `approval_response` payloads fail closed even if generic prompt-based continue remains allowlisted. When `clarification_response_allowed=false`, typed `continue_world_worker` `clarification_response` payloads fail closed even if generic prompt-based continue remains allowlisted. When `control_directives_allowed=false`, typed `continue_world_worker` `control_directive` payloads fail closed even if generic prompt-based continue remains allowlisted. When `progress_acks_allowed=false`, typed `continue_world_worker` `progress_ack` payloads fail closed even if generic prompt-based continue remains allowlisted. When `commands_allowed=false`, typed `continue_world_worker` `fork_command` payloads fail closed even if generic prompt-based continue remains allowlisted.
- Enabling the retained-worker event keys admits only durable local obligation production for retained-worker `approval_request`, `fork_request`, `fork_recommendation`, `follow_up_question`, and `blocked`.
- Enabling `approval_response_allowed` admits only the narrow typed host `approval_response` bootstrap over the existing `continue_world_worker` prompt seam with exact unresolved `ApprovalRequired` obligation binding and post-delivery closeout. It does not widen the public control surface, add broader control-directive/control-ack handling, allow child auto-allocation from worker-issued fork events, or imply a broader typed transport redesign.
- Enabling `clarification_response_allowed` admits only the narrow typed host `clarification_response` bootstrap over the existing `continue_world_worker` prompt seam with exact unresolved `FollowUpRequired` obligation binding, deterministic implementation-owned prompt rendering, and post-delivery closeout. It does not widen the public control surface, add broader host response/control classes, allow worker-issued fork execution, or imply a broader typed transport redesign.
- Enabling `control_directives_allowed` admits only the narrow typed host `control_directive` bootstrap over the existing `continue_world_worker` prompt seam with exact retained-worker identity binding and deterministic implementation-owned prompt rendering, plus the paired immediate retained-worker `control_ack` acknowledgement on that same live stream. It does not widen the public control surface, add typed `fork_command`, allow worker-issued fork execution, or imply generalized control transport or Family-2 router execution.
- Enabling `progress_acks_allowed` admits only the narrow typed host `progress_ack` bootstrap over the existing `continue_world_worker` prompt seam with exact retained-worker identity binding, deterministic implementation-owned prompt rendering, and delivery-only acknowledgement semantics. It does not widen the public control surface, create durable progress review state, imply completion or pause, or imply generalized typed host transport or Family-2 router execution.
- Enabling `fork.commands_allowed` admits only the narrow typed host `fork_command` bootstrap over the existing `continue_world_worker` prompt seam with exact retained-worker identity binding, deterministic implementation-owned prompt rendering, and reuse of the already-landed exact-source `fork_world_worker` bootstrap path to allocate at most one retained child with explicit source-to-child lineage. It does not widen the public control surface, allow worker-issued fork execution or auto-fork, or imply generalized typed host transport or Family-2 router execution.
...
- `inspect_world_worker` remains internal and dual-target in Slice `47`. In `mode=retained`, it still requires exact `target_participant_id` and returns an authoritative store-backed retained-worker snapshot without invoking world-side execution transport. In `mode=ephemeral`, it requires the exact active `task_run_id` surfaced by in-flight `run_world_task` execution and returns an authoritative active snapshot without mutating lifecycle state. Routed snapshot delivery is currently supported only on Linux in v1; non-Linux builds fail closed with `unsupported_platform_or_posture`.
- `cancel_world_work` remains internal and distinct from `stop_world_worker`. In `mode=retained`, it still requires exact `target_participant_id` and uses the dedicated private owner cancel surface to interrupt active retained work in flight and wait for authoritative cancelled closeout. In `mode=ephemeral`, it requires the exact active `task_run_id` surfaced by in-flight `run_world_task` execution and interrupts that one exact in-flight task with truthful cancel closeout distinct from retained cancel and stop semantics. Linux is the routed v1 platform; non-Linux builds fail closed with `unsupported_platform_or_posture`.
...
- Outside the ten narrow Slice `39`/`40`/`41`/`42`/`43`/`45`/`46` approval/fork/follow-up/blocked/clarification/control/progress-ack gates above plus Slice `48`/`49`'s explicit `workflow.router.enabled` conjunction with the relevant existing retained-obligation gate and exact same-host obligation targeting checks, Slice `50`'s local-only ingress/causation envelope widening, and Slice `51`'s bounded `host_inbox -> local obligation -> router` layering, this patch surface does not imply broader typed host response/control classes beyond the landed `approval_response`, `clarification_response`, `control_directive`, `progress_ack`, and `fork_command` bootstraps plus immediate same-stream worker `control_ack`, routed retained-child bootstrap reuse, retained promotion or future continuation of ephemeral work, auto-fork, remote ingress sync, lease coordination, cross-host delivery, or broader Family-2 autonomy/execution policy.
...
agents:
  world_dispatch:
    ...
    obligations:
      ...
      approval_response_allowed: false
      follow_up_allowed: false
      blocked_allowed: false
      clarification_response_allowed: false
    control:

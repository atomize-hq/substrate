...
- Slices `39` and `40` keep retained-worker approval/fork obligation bootstrap and typed host approval-response bootstrap on that same internal surface through four deny-by-default keys: `agents.world_dispatch.obligations.approval_allowed`, `agents.world_dispatch.obligations.approval_response_allowed`, `agents.world_dispatch.fork.requests_allowed`, and `agents.world_dispatch.fork.recommendations_allowed`.
...
- All four approval/fork gates default to `false`. When the worker-event keys are `false`, retained-worker `approval_request`, `fork_request`, and `fork_recommendation` events emitted during `continue_world_worker` fail closed even if `continue_world_worker` itself is allowlisted. When `approval_response_allowed=false`, typed `continue_world_worker` `approval_response` payloads fail closed even if generic prompt-based continue remains allowlisted.
- Enabling the worker-event keys admits only durable local obligation production for retained-worker `approval_request`, `fork_request`, and `fork_recommendation`.

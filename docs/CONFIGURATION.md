...
- Enabling `control_directives_allowed` admits only the narrow typed host `control_directive` bootstrap over the existing `continue_world_worker` prompt seam with exact retained-worker identity binding and deterministic implementation-owned prompt rendering. It does not widen the public control surface, add typed `control_ack` or `fork_command`, widen active-ephemeral task identity, allow worker-issued fork execution, or imply generalized control transport or Family-2 router execution.

...
- All seven approval/fork/follow-up/blocked/clarification gates default to `false`. When the worker-event keys are `false`, retained-worker `approval_request`, `fork_request`, `fork_recommendation`, `follow_up_question`, and `blocked` events emitted during `continue_world_worker` fail closed even if `continue_world_worker` itself is allowlisted. When `approval_response_allowed=false`, typed `continue_world_worker` `approval_response` payloads fail closed even if generic prompt-based continue remains allowlisted. When `clarification_response_allowed=false`, typed `continue_world_worker` `clarification_response` payloads fail closed even if generic prompt-based continue remains allowlisted.
...
- Outside the seven narrow Slice `39`/`40`/`41`/`42` approval/fork/follow-up/blocked/clarification gates above, this patch surface does not imply broader typed host response/control classes beyond the landed `approval_response` and `clarification_response` bootstraps, deferred host `progress_ack`, deferred host `control_directive`, deferred host `control_ack`, deferred host-to-worker `fork_command` control class, auto-fork, active-ephemeral inspect, active-ephemeral or dual-target cancel semantics, router-owned attach execution, or broader Family-2 autonomy/execution policy.

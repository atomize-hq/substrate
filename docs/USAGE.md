...
- For the direct `cli:codex-world` member runtime, the host-side effective policy must also allow `cli:codex-world` under `agents.host_credentials.read.allowed_backends`; the launch path seeds an isolated per-member `CODEX_HOME` from the host `~/.codex` login artifacts before the world member starts.

...
- `substrate agent toolbox status` is a pre-runtime introspection surface: it projects the effective toolbox posture, the selected orchestrator identity, and either the active per-session UDS endpoint or the deterministic endpoint template when no orchestrator session is active yet. On Unix hosts, the endpoint path falls back to a short deterministic `/tmp/substrate-agent-toolbox/...` location when the normal `SUBSTRATE_HOME/run/agent-toolbox/...` UDS path would exceed platform socket-length limits.
...
- Fresh toolbox world-dispatch calls become actionable only when that live host orchestrator session already carries `active_world_binding`. The supported public bootstrap for that posture is `substrate agent start --backend <placement-qualified world backend> --scope world`; a plain host-only start still exposes the toolbox contract for discovery, but fresh `run_world_task` / `spawn_world_worker` calls fail closed with `missing_world_binding`.
- For the current direct `cli:codex-world` member runtime, the host-side effective policy must also allow `cli:codex-world` under `agents.host_credentials.read.allowed_backends`; when that exact backend is allowlisted, the launch path materializes an isolated per-member `CODEX_HOME` compatibility bridge from the host `~/.codex`.
- That compatibility bridge is intentionally transitional and narrow for the current file-backed direct-member path: it seeds auth artifacts plus only the bounded non-secret startup config needed for truthful launch, and it is not the final gateway-front-door architecture or a generic future-capability projection base.
- In full-isolation worlds, internal world-dispatch writes follow the ordinary pending-diff contract: relative workspace writes land in the world view first and become host-visible only after `substrate workspace sync --direction from_world` (or equivalent auto-sync); absolute host workspace paths are not valid writable targets from inside the member runtime.
- Of the two internal verbs, `spawn_world_worker` validates a streamed `registered` event, returns an authoritative receipt, and persists retained-worker identity into live runtime/session state that `substrate agent status` can read, while `run_world_task` remains a terminal one-shot bootstrap and does not currently expose a first-class dedicated toolbox trace stream.
...
substrate agent start --backend <backend_id> --scope world --disable-capability event_stream --prompt "hello" --json

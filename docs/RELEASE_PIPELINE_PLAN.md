...
- Accepts both `vMAJOR.MINOR.PATCH` and bare semver tags so promotion can continue using the `v` prefix while still supporting the original pattern.
- **Jobs** (generated by `dist init`):
  ...
  2. **Build Matrix**:
     ...
     - Linux runners install `libseccomp-dev` (and `libseccomp-dev:arm64` for
       `aarch64-unknown-linux-gnu`) before invoking `dist build` to satisfy the
       linker requirements added by the world crate.
  3. **Host** (Ubuntu):

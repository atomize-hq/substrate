...
| Diagnostics surface area | `substrate host doctor` reports host prerequisites + transport readiness; `substrate world doctor` includes the host report plus guest-kernel facts from the world-agent (`landlock`, overlayfs probe). | `substrate host doctor (macOS)` focuses on Lima + transport + guest service status; `substrate world doctor (macOS)` includes the host report plus guest-kernel facts from the world-agent (`landlock`, overlayfs probe). |

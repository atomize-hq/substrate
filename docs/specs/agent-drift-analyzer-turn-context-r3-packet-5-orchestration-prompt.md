/goal Land Packet `R3-5` from `docs/specs/agent-drift-analyzer-turn-context-r3-tasks.md` in `/Users/spensermcconnell/.codex/worktrees/97a0/substrate` using a packet-scoped implementation -> commit -> review -> fix -> commit loop until the packet is review-clean.

You are the orchestration agent. Stay strictly scoped to Packet `R3-5` only, assuming Packets `R3-1` through `R3-4` are already landed.
...
Packet `R3-5` scope only:

- extend analyzer `summary.md` with compact turn-context inspection
- preserve the landed calibration metrics and their meanings
- keep turn-context rendering concise and operator-readable

Out of scope:

- checkpoint-contract changes beyond what earlier packets already landed
- sentinel compatibility or presentation work
- `R3-6+`
...
5. Commit the landed Packet `R3-5` implementation before any review subagent is dispatched.
...
12. Do not start Packet `R3-6`.

Required implementation verification wall for Packet `R3-5`:
...
```text
/goal Implement Packet R3-5 only from `docs/specs/agent-drift-analyzer-turn-context-r3-tasks.md` in `/Users/spensermcconnell/.codex/worktrees/97a0/substrate`, assuming Packets R3-1 through R3-4 are already landed.

Use the `$incremental-implementation` skill.

You are landing only Packet `R3-5`:
- extend analyzer `summary.md` with compact turn-context inspection
- preserve the landed calibration metrics and their meanings
- keep turn-context rendering concise and operator-readable
...
Execution rules:
- stay strictly within Packet `R3-5`
- do not redefine `turns observed` or `user prompts observed`
- render turn-context as an additive inspection layer, not a replacement for existing metrics
- do not start sentinel or `R3-6+` work
...
```text
/goal Review the already-landed Packet R3-5 implementation in `/Users/spensermcconnell/.codex/worktrees/97a0/substrate` against its spec, plan, and tasks, and determine whether it is ready to keep.

Use the `$code-review-and-quality` skill.

Review only Packet `R3-5` from:
- `docs/specs/agent-drift-analyzer-turn-context-r3-spec.md`
- `docs/specs/agent-drift-analyzer-turn-context-r3-plan.md`
- `docs/specs/agent-drift-analyzer-turn-context-r3-tasks.md`

Focus:
- correctness of summary rendering
- preservation of the landed calibration semantics
- readability and operator usefulness of the added turn-context output
...
```text
/goal Address the review findings for already-landed Packet R3-5 only in `/Users/spensermcconnell/.codex/worktrees/97a0/substrate`, then rerun the relevant Packet R3-5 verification commands.
...
Rules:
- fix only the flagged Packet `R3-5` issues
- do not broaden into Packet `R3-6+`
...
- implementation commit message example: `feat: render turn context in analyzer summary`
- fix commit message example: `fix: address r3-5 review findings`

Your job is done only when Packet `R3-5` is committed and a fresh review subagent reports it review-clean.

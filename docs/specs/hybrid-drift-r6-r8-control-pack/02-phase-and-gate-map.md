...
| `R6-REPLAY` | ACTIVE | Bounded real-rollout/replay closeout | SATISFIED — controls and all conditional fixes complete; authority transition series `56bb9966f` + `07a3b1fe5` fresh independent `REVIEW CLEAN` | OPEN — `CTX-R6-01` and `CTX-R6-02` complete; `R6-GAP-DET-REPLAY-STALL` implementation/proof commit `6eda87e60` fresh independent `REVIEW CLEAN`; active packet `none`; transition review pending. Current sticky `CTX-R6-06` authority is `HistoricalOnly / 20`, unflagged; old `Recovered / 20` is historical baseline only. After transition review-clean, run `CTX-R6-06` replay proof and the R6 family wall. |
...
`default` `REVIEW CLEAN`. `R6-GAP-DET-REPLAY-STALL` and `CTX-R6-02` are complete; `R6-REPLAY`
remains active with packet `none`. Sticky authority remains `HistoricalOnly / 20`, unflagged, and
`Recovered / 20` remains historical baseline only. This transition awaits fresh review; afterward
`CTX-R6-06` replay proof and the family wall are next. R6 closeout, terminal scorer dispositions,
and R7/R8 remain pending or blocked.

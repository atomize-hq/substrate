...
| `R6-REPLAY` | ACTIVE | Bounded real-rollout/replay closeout | SATISFIED — controls and all conditional fixes complete; authority transition series `56bb9966f` + `07a3b1fe5` fresh independent `REVIEW CLEAN` | OPEN — `CTX-R6-01` review-clean; committed-baseline `CTX-R6-02` witness `60cde3dd7`; Task `.1` receipt `d788f45c9` fresh independent `REVIEW CLEAN`; Task `.2` complete. Task `.2A` decision `R6-REPLAY-STALL-POST-PAIRING-PROGRESS-01` gates an uncommitted Task `.3` candidate that exposes progress/recovery reds; Task `.4`, `CTX-R6-06`, and family wall remain blocked. |
...
`R6-GAP-DET-REPLAY-STALL`. Task `.0` series `200725001` + `08fa86e94` + `d03f5a355` + `9edf564d3` and Task `.1` receipt `d788f45c9` received fresh independent built-in `default` `REVIEW CLEAN`; Task `.2` is complete. The uncommitted Task `.3` candidate makes pairing truthful but exposes `CTX-R6-02` progress and frozen `CTX-R6-06` recovery reds beyond the prior acceptance. Task `.2A` decision `R6-REPLAY-STALL-POST-PAIRING-PROGRESS-01` is current and Task `.4` is blocked. `CTX-R6-06`,

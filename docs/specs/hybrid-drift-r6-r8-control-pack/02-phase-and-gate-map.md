...
| `R8-SPEC` | ACTIVE — ENTRY ONLY | R8 MAP/SPEC/PLAN/TASKS | SATISFIED — R7 is closed with a stable analyzer contract; `CTX-R8-01` is `OPEN`; active packet is `none`; transition/fix series `b4b2149b2` + `bcb664f03` is fresh independent built-in `default` `CLEAN` | PENDING — R8 MAP/SPEC/PLAN/TASKS authoring is unchecked and unstarted. Prompt 1 selectors `PHASE_ID: R8-SPEC` / `ACTIVE_PACKET: none` are prepared and eligible but uninvoked. |
| `R8-IMPLEMENT` | BLOCKED / BOUNDARY ONLY | Sentinel interpretation consolidation/integration | BLOCKED — R8 docs are not authored or landed | Replay/live share one seam, compatibility is centralized, presentation stays presentation-first. |

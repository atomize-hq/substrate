...
**Ledger status:** ACTIVE — `R6-GAP-DET-OPAQUE-PARENT` (review-fix candidate `931e50c85` landed with green proof; fresh re-review and separate transition pending)

**Verified against:** `beed76446` (post-docs ledger reconciliation `REVIEW CLEAN`), initial production commit `bcd94bf4f`, and landed review-fix candidate `931e50c85` with the exact ordered proof below
...
| `CTX-R6-04` | Opaque delegated parent does not become child thrash | PROVEN FOCUSED / ACTIVE GAP | The preserved pre-edit control remained red at `0 / Medium / Cleared`, unflagged, empty evidence. Production commit `bcd94bf4f` made the exact control pass at required `0 / Low / Cleared`, unflagged, empty evidence, while preserving `ParentVisibleOrchestration`, but review found its exception too broad and the ledger state stale. Landed review-fix candidate `931e50c85` narrows the exception to typed `ParentVisibleOrchestration + Low` progress and adds a partial/mixed regression at `0 / Medium / Cleared`, unflagged, empty evidence. Ordered proof is green: the three focused controls each passed `1 / 0`; the `dead_end_thrash` command passed all `21` matching tests (`18` owning integration, `1` acceptance, `2` export); and the checkpoint wall passed all `167` matching tests (`35` unit, `131` checkpoints integration, `1` export), with zero failures. | Obtain fresh `REVIEW CLEAN` for the production series through `931e50c85`, and only then land and independently review the separate authority transition. Keep this named gap ACTIVE until that transition; no-code closure and successor activation remain unavailable here. | `R6-GAP-DET-OPAQUE-PARENT` |
...
| `CTX-R6-12` | Truth-path-touching action before read | PRESERVED RED / GAP BLOCKED | Exact control `truth_grounding_gap_flags_truth_path_action_before_read` declared one truth path and made the first write-like `apply_patch` action touch that same path before any read, but failed the locked triple: actual `0 / Medium / Cleared`, unflagged, versus required `80 / High / Active`, flagged, with authority and action evidence. | Preserve the committed witness; wait for `R6-GAP-DET-OPAQUE-PARENT` to complete and transition this distinct route active. | `R6-GAP-TGG-TRUTH-PATH-ACTION` |
...
|---|---|---|---|---|---|---|
| `R6-GAP-DET-OPAQUE-PARENT` | `CTX-R6-04` — `dead_end_thrash_keeps_opaque_parent_orchestration_clear_without_child_activity` | `docs/specs/r6/R6-GAP-DET-OPAQUE-PARENT/R6-GAP-DET-OPAQUE-PARENT-spec.md`<br>`docs/specs/r6/R6-GAP-DET-OPAQUE-PARENT/R6-GAP-DET-OPAQUE-PARENT-plan.md`<br>`docs/specs/r6/R6-GAP-DET-OPAQUE-PARENT/R6-GAP-DET-OPAQUE-PARENT-tasks.md` | ACTIVE | `R6-C.1-CONTROLS` | `R6-GAP-TGG-TRUTH-PATH-ACTION` | Witness `87409b39a`; controls wall `5618f7864`; packet docs `59092df2a` `REVIEW CLEAN`; ledger reconciliation `beed76446` `REVIEW CLEAN`; initial production commit `bcd94bf4f`; landed review-fix candidate `931e50c85`. Its ordered proof remains green: all three focused controls passed, all `21` matching `dead_end_thrash` tests passed, and all `167` matching checkpoint tests passed with zero failures. Fresh re-review found one P2 because the TASKS and ledger still called `931e50c85` an uncommitted worktree; the bookkeeping is corrected here, and fresh re-review plus the separate transition remain pending. |
| `R6-GAP-TGG-TRUTH-PATH-ACTION` | `CTX-R6-12` — `truth_grounding_gap_flags_truth_path_action_before_read` | TO CREATE `docs/specs/r6/R6-GAP-TGG-TRUTH-PATH-ACTION/R6-GAP-TGG-TRUTH-PATH-ACTION-spec.md`<br>TO CREATE `docs/specs/r6/R6-GAP-TGG-TRUTH-PATH-ACTION/R6-GAP-TGG-TRUTH-PATH-ACTION-plan.md`<br>TO CREATE `docs/specs/r6/R6-GAP-TGG-TRUTH-PATH-ACTION/R6-GAP-TGG-TRUTH-PATH-ACTION-tasks.md` | BLOCKED | `R6-GAP-DET-OPAQUE-PARENT` | `R6-GAP-WPB-EMPTY-AUTHORITY` | Witness `e67d8b214`; controls wall `5618f7864`. |
...
|---|---|---|
| 2026-07-13 | `931e50c85` (landed review-fix candidate) | Narrowed the confidence exception to typed `ParentVisibleOrchestration + Low`, added the partial/mixed parent-visible Medium-confidence regression, and retained exact ordered proof passing all three focused controls, all `21` matching `dead_end_thrash` tests, and all `167` matching checkpoint tests. Fresh re-review returned one P2 bookkeeping finding because the TASKS and ledger still described this landed candidate as an uncommitted worktree. The bookkeeping is corrected while `R6-GAP-DET-OPAQUE-PARENT` remains ACTIVE pending fresh re-review and a separate transition. |
...
| 2026-07-13 | `59092df2a` | Reconciled the `R6-GAP-DET-OPAQUE-PARENT` packet docs after fresh `REVIEW CLEAN`; kept the gap active and preserved red, with exact witness/family reconfirmation next and no-code closure unavailable. |
| 2026-07-13 | `5618f7864` | Closed `R6-C.1-CONTROLS` after the wall reconciled all thirteen synthetic controls as `10 PASS / 3 preserved RED`; instantiated the three matrix-ordered named gap routes, activated only `R6-GAP-DET-OPAQUE-PARENT` at its packet-docs gate, kept the later gaps and `R6-REPLAY` blocked, and recorded no production change. |

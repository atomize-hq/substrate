...
**Ledger status:** ACTIVE — `R6-GAP-DET-OPAQUE-PARENT` (packet docs `REVIEW CLEAN`; witness reconfirmation next)

**Verified against:** `59092df2a` (`R6-GAP-DET-OPAQUE-PARENT` packet docs `REVIEW CLEAN`)
...
| `CTX-R6-04` | Opaque delegated parent does not become child thrash | PRESERVED RED / ACTIVE GAP | Exact control `dead_end_thrash_keeps_opaque_parent_orchestration_clear_without_child_activity` reached `ParentVisibleOrchestration` but failed the locked triple: actual `0 / Medium / Cleared`, unflagged, empty evidence, versus required `0 / Low / Cleared`. | Per the now-review-clean packet, rerun the exact control and then the owning `dead_end_thrash` family before impact analysis or any production fix; no-code closure remains unavailable for this first named gap. | `R6-GAP-DET-OPAQUE-PARENT` |

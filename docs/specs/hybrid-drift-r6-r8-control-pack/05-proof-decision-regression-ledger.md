...
| `CTX-R6-11` | Truth grounding long-turn/delegation invariance | PROVEN focused / OPEN opaque-parent control | Exact control `truth_grounding_gap_is_event_order_invariant_across_turn_shapes` passed with both representations at `80 / High / Active`, flagged; identical declared truth and event order reached the locked long-autonomous and many-short-conversational modes with equivalent `truth artifact hint:` and `command family: cargo` evidence semantics. The opaque-parent half remains untested. | Preserve the turn-shape result through the reviewed `truth_grounding_gap` family wall; execute the remaining opaque-parent control, and do not create `R6-GAP-TGG-TURN-INVARIANCE`. | `R6-C.1-CONTROLS` |

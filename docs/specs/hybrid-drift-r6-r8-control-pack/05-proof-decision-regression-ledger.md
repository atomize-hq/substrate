...
| `CTX-R6-06` | Frozen four-case dead-end corpus | INVARIANCE | Three cleared final postures and one recovered sticky tail | Preserve as invariance; do not call comparative improvement. | `R6-C.0A`, `R6-REPLAY` |
| `CTX-R6-07` | `semantic_goal_drift` context applicability | PROVEN | Structured goal, replan, delegation, and live-path scorer controls | Correct proof citation wording only; no reopen absent new witness. | `R6-C.0A` |
| `CTX-R6-08` | Semantic acceptance fixture integrity | PROVEN integrity, not behavior | Bounded corpus shape test | Describe separately from the one live analyzer-path acceptance test. | `R6-C.0A` |
...
| `CTX-R6-16` | `scoring/mod.rs` exact order | PROVEN source / not behavior-tested | Deterministic source sort | Do not claim test proof; add a test only if order is a closure contract. | `R6-C.0A` |
| `CTX-R6-17` | R6 terminal disposition gate | OPEN | Current docs allow “still open” in places | Replace with four terminal categories across root/R6/R7 authority. | `R6-C.0A`, `R6-CLOSE` |
| `CTX-R6-18` | Historical pre-cutover status prose | OPEN | R6 MAP preliminary section and root 2026-07-04 note conflict with live posture | Mark explicitly historical/superseded without deleting history. | `R6-C.0A` |

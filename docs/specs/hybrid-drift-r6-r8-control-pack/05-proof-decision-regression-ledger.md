...
| `CTX-R6-14` | `wrong_plan_branch` read-only/replan/delegation | PROVEN read-only / OPEN replan and delegation | Exact control `wrong_plan_branch_ignores_read_only_out_of_scope_exploration` passed with non-empty source/truth authority plus a read-only `sed` command naming a different path: `0 / Medium / Cleared`, unflagged, with empty evidence. | Execute the sanctioned-replan and opaque-parent controls; no `R6-GAP-WPB-READ-ONLY` route is required. | `R6-C.1-CONTROLS` |

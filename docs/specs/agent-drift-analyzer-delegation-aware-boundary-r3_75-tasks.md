...
- Repo-root sequencing and the delegation-boundary design doc already define `R3.75` as the
  immediate next packet and keep `R7` distinct from it.
- No `R3.75` implementation code has landed yet.

Validation gate for this phase:

- this task list is the `TASKS` phase artifact for review
- do not begin `R3.75-2+` implementation until the spec and plan are reviewed and accepted
...
- [ ] Task: Add analyzer-local delegation types and conservative marker harvesting
  - Acceptance:
...
- [ ] Task: Derive checkpoint-local delegation topology and child-work visibility
  - Acceptance:
...
- [ ] Task: Surface compact analyzer-owned delegation inspection if needed for packet proof
  - Acceptance:

/goal Land Packet `R5.5-5` from `docs/specs/r5/agent-drift-analyzer-session-progress-r5_5-tasks.md` in `/Users/spensermcconnell/.codex/worktrees/97a0/substrate` using a packet-scoped implementation -> commit -> review -> fix -> commit loop until the packet is review-clean.

You are the orchestration agent. Stay strictly scoped to Packet `R5.5-5` only, assuming Packets `R5.5-0` through `R5.5-4` are already landed.
...
Packet `R5.5-5` scope only:

- route `parent_visible_orchestration_progress(...)` through the shared normalization order
- make the normalization order explicit: build candidate -> apply delegation caps and limiting counter-evidence -> finalize exactly once
- preserve conservative delegated-parent status while aligning normalized shape with the shared finalizer behavior
- add delegated-parent regressions proving the path stays conservative after the normalization change

Out of scope:

- new direct-progress semantics unrelated to parent-visible normalization
- real-rollout corpus work
- delegation limiting-evidence promotion beyond the separate `R5.5-6` packet
- `R5.5-6+`
...
6. Commit the landed Packet `R5.5-5` implementation before any review subagent is dispatched.
...
13. Do not start Packet `R5.5-6`.

Required verification wall for Packet `R5.5-5`:
...
```text
/goal Implement Packet R5.5-5 only from `docs/specs/r5/agent-drift-analyzer-session-progress-r5_5-tasks.md` in `/Users/spensermcconnell/.codex/worktrees/97a0/substrate`, assuming Packets `R5.5-0` through `R5.5-4` are already landed

Use the `$incremental-implementation` skill.

You are landing only Packet `R5.5-5`:
- route `parent_visible_orchestration_progress(...)` through the shared normalization order
- make the normalization order explicit: build candidate -> apply delegation caps and limiting counter-evidence -> finalize exactly once
- preserve conservative delegated-parent status while aligning normalized shape with the shared finalizer behavior
- add delegated-parent regressions proving the path stays conservative after the normalization change
...
Execution rules:
- stay strictly within Packet `R5.5-5`
...
```text
/goal Review the already-landed Packet R5.5-5 implementation in `/Users/spensermcconnell/.codex/worktrees/97a0/substrate` against its authority docs and determine whether it is ready to keep.

Use the `$code-review-and-quality` skill.

Review only Packet `R5.5-5` from:
- docs/specs/r5/agent-drift-analyzer-session-progress-r5_5-spec.md
- docs/specs/r5/agent-drift-analyzer-session-progress-r5_5-plan.md
- docs/specs/r5/agent-drift-analyzer-session-progress-r5_5-tasks.md
- docs/specs/r5/agent-drift-analyzer-session-progress-r5_5-planning-input.md
...
Focus:
- correctness of the single shared normalization path for parent-visible progress
- preservation of conservative delegated-parent behavior
- signal sorting, dedupe, and normalized shape parity after the change
...
```text
/goal Address the review findings for already-landed Packet R5.5-5 only in `/Users/spensermcconnell/.codex/worktrees/97a0/substrate`, then rerun the relevant Packet R5.5-5 verification commands.
...
Rules:
- fix only the flagged Packet `R5.5-5` issues
- do not broaden into `R5.5-6+`
...
- implementation commit message example: `feat: normalize r5.5 parent-visible progress once`
- fix commit message example: `fix: address r5.5-5 review findings`

Your job is done only when Packet `R5.5-5` is committed and a fresh review subagent reports it review-clean.

...
Status: landed/closed on 2026-06-12 for the first sequential `R5.75` landing.
...
- [x] Task R5.75-0.2: Reconcile the `R5.5` task ledger so landed work is not still presented as
...
- [x] Task R5.75-0.3: Update root landing-order authority to name `R5.75` as the active pre-`R6`
      family.
...
- [x] Task R5.75-0.4: Run a cross-doc authority audit after the edits.
...
- [x] Task R5.75-0.5: Run baseline validation and optional control smoke before promotion.
  - Acceptance:
...
Satisfied on 2026-06-12 once all tasks above were complete and the authority audit proved no
touched doc still presented already-landed `R5.5` work as open or `R6` as currently ready to
open.

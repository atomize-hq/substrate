...
- [x] Task: Add `v0.2` export-facing manifest, file, row, and row-ref DTOs
  - Acceptance: `agent-session-compactor` defines explicit `v0.2` DTOs for the manifest-owned file registry, exported rows, and id-backed dedupe refs, with `session_id` stored on manifest file entries instead of repeated on rows and repeated `turn_id` strings compacted into file-scoped turn tables, without changing the internal path-backed normalization and dedupe contracts.
...
- [x] Task: Emit archival and compact row JSONL with `source_file_id`
  - Acceptance: `rows.archival.jsonl` and `rows.compact.jsonl` serialize `source_file_id` instead of `source_file`, row `session_id` duplication is removed in favor of manifest file-entry `session_id`, repeated row `turn_id` strings are replaced by file-scoped `turn_id_ref`, `manifest.json` reports `schema_version = "v0.2"` plus the deterministic file registry, and `summary.md` remains operator-readable.
  - Verify: `cargo test -p agent-session-compactor export_bundle -- --nocapture`
...
- [x] Task: Migrate dedupe audit export to id-backed row refs and validate registry sync
...
- [x] Task: Cut analyzer input over to `v0.2` and resolve ids at the load boundary
  - Acceptance: `agent-drift-analyzer` loads the manifest file registry and `v0.2` row/audit DTOs, resolves `source_file_id` into `{path, session_id, turns}` before sorting or validation, reconstructs the existing internal path-backed row and ref types, and fails clearly on unknown or duplicate file ids or turn refs.
...
- [x] Task: Preserve analyzer semantic behavior under the new bundle contract
...
- [x] Task: Gate the cutover on a bounded compactor-to-analyzer smoke run

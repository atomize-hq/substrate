...
- `~/.substrate/run/agent-hub/sessions/<orchestration_session_id>.json`, `~/.substrate/run/agent-hub/participants/*.json`, and `~/.substrate/run/agent-hub/handles/*.json` remains legacy compatibility input only during the cutover. They may fill torn-root gaps on read, but they do not outrank canonical session-root records and must not be treated as live-state authority.

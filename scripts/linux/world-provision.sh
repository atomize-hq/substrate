...
show_install_context_cmd() {
    ...
    for arg in "$@"; do
        if [[ "${redact_next}" -eq 1 ]]; then
            printf ' %q' '<redacted>'
            redact_next=0
        elif [[ "${arg}" == "--install-bootstrap-context-v1" ]]; then
            printf ' %q' "${arg}"
            redact_next=1
        elif [[ "${arg}" == SUBSTRATE_INSTALL_BOOTSTRAP_CONTEXT_V1=* ]]; then
            printf ' %q' 'SUBSTRATE_INSTALL_BOOTSTRAP_CONTEXT_V1=<redacted>'
        else
            printf ' %q' "${arg}"
        fi
    done
...
sudo_cmd() {
    if [[ -z "${INSTALL_PREFIX}" \
        || "${SUBSTRATE_HOME:-}" != "${INSTALL_PREFIX}" \
        || "${SUBSTRATE_ROOT:-}" != "${INSTALL_PREFIX}" \
        ...
        echo "Install bootstrap context projection changed before privileged tool dispatch." >&2
        return 2
    fi
    local tool="$1"
    shift
    ...
    if [[ -z "${env_path}" ]]; then
        echo "Unable to resolve privileged environment scrubber: env" >&2
        return 127
    fi
...
maybe_run_gateway_lifecycle_proof() {
    ...
    if [[ ${DRY_RUN} -eq 1 ]]; then
        ...
        show_install_context_cmd "${substrate_cli}" --install-bootstrap-context-v1 "${INSTALL_BOOTSTRAP_CONTEXT_V1}" world gateway sync
        show_install_context_cmd "${substrate_cli}" --install-bootstrap-context-v1 "${INSTALL_BOOTSTRAP_CONTEXT_V1}" world gateway status --json
        show_install_context_cmd "${substrate_cli}" --install-bootstrap-context-v1 "${INSTALL_BOOTSTRAP_CONTEXT_V1}" world gateway restart
...
if [[ "${INSTALL_BOOTSTRAP_CONTEXT_DECLARED}" -eq 1 && "${INSTALL_PREFIX_DECLARED}" -ne 1 ]]; then
    echo "internal world-provision child requires --home" >&2
    exit 2
fi

...
if [[ "${TASK_TYPE}" != "ops" ]]; then
    ...
    if [[ "${DRY_RUN}" -eq 0 ]]; then
        ...
        taskmeta_path="${WORKTREE_ABS}/.taskmeta.json"
        if [[ -f "${taskmeta_path}" ]]; then
            existing_task_id="$(jq -r '.task_id // empty' "${taskmeta_path}" 2>/dev/null || true)"
            if [[ "${existing_task_id}" != "${TASK_ID}" ]]; then
                die "Existing .taskmeta.json task_id mismatch: ${existing_task_id} (expected ${TASK_ID})"
            fi
            existing_created_from_sha="$(jq -r '.created_from_sha // empty' "${taskmeta_path}" 2>/dev/null || true)"
            existing_created_at_utc="$(jq -r '.created_at_utc // empty' "${taskmeta_path}" 2>/dev/null || true)"
            if [[ -n "${existing_created_from_sha}" ]]; then
                created_from_sha="${existing_created_from_sha}"
            fi
            if [[ -n "${existing_created_at_utc}" ]]; then
                created_at_utc="${existing_created_at_utc}"
            fi
        else
...
write_missing_last_message_stub() {
    ...
    if [[ -z "${codex_last_message}" ]]; then
        return 0
    fi
    if [[ -f "${codex_last_message}" ]]; then
        return 0
    fi
    mkdir -p "$(dirname "${codex_last_message}")" >/dev/null 2>&1 || true
    {
        printf '# Generated task summary (Codex last message missing)\n\n'
        printf 'This file was generated by `scripts/triad/task_start.sh` because Codex did not write `--output-last-message`.\n'

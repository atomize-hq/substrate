...
def validate_launchctl_not_found_streams(
    ...
    try:
        ...
        raise Stop("launchctl not-found stderr is not UTF-8") from error
    if (
        exit_status != 113
        or stdout_bytes
        or "Could not find service" not in stderr_text
        or label not in stderr_text
        or "domain for system" not in stderr_text
    ):
...
def validate_bounded_launchctl_not_found_streams(
    ...
    try:
        ...
        raise Stop("bounded launchctl not-found stderr is not UTF-8") from error
    if (
        exit_status != 113
        or stdout_bytes
        or "Could not find service" not in stderr_text
...
def validate_process_enumeration(
    enumeration: Any, expected_path: str
) -> list[dict[str, Any]]:
    expected_fields = {
        "total_process_count",
        "matching_pids",
    ...
    if not isinstance(enumeration, dict) or set(enumeration) != expected_fields:
        fail("process enumeration changed its closed raw-evidence shape")
    records = validate_process_snapshot_records(
        enumeration.get("process_records"),
        enumeration.get("process_snapshot"),
        enumeration.get("process_snapshot_sha256"),
    )
    if enumeration.get("total_process_count") != len(records):
        fail("process enumeration changed its exact record cardinality")
    matching = [
        record["pid"] for record in records if record.get("path") == expected_path
    ]
...
def validate_live_observation(value: dict[str, Any], probe: dict[str, Any]) -> None:
    ...
    if kind in ("filesystem_path", "unix_endpoint"):
        ...
        if (
            not isinstance(enumeration, dict)
            or enumeration.get("matching_pids") != []
            or not isinstance(enumeration.get("total_process_count"), int)
            or not isinstance(enumeration.get("process_records"), list)
            or not isinstance(enumeration.get("process_snapshot"), dict)
            or not is_sha256(enumeration.get("process_snapshot_sha256"))
        ):
            fail("root-install process absence is incomplete")
        validate_process_enumeration(enumeration, probe["identity"])
...
def decode_candidate_raw_observation(
    observation: dict[str, Any], probe: dict[str, Any]
) -> dict[str, Any]:
    ...
    if probe["kind"] in ("filesystem_path", "unix_endpoint"):
        ...
        if (
            ...
            or not isinstance(enumeration["total_process_count"], int)
            or enumeration["total_process_count"] < 1
...
def verify_manifest_shape(manifest: dict[str, Any], manifest_bytes: bytes) -> None:
    ...
    for observation, probe in zip(observations, probes):
        ...
        decode_candidate_raw_observation(observation, probe)
...
def load_success_cleanup_authority(
    ...
    bounded_control_stream_bytes(
        cleanup.get("launchctl_bootout_stdout"),
        "native cleanup launchctl bootout stdout",
    )
    bounded_control_stream_bytes(
        cleanup.get("launchctl_bootout_stderr"),
        "native cleanup launchctl bootout stderr",
    )
...
def validate_preclaim(
    ...
    for observation, probe in zip(observations, expected):
        validate_live_observation(observation, probe)

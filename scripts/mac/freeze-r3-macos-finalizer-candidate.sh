...
readonly REPOSITORY="/Users/spensermcconnell/__Active_Code/atomize-hq/substrate"
readonly BRANCH="feat/internal-host-orchestrator-world-dispatch-bootstrap"
...
readonly UPSTREAM_COMMIT="$(${GIT} -C "${REPOSITORY}" rev-parse '@{upstream}')"
readonly REMOTE_COMMIT="$(${GIT} -C "${REPOSITORY}" ls-remote --heads origin "${BRANCH}" | /usr/bin/awk '{print $1}')"
[[ "${SOURCE_COMMIT}" == "${STARTING_HEAD}" \
   && "${UPSTREAM_COMMIT}" == "${STARTING_HEAD}" \
   && "${REMOTE_COMMIT}" == "${STARTING_HEAD}" ]] \
    || fail "HEAD, tracking, or live remote diverged from the bound uncommitted implementation base"
...
REPO = pathlib.Path("/Users/spensermcconnell/__Active_Code/atomize-hq/substrate")
...
def observe_absence():
    ...
    for probe in absence_plan():
        ...
        if kind in ("filesystem_path", "unix_endpoint"):
            ...
            stderr_text = proc.stderr.decode("utf-8", errors="strict")
            if proc.stdout or "Could not find service" not in stderr_text or identity not in stderr_text or "domain for system" not in stderr_text:
            ...
            output.append({"kind": kind, "identity": identity, "enumeration": {"total_process_count": len(processes), "matching_pids": matching, "process_records": processes, "process_snapshot": snapshot_stream, "process_snapshot_sha256": snapshot_digest}})
        else:
...
for target, manifest_path in zip(targets, manifests):
    ...
    for recipe in manifest["recipes"]:
        ...
        for product in recipe["binary_products"]:
            ...
            for token in dep_info.read_text().replace("\\\n", " ").split():
...
env -i "${global_env[@]}" "${SWIFTC}" -O -whole-module-optimization -target arm64-apple-macos15.0 \

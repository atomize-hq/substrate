...
_BLA_make_braille_fill_animation() {
  ...
  # Active slot colors (rainbow)
  local seq=(
    $'\e[31m⠁\e[0m' $'\e[33m⠃\e[0m' $'\e[32m⠇\e[0m' $'\e[36m⡇\e[0m'
  ...
  for ((pos=0; pos<total_slots; pos++)); do
    for char in "${seq[@]}"; do
      local frame=""
      for ((left=0; left<pos; left++)); do
        frame+="${complete_color}"
      done
      frame+="${char}"
      local remaining=$(( total_slots - pos - 1 ))
      if (( remaining > 0 )); then
        frame+="$(printf '%*s' ${remaining} '')"
      fi
      frames+=("${frame}")
    done
  done

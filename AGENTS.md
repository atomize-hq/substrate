...
- **Linux**: Run `scripts/linux/world-provision.sh --profile release` to install `world-service` plus the matching `.service`/`.socket` units so systemd manages `/run/substrate.sock`. Dev installs invoke this script automatically (sudo prompts expected). The installers now create the `substrate` group, add the invoking user, reload the socket units so `/run/substrate.sock` is `root:substrate 0660`, and reapply named-user ACL bridge entries for current `substrate` group members when systemd recreates the socket. After provisioning, validate with `systemctl status substrate-world-service.socket`, `systemctl status substrate-world-service.service`, `sudo getfacl -cp /run/substrate.sock`, and `substrate world doctor --json | jq '.host.world_socket'`. Run `loginctl enable-linger "$USER"` so the socket-activated service is available after logout/reboot.
...
This project is indexed by GitNexus as **substrate-current** (35899 symbols, 72336 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/substrate-current/context` | Codebase overview, check index freshness |
| `gitnexus://repo/substrate-current/clusters` | All functional areas |
| `gitnexus://repo/substrate-current/processes` | All execution flows |
| `gitnexus://repo/substrate-current/process/{name}` | Step-by-step execution trace |

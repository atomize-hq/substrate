...
def main() -> int:
    ...
    for index, entry in enumerate(evidence_dispatches):
        ...
        require_oid(
            source.get("commit"),
            f"active_evidence_dispatches[{index}].source.commit",
        )
        require_oid(
            source.get("tree"),
            f"active_evidence_dispatches[{index}].source.tree",
        )
        if source.get("target_ref") != target_ref:
            fail(f"active_evidence_dispatches[{index}] source target_ref must match state")
    ...
    for index, entry in enumerate(verified_evidence):
        ...
        require_oid(
            source.get("commit"),
            f"verified_evidence[{index}].source.commit",
        )
        require_oid(
            source.get("tree"),
            f"verified_evidence[{index}].source.tree",
        )
        if source.get("target_ref") != target_ref:
            fail(f"verified_evidence[{index}] source target_ref must match state")

...
USER-AUTHORIZED READ-ONLY WALL CLASSIFICATION

After the blocked receipt
`orchestration/r2-3/receipts/R2-3ZR1-attempt-6.json`, the user explicitly authorized a read-only
proof-classification retry in the same task.

The task must preserve:

- local unpublished commit `199e1b1a5f3854934a08727f2846e028c065c5cb`;
- local tree `15e17d17fa1a9222488a695a0e4e360376e3556c`;
- accepted remote base `ec05167bd7bcb7be874af48fe33d6edb71492516`;
- clean task checkout, 1 ahead / 0 behind, and the existing proof/review artifacts.

This classification is read-only. It authorizes no tracked or untracked change in the task
checkout, no source/test/runner/control-pack edit, no amend/reset/rebase/merge, and no commit,
push, publication, native evidence, or R2-3Z start. All new evidence must be written under fresh
external disposable roots.

Using the same authenticated runner bytes, toolchain, command, environment contract, and isolated
full non-worktree-checkout conditions, compare the accepted base and local unpublished commit:

1. verify exact source and test-inventory deltas, including that the R2-3ZR1 subject adds no shell
   test and does not change the extra retained-worker test;
2. run authenticated serial and parallel wall attempts for both commits under distinct fresh
   evidence roots;
3. run the exact extra test
   ...
   repeatedly under the same explicit environment and both serial/parallel-relevant conditions;
4. separate runner/environment ineligibility (`evidence_write_failed`, `mount_teardown_failed`, or
   any successor condition) from test count/name/signature outcomes;
5. compare discovered test inventories and explain the 1309-versus-1322 difference without
   changing the frozen baseline; and
6. report whether the local subject causes a reproducible base-to-local transition, the accepted
   baseline is stale/incomplete for the current identical conditions, the extra failure is
   concurrency/environment-sensitive, or more evidence is required.

Do not treat an ineligible run as canonical proof. Do not bless a new baseline or repair any test.
Return a formal blocked/authority-required receipt with paths and SHA-256 digests for all new
classification artifacts and an exact recommended next authority. The receipt send to the meta
task remains the final tool action.

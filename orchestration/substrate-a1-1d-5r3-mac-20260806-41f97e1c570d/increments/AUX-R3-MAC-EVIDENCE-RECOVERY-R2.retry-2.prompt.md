...
You are the fresh top-level increment orchestrator for AUX-R3-MAC-EVIDENCE-RECOVERY-R2. You own this increment only.
...
- dispatch_nonce: 39ae4ba470520f8a9267e1dc628c36299820b612a3f6ef5e0606b3d767b86a86
- meta_thread_id: 019fd8c3-b12c-7e73-919f-898600ab0f64
- meta_host_id: local
- increment: AUX-R3-MAC-EVIDENCE-RECOVERY-R2
- packet_id: AUX-R3-MAC-EVIDENCE-RECOVERY-R2
- next_increment: AUX-R3-MAC-EVIDENCE-RECOVERY-R3
...
- expected base commit: bb098b6e265a4b6cadc4b7ca908c8b6ccf59f2b9
- expected base tree: 688ae2860d99d6d5cf1019e0d3ba27c7cc26b6cc
- required ancestor: bb098b6e265a4b6cadc4b7ca908c8b6ccf59f2b9
...
# R2 retry-2 compile-gate ownership correction

Amendment 0013 corrects a sequencing contradiction in the landed recovery plan. R2 proves only its
named shell/macOS cfg admissions and the installer's exact two-binary build/copy **source shape**.
R2 must not compile or repair `src/bin/substrate-lifecycle-macos.rs`, Cargo dependency closure,
AuditToken visibility, or FFI pointer typing; those are frozen R4-owned surfaces. Replace the donor
compile-surface test's lifecycle-binary Cargo invocation with: (a) native and
`x86_64-apple-darwin` checks of the R2-owned shell/ordinary host compile surface, and (b) static
assertions that the installer BUILD_FLAGS and fixed managed-copy branch name exactly
`substrate-lifecycle-control` and `substrate-lifecycle-macos`. Preserve the failing lifecycle-macos
compile observation as a mandatory R4 predecessor gate. R4 may not complete without compiling the
lifecycle-macos binary and resolving the recorded sha2, AuditToken, and pointer-cast failures inside
its own authorized surface.

The attempt-2 worktree `/Users/spensermcconnell/.codex/worktrees/bacb/substrate` is an additional
read-only partial-diff evidence source, bound to tracked diff
`200f5e0563ce1af63625d0ed33822520d96f9dcadedee6b32a7b843fdfd314f2` and untracked test hash
`6f926b83ce7263f0950ecdd98d451a85f80a2a496cb98e31503ce07954255781`. Never mutate or bulk-copy it.
Manually recreate only independently justified R2 hunks in the fresh worktree. All other retry-1
contract fences remain controlling, including the GitNexus no-refresh/manual fallback. Where the
retry-1 deterministic proof text requires compiling lifecycle-macos or all configured target bins,
this ownership correction supersedes only that command; it does not weaken any R2-owned check.

# R2 retry-1 adjudication envelope

This fresh retry supersedes only the GitNexus refresh behavior from attempt 1. The dirty attempt-1
worktree remains protected and must not be restored, cleaned, reused, or mutated. Begin from a new
exact-base worktree. Do **not** run `npx gitnexus analyze` or any command that injects/rewrites
tracked repository context files. Run available GitNexus query/impact/detect tools without refresh;
when they report degraded, stale, UNKNOWN, or no indexed symbol, preserve that result and perform
the contract-required complete manual exact-symbol/cfg/caller analysis. This is the authorized
index-only/manual fallback and does not weaken the exact R2 path/symbol/review gates. Any tracked
change to `AGENTS.md`, `CLAUDE.md`, or another unlisted path is again a contradiction.
...
Do not generate the next increment prompt and do not begin AUX-R3-MAC-EVIDENCE-RECOVERY-R3. The meta

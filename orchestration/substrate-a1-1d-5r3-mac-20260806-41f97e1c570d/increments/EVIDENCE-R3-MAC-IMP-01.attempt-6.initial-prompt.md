...
# EVIDENCE:R3-MAC-IMP-01 attempt-6 agent-operated native read-only contract
...
## Attempt-6 authority and recovered-source correction
...
ref instead of `detached`. Reverify it before use and keep it byte-clean. Do not edit, stage,
commit, push, merge, rebase, reset, clean, switch, or create any evidence byte inside the assigned
...
## Attempt-6 mandatory recovery gates
...
The historical attempts 1-5 and their blocked artifacts are diagnostic only and cannot satisfy any gate. In
...
## Attempt-6 bound platform prerequisites
...
## Attempt-6 verified platform-preparation inputs
...
source-relative native command. Use external target/output roots so it remains clean. The Stage-1

...
checkout. Bind the evidence task to the exact source ref, commit, and tree. For remote publication,
verify the live remote. For a local-only checkpoint, bind the exact clean worktree and local branch
ref and dispatch only where that local checkpoint is accessible; never push merely to transport
evidence.
...
- exact source ref, commit, and tree;
- publication mode and either live-remote or exact local-worktree/ref authority;

...
def main() -> int:
    ...
    publication_mode = source.get("publication_mode", "remote")
    if publication_mode == "remote":
        if "local_ref" in source or "worktree" in source:
            fail("remote evidence source must not contain local_ref or worktree")
        if require_oid(source.get("live_remote"), "source.live_remote") != commit:
            fail("source.live_remote must equal source.commit")
    elif publication_mode == "local":
        if "live_remote" in source:
            fail("local evidence source must not contain live_remote")
        if require_oid(source.get("local_ref"), "source.local_ref") != commit:
            fail("source.local_ref must equal source.commit")
        worktree = require_string(source.get("worktree"), "source.worktree")
        if not is_absolute_platform_path(worktree):
            fail("source.worktree must be an absolute platform path")
    else:
        fail("source.publication_mode must be remote or local")

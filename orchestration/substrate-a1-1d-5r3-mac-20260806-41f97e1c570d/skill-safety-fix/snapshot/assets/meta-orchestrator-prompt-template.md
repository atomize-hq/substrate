...
2. Read the reusable protocol from the absolute {{SKILL_PATH}}. Do not assume the task worktree
   contains `.agents/`; repo-local ignored skills are not materialized by Git worktree creation.
   Snapshot the required templates, references, and validator scripts into durable orchestration
   state and record their source paths and SHA-256s before dispatch.

...
1. Resolve the absolute `.agents/skills` suite root from this loaded `SKILL.md`. This is a
   repository-local skill suite; never install or register any of it as a global Codex skill.
...
binding or start authority to any newly created meta, increment, or evidence task, hydrate and
verify the complete repository-local `.agents/skills` suite in that task's assigned worktree:
...
Run the script from the authoritative repository-local orchestration skill. It copies the complete
sibling skill suite, not only `orchestrate-top-level-tasks`. Persist its reported source, target,
skill count, skill names, file count, and digest, and do not bind the task if hydration fails. The
task's initial prompt must remain self-contained until the next turn can load the hydrated suite.
...
6. Resolve the meta task's exact assigned worktree, hydrate the complete repository-local skill
   suite into it, verify the hydrated digest, and persist the hydration record.
...
6. Resolve and record the real thread ID, host ID, exact assigned worktree, nonce, and expected
   base.
7. Hydrate the complete repository-local skill suite into the fresh worktree, verify its digest,
   and persist the hydration record before granting any authority.
...
the complete repository-local skill suite in every evidence-task worktree before sending identity
binding or read-only start authority.

...
- Resolved feature dir: `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/`
- Resolved ADR paths:
  - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/ADR-0001-fixture.md`
...
- Staged tracked-output candidates (write only these under logs):
  - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/staged/pre-planning/workstream_triage.md`
- Direct writes to canonical tracked paths are forbidden.
- Logs allowed (untracked only): `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/`
...
You are the Workstream Triage agent for orchestrated_staged_candidate.
...
- Do not modify any tracked pack files *except* `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/pre-planning/workstream_triage.md`.
...
- `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/pre-planning/spec_manifest.md`
- `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/pre-planning/impact_map.md` (if it exists; otherwise use upstream handoff/scratch in Phase A)
- `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/pre-planning/minimal_spec_draft.md` (if it exists; otherwise use upstream handoff/scratch in Phase A)
- `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/pre-planning/ci_checkpoint_plan.md` (if it exists)
- `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/tasks.json`

Allowed writes:
- Tracked (canonical): none. Do not write tracked files directly.
- Staged candidate (logs-only; promoted later by runner/wrapper): `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/staged/pre-planning/workstream_triage.md`
- Logs only (untracked): you may write under `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/**` only.
- Do not edit any other tracked files directly.

Overlap execution model (required):
- Phase A (start immediately; logs only):
  - Orchestration note:
    - This agent may be launched as soon as `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/min-spec-draft/handoff.md` exists (before `impact_map.md` / `minimal_spec_draft.md` / `ci_checkpoint_plan.md` are written).
  ...
  - Compute pack-derived Work Lift v1 (impact-map-based) and treat it as an initial sizing signal:
    - From repo root, run and capture outputs into logs:
      - `make pm-lift-pack PACK=".tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate" > ".tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/pm_lift_pack.txt"`
      - `make pm-lift-pack PACK=".tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate" EMIT_JSON=1 > ".tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/pm_lift_pack.json"`
    - Eligibility note:
      - `pm-lift-pack` is only impact-map-derived when `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/tasks.json` indicates strict packs (`meta.slice_spec_version >= 2`).
  ...
  - Optional (recommended): capture discovery-time lift (vector-authored) from the ADR or intake:
    - Prefer ADR:
      - If exactly one ADR path exists in `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/tasks.json` (`meta.adr_paths`), run:
        - `make pm-lift-intake FILE="<ADR_PATH>" > ".tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/pm_lift_intake.txt"`
        - `make pm-lift-intake FILE="<ADR_PATH>" EMIT_JSON=1 > ".tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/pm_lift_intake.json"`
  ...
  - Create and iteratively refine:
    - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/workstream_triage_draft.md`
  - If present, read upstream handoff notes as inputs:
    - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/spec-manifest/handoff.md`
    - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/impact-map/handoff.md`
    - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/min-spec-draft/handoff.md`
    - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/CI-checkpoint/handoff.md`
- Phase B (finalization gate; still logs only):
  - Before treating the draft as “final for pre-planning”, poll until BOTH are true:
    - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/CI-checkpoint/last_message.md` exists, and
    - `git status --porcelain=v1 -- ".tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate"` is empty.
  ...
  - After the gate clears, re-read `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/pre-planning/impact_map.md` (not just `logs/impact-map/handoff.md`) before finalizing the tracked artifact.
  - Once the gate clears:
    - Write/overwrite the staged candidate: `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/staged/pre-planning/workstream_triage.md`
...
0) Planning workstream IDs (PWS) (required):
   ...
   - `SLICE_PREFIX` source of truth:
     - Use the slice prefix explicitly stated in `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/pre-planning/minimal_spec_draft.md` (Draft slice skeleton section).
  ...
  - Embed exactly one fenced JSON block in the staged candidate artifact (`.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/staged/pre-planning/workstream_triage.md`) using these markers:
     - `<!-- PM_PWS_INDEX:BEGIN -->`
     - `<!-- PM_PWS_INDEX:END -->`
...
2) Proposed planning workstreams (PWS):
   ...
   - If `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/pre-planning/minimal_spec_draft.md` contains `## Draft slice skeleton (pre-planning only)`, treat it as the starting point for slice naming/IDs, then explicitly encode the post-triage result in `accepted_slice_order`.
3) Sequencing + gates:
...
5) Evidence links:
   - Link to the stable step completion sentinels:
     - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/spec-manifest/last_message.md`
     - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/impact-map/last_message.md`
     - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/min-spec-draft/last_message.md`
     - `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/CI-checkpoint/last_message.md`
...
6) Slice skeleton recommendations (required):
   - If lift/impact indicates the slice skeleton should change (more/fewer slices, split/merge, different seam boundaries):
    - Propose explicit edits as recommendations inside the staged candidate artifact (`.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/staged/pre-planning/workstream_triage.md`), not by editing `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/pre-planning/minimal_spec_draft.md`.
...
Output:
- Ensure `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/workstream_triage_draft.md` is readable and structured (headings + bullets; no prose essays).
- Ensure `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/staged/pre-planning/workstream_triage.md` exists and is readable/structured.
- Optionally write/overwrite: `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/logs/workstream-triage/handoff.md` as a short “executive summary” for the operator.

Closeout validation:
- Do not write `.tmp_pm_script_tests/run_planning_agent/orchestrated_staged_candidate/pre-planning/workstream_triage.md` directly.

...
- Resolved feature dir: `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/`
- Resolved ADR paths:
  - `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/ADR-0001-fixture.md`
...
- Staged tracked-output candidates (write only these under logs):
  - `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/CI-checkpoint/staged/pre-planning/ci_checkpoint_plan.md`
  - `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/CI-checkpoint/staged/tasks.json`
- Direct writes to canonical tracked paths are forbidden.
- Logs allowed (untracked only): `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/CI-checkpoint/`
...
You are the CI Checkpoint Planning agent for ci_checkpoint_plan_only.

Goal:
- Produce a **pre-planning first pass** CI checkpoint plan at:
  - `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/pre-planning/ci_checkpoint_plan.md`
- Ensure `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/tasks.json` reflects the required baseline for this pack:
...
- `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/tasks.json`
- `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/pre-planning/impact_map.md`
- `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/pre-planning/spec_manifest.md`
- `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/pre-planning/minimal_spec_draft.md`
- `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/pre-planning/ci_checkpoint_plan.md` (if it already exists)
...
- Staged candidates (logs-only; promoted later by runner/wrapper): you may write/overwrite only:
  - `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/CI-checkpoint/staged/pre-planning/ci_checkpoint_plan.md`
  - `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/CI-checkpoint/staged/tasks.json` (only if required to satisfy schema-v4 cross-platform planning requirements)
- Logs (untracked; scratch + orchestration handoff): you may write under `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/CI-checkpoint/**` only.
- Do not edit any other tracked files directly.

Preflight (required; do first):
1) Read `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/tasks.json` and ensure the pack baseline is set for pre-planning:
...
3) If any of the above is missing or wrong, prepare a staged candidate for `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/tasks.json` containing only those field changes.
...
- Phase A (start immediately; logs only):
  - Draft checkpoint grouping and gates as scratch:
    - `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/CI-checkpoint/scratch.md`
  - Emit an orchestration handoff signal once you have a usable checkpoint outline:
    - Write/overwrite: `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/CI-checkpoint/handoff.md`
...
- Phase B (staged candidate write gate; required):
  - Before writing staged candidates, poll until BOTH are true:
    - `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/min-spec-draft/last_message.md` exists, and
    - `git status --porcelain=v1 -- ".tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only"` is empty.
  ...
  - After the gate clears, re-read `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/pre-planning/impact_map.md` (not just upstream handoff/scratch artifacts) before writing the staged candidates.

Tracked output requirements (pre-planning first pass; required):
1) Write/overwrite `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/CI-checkpoint/staged/pre-planning/ci_checkpoint_plan.md` using the template:
   - `docs/project_management/system/templates/planning_pack/ci_checkpoint_plan.md.tmpl`
2) Slice-awareness rule:
   - If `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/tasks.json` already defines slice integration tasks (`*-integ`), then:
     ...
     - Validate mechanically (must pass):
       - `python3 docs/project_management/system/scripts/planning/validate_ci_checkpoint_plan.py --feature-dir ".tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only"`
   - If `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/tasks.json` does NOT yet define slices, then:
     - Still write a useful first-pass plan:
       ...
       - prefer the slice ids from `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/pre-planning/minimal_spec_draft.md` `## Draft slice skeleton (pre-planning only)` when populating the machine-readable JSON `checkpoints[].slices` list,
...
- If the pack lacks enough information to choose code-grounded boundaries, record follow-ups in:
  - `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/pre-planning/ci_checkpoint_plan.md` under a “Follow-ups” section, and
  - `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/logs/CI-checkpoint/scratch.md` (evidence and rationale).
...
Closeout validation:
- Do not write `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/pre-planning/ci_checkpoint_plan.md` or `.tmp_pm_script_tests/run_planning_agent/ci_checkpoint_plan_only/tasks.json` directly.
